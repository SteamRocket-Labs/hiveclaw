...
| 检索结果不是 source-bound evidence | `TruthEvidencePackV1` 增加 `snippets`，Truth Search 输出 citations/source refs/snippets，prompt context 只渲染证据，不授予权限 | `backend/app/runtime/ccplus_contracts.py`、`backend/app/services/truth_search_service.py` | `pytest tests/services/test_truth_search_service.py -q` 通过；全量后端 `pytest tests -q`：`5324 passed, 2 skipped, 4 warnings` |
