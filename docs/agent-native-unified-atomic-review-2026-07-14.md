...
| 2 | `EVID-G2-*` | 14 leaf / 0 Missing | `in_progress`：13/14 closed；`EVID-G2-016/017` 已证明 Task、commentary persistence/replay 与 A2A read-only；`EVID-G2-018` 已部署并证明 live bridge 转发；`EVID-G2-019` 的 suffix/tail-watermark 已发布，canary D 再定位首个 backfill readiness 仍等待完整历史，独立 `liveReady` 已本地 Green | 发布 first-snapshot `liveReady` source；真实模型必须无刷新显示 accepted input/progress/Task/final，raw task/progress tool 不出现，reload 后顺序、折叠和 render ownership 等价；不得用 fixture、REST reload 或 deployment success 代替 live browser path proof |
...
context_read_receipt:
  ...
  source_baselines:
    hive_head: "9ad195fc3089 + owned typed live-tail diff; deployed bridge source=9ebb51f1bac1"
    freecode_head: "7dc15d6c8fb0c40c7fcc02ce9b58204324252632"
    codex_head: "5c19155cbd93bfa099016e7487259f61669823ff"
...
- local acceptance：baseline 首次 Red=`2 failed / 3 passed`；baseline Green focused Session disclosure/hydration=`6 files / 194 tests`；commit `70c4a12f2` 的 clean exact-source archive（含 `.github` harness contract）=`120 files / 730 tests`。tail-watermark follow-up Red 同样为 `2 failed / 3 passed`，Green focused transport/hydration=`6 files / 36 tests`；commit `d29f52493` clean exact-source frontend full=`120 files / 730 tests`。first-snapshot readiness Red=`2 failed / 12 passed`，Green focused=`2 files / 14 tests`、相关 Session suite=`4 files / 133 tests`、共享工作树 frontend full=`122 files / 739 tests`，commit `9ad195fc3` 已三服务发布。live-tail Red=backend collection error + frontend `2 failed / 11 passed`；Green backend Session/WebSocket=`43 passed`、frontend focused=`4 files / 27 tests`、当前共享工作树 frontend full=`122 files / 744 tests`，ruff/format 与 architecture line budget 通过。`npm run build` 通过，AgentDetail=`340022/380000` bytes、gzip=`93847/115000`，vendor=`591449/620000`、gzip=`186474/200000`；`git diff --check` 通过。
...
- status：`in_progress-local-green-live-tail`。suffix baseline、tail-watermark 与 first-snapshot `liveReady` source 已部署；canary F 证明首个 REST page 504 时仍没有 live cursor。只有包含 typed `live_tail` 的 exact commit 三服务发布后，真实模型在同一未刷新页面依次出现 accepted input、公开 progress、Task live 变化和 final，raw task/progress tool 为零，terminal Task/Run 正确折叠，hard reload 顺序与字节等价，才能关闭 `SES-CONSUMER-001`。
