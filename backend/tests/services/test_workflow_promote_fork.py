...
async def test_agent_proposal_lands_as_draft_never_active(service, tenant_id):
    agent_id = uuid.uuid4()
...
async def test_approval_requires_a_human_approver(service, tenant_id):
    proposal = await service.submit_promote_proposal(
        tenant_id=tenant_id, agent_id=uuid.uuid4(), definition_data=_definition_data("needs-user")
    )
    with pytest.raises(PermissionError):
        await service.approve_promotion(proposal.id, tenant_id=tenant_id, approver_user_id=None)


async def test_approval_activates_with_provenance(service, tenant_id):
    run_id = uuid.uuid4()
    proposal = await service.submit_promote_proposal(
        tenant_id=tenant_id,
        agent_id=uuid.uuid4(),
    ...
    approved = await service.approve_promotion(proposal.id, tenant_id=tenant_id, approver_user_id=uuid.uuid4())
    assert approved.status == "active"
    assert approved.promoted_from_run_id == run_id
    assert approved.definition_hash == proposal.definition_hash


async def test_approval_reruns_capability_check(service, tenant_id):
    """Approval re-runs compile/admission with the tenant's leaf catalog —
    an unauthorized leaf fails the promotion (fail-closed)."""
    proposal = await service.submit_promote_proposal(
        tenant_id=tenant_id, agent_id=uuid.uuid4(), definition_data=_definition_data("bad-leaf")
    )
...
async def test_fork_returns_ephemeral_and_keeps_original_hash(service, tenant_id):
...
async def test_fork_of_revoked_definition_refused(service, tenant_id):
    ...
    with pytest.raises(DefinitionLifecycleError):
        await service.fork_to_ephemeral(tenant_id=tenant_id, name="dead", agent_id=uuid.uuid4())
