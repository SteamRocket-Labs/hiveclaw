...
六路并行深审合成：①架构北极星（CCPlus）②Memory + QKV Attention Control ③Plugin 系统 × CC 插件市场 ④Personal KB ⑤HR Agent 兼容性 ⑥Loop / Target Mode 对标考证。全部对标 FreeCode 第一基线（/Users/example-owner/vc-saas/free-code-main）+ claude-code-org 交叉核对 + codex-rs delta。每项发现均有 file:line 证据，标注 Fact / Inference / Speculation。
...
| 2026-07-09 | §7.3.2 M1-M3 BaseLevel 线（sidecar 环+credit → 幂律公式 → FeedbackCredit 接线） | ✅ 已闭环 | Red: `tests/memory/test_base_level_activation.py` collection error + `test_feedback_credit_reaches_recently_activated_sidecar_entries` failed; Green: `pytest tests/memory tests/evals tests/runtime tests/services/test_session_feedback.py -q` → `1093 passed, 19 skipped`。M1：lifecycle sidecar 加 `recent_accesses` K=8 环形数组 + `credit` 字段（serde 兼容旧记录），`bump_access` 写环、`create_if_missing` 覆盖 knowledge/milestone 页，**读侧 join 首次接通**（`read_access_telemetry` 原零调用者 → retriever `_join_access_telemetry` 在打分前 join、bump 后置保证单 turn 读 pre-turn 态）。M2：`_usage_heat`（0.05 tie-break）替换为 `_base_level` 幂律公式（d=0.5、min-age 1min、log 饱和 20、weight 0.2），`ActivationContext.now` 可注入时钟；**发现并修复排序饱和缺陷**：score 在 1.0 clamp 导致高分区 tie 退化为检索顺序 → `ActivationDecision.raw_score`（未 clamp）供排序、display score 契约不变。**M0 对照门通过**：无 sidecar 基线逐位不变（recall 0.833333/MRR 0.857143）；seed 遥测后 api-timeout headroom case recall 0→1.0、aggregate recall/MRR 真涨（`test_recall_eval_base_level_flips_headroom_case_and_raises_aggregate`）；eval runner per-case sidecar 快照隔离（case 间 bump 不串味）。M3：`apply_feedback_credit(_to_recent)` 接通 heat_delta 断环——owner useful/misleading → session 窗口内激活过的条目 credit ±0.5（只写 sidecar 不触 MD/Memory Gate），`credited_entry_ids` 进 feedback sidecar 可查；W_t（M4）落地后收窄到精确工作集成员 |
| 2026-07-09 | §7.2-F 瘦身收尾（死模块+台账合并） | ✅ 已闭环 | 部分1：6 个 test-only 死模块退役（self_evolution_audit/proactive_employee_loop/charter_proposals 服务层/template_seeder/heartbeat_reflection_backfill/extract_queue_replay，连带 5 测试文件）+ is_pack_enabled 死 shim 删 + CLAUDE.md steward-loop 行修正；**假阳性甄别**：office_workflow_examples 保留（其"配对测试"实为活的 workflow admission/compiler/runtime 真 PG 集成测试的共享 fixture——正是审计"假阳性别砍"精神）；external_capabilities 三文件因 C4 接线保留。部分2：台账工厂合并——cache_decision_ledger(37L)+runtime_decision_ledger(70L)+authorization_decision(46L) 并入 decision_ledger（同 schema 同行为，8 个消费文件 import 重定向），outcome.py(25L) 内联唯一消费者 loop_guard；4 文件退役。**assembly-state 持久化裁剪判定不做**：P1-4/6/7 刚把 cache/agent_cycle/context_artifacts 接进前端读面，persist 已是条件写，进一步裁剪的读者矩阵风险大于 DB 字节收益（§0.2 登记项非必改，此为显式裁决）。验证：runtime+kernel+tools 全量 `1424 passed, 19 skipped` |
| 2026-07-09 | §7.2-B Loop 四件全对齐（基线 CC /loop 两代） | ✅ 已闭环 | B1 `/loop <interval> <prompt>` 命令层（同 /schedule 治理：creator+plan gate；建 interval trigger 落库+**立即先跑一次**走 daemon 正常 fire 序列只跳时间判定，preflight/failure_policy/budget 全过——对齐 loop.ts:67）；B3 `delivery=same_session`（fire 投递进源 session 作下一 turn=CC 一代"塞进当前 session 队列"语义；busy 排队不并发；session 缺失回退 new_invocation；默认路径逐字节钉死；source_session_id 由运行时注入模型永不复述）；B2 self-pace（`schedule_wakeup(delay_seconds, prompt, stop)` 工具=CC 二代 dynamic ScheduleWakeup 等价：clamp [60,3600]、once trigger 骑 B3 same_session 轨、每 session 单 pending 新调度替换旧、stop 取消、fire 过完整 trigger 治理+budget 准入；CAPABILITY_MAP `agent.trigger.modify` 三处+audit 清零；`/loop` 无 interval → self-pace 启动=投递 prompt+节奏契约进本 session，模型自调度到 stop）；B4 loop×goal 组合钉死（same_session loop turn 完成 → Active goal 的 continuation 接管——wakeup 是心跳、goal 是方向）。验证：B 面 42 passed + B4 16 passed |
| 2026-07-09 | §7.2-G Budget Plane 收窄审计+breaker 四维接活 | ✅ 已闭环 | **事实修正：Step1-7 主体 07-04 已实现合入 main（`7bcd9b10d`+`5e53c988a`），§7.2-G"先落地"是依赖表述**。本轮=一致性审计（docs/runtime-budget-conformance-audit-2026-07-09.md 逐条 §7/§8/§9/§10/§7.7/§8.7）+ 补真实缺口：§10 breaker 四维（max_failures/max_needs_reconciliation/max_child_failure_ratio/max_parent_invocations）从硬编码常量转 policy 驱动（tenant 可覆盖+内置默认=事故校准 5/3/0.5+§9.1 的 16/16/64/24），settle 后+wake 前双点真实 enforce（纯函数 breaker+circuit_break event+pending task 取消）；failures/needs_reconciliation=ground-truth 查询推导（拒绝易漂移散落计数器）；parent_invocations=真计数器单一写入点；§7.4 补 root_runtime_task_id/budget_snapshot_json 两列（单头 migration）。**A3/B2 的安全门前提由此确认成立**。验证：27 passed/24 skipped（skip=Docker 门控，CI 跑） |
| 2026-07-09 | §7.2-A Goal Mode 九件全对齐（基线 Codex ext/goal） | ✅ 已闭环 | 全 TDD。A9 `persist_session_goal_from_tool`（goal_start 真落库+active supersede）；A1 `update_goal` 工具（complete/blocked/paused/active+summary+objective 回写，CAPABILITY_MAP 三处 `agent.goal.modify`，audit 清零）——**模型自判完成的回写桥首通**；A5 `get_goal`（`agent.goal.read`）；A2 continuation prompt 3 行→三审计段（Completion/Blocked/Fidelity，vendor-neutral，contract 测试钉结构）；A4 token 记账接活（真实来源 engine `accumulated_tokens`→invoker→web_chat `turn_tokens_used` metadata→桥→`account_goal_tokens`，超 budget→BUDGET_LIMITED）；A6 连败宽容（provider_error/turn_abort/persistence_error 可重试 <3，`mark_goal_blocked_if_repeated` 接活 ≥3→BLOCKED，clean turn 归零）；A7 objective steering（`objective_updated_pending`→下轮 prepend re-orient prompt）；A8 resume 行为锁（恢复标记的 completed turn 照常续跑）；**A3 自主续跑循环打通**（`goal_continuation` 完成 turn 允许续跑=idle→continue 直到终态；`recursive_goal_continuation` 阻断退役 grep 零残留；终止链=cap/token budget/blocked/terminal 映射+budget-plane run 继承）。验证：goal 面 145 passed + A3 后 122 passed |
| 2026-07-09 | §7.3.2 M7 TaskModulation 智能级（attention set 声明） | ✅ 已闭环 | 最 AI-native 形态：声明由模型本人产出——`goal_start`/`update_goal` 工具新增 `attention_set` 参数（调工具的就是 LLM，零额外调用，L1 判断归模型/平台只记指针）。W_t 新增 pinned 种子语义（不衰减不剔除，goal 终态 complete/blocked/cancelled 自动 clear，新声明 supersede 旧声明）；声明主题经 `slugify` 解析到 knowledge page id（图内享 PPR 扩散，未解析保留字面自 boost）；持久化 schema ⊆{ref,strength,last_turn,ts,pinned} ACL 边界不变。验证：goal+memory 面 473 passed。**至此主线 2（M0-M8）九件全部闭环** |
| 2026-07-09 | §7.3.2 M8 消费契约兑现 + 统一 score_trace（最后硬门） | ✅ 已闭环 | Red: `test_kb_ordering_consumes_working_set_activation` failed; Green: `pytest tests/runtime/test_personal_knowledge_activation.py tests/memory tests/evals tests/runtime tests/services/test_memory_service.py -q` → `1112 passed, 19 skipped`。①retriever 出口：M2/M5 已让激活分真实决定排序（raw_score 排序修复 + 顺序改变断言）②KB 出口：invoker KB hint 路径 `_apply_working_set_activation_to_kb_candidates`——W_t strength 命中 source_ref 的文档按有界乘法因子（max ×1.3，设计 [0.8,1.6] 界内）rerank，**recorded candidates 与 prompt hint 均消费重排后顺序**（record-only 即 QKV 死因，测试钉死 recorded[0] 与 hint 顺序一致改变）③KB 激活 refs 经 `touch_working_set` 进 W_t（同 turn 不推进 turn_index/不衰减——一个 turn 恰好一次衰减）④统一 score_trace：retriever 侧 activation_reasons（goal_relevance/base_level/context_boost）+ KB 侧 activation_score_trace{base_score, context_boost, activation_rank_score, reason} 全落 |
| 2026-07-09 | §7.3.2 M6 TaskModulation 确定性级（goal_terms 首次真填） | ✅ 已闭环 | Red: `test_activation_context_carries_active_goal_terms` failed; Green: `pytest tests/services/test_memory_service.py tests/memory tests/evals tests/runtime -q` → `1111 passed, 19 skipped`。`_load_active_goal_objective`（AgentSessionGoal active 查询，fail-open）→ `_goal_terms_from_objective`（≥3 字符 + 最小 stopword 过滤）→ `_attach_goal_terms` 接进 `build_memory_context` 主路径——`ActivationContext.goal_terms` 自诞生以来首次被真实填充，`goal_relevance` 不再是 query overlap 的误命名。零 LLM、每 turn 一次轻量查询、goal 缺失零影响 |
| 2026-07-09 | §7.3.2 M4-M5 ContextBoost 线（W_t 会话工作集 → PPR 扩散） | ✅ 已闭环 | Red: `tests/memory/test_context_boost_activation.py` collection error + `test_build_memory_context_evolves_session_working_set` failed; Green: `pytest tests/services/test_memory_service.py tests/memory tests/evals tests/runtime tests/services/test_session_feedback.py -q` → `1117 passed, 19 skipped`。M4：新模块 `memory/session_working_set.py`——W_t 每 turn 衰减 0.8、激活 refresh=1.0、floor 0.1 剔除、cap 32、原子写 `memory/control/working_sets/<session>.json`；**ACL 硬边界钉死**（持久化项 schema ⊆ {ref,strength,last_turn,ts}，永不含正文）；生产接线=`build_memory_context` 每 turn load→注入 `ActivationContext.working_set`→检索后 advance+save（非孤儿，spy 测试断言 turn 间演化）。M5：retriever `_join_context_boost`——W_t seeds → `personalized_pagerank` 复用 → **非 seed 峰值归一**（seed 质量支配性会把邻居压零；W_t 成员自身按 strength 直接热）→ `context_boost` 因子进 scorer（weight 0.15 有界）。eval 门三证：①消歧翻转（新 headroom case：词汇平局的 checklist 对、release 词频堆砌 baseline 赢，W_t=platform-gate 语境下 gate-review 反超，recall 0→1.0）②**不劫持守门**（2hop case 保持 2/3——有界 boost 不得把零词汇页推过真实词汇命中页，此为负向验收）③aggregate recall 0.740741→0.851852 / MRR 0.75→0.875 真涨。基线重锚（fixture 15 页 9 cases）；M3 的 credit 应用面待 M8 收口时切换到 W_t 精确成员 |
...
| 2026-07-09 | §7.2-C Plugin CC 市场六件全对齐（基线 FreeCode schemas.ts） | ✅ 已闭环 | **事实修正：07-08 审计"≈15%"快照已被并行线超越，开工时 C1-C3 已 ~55%，本轮补齐至六件全闭环**（commits `284513ac7` C1/C2/C3/C5 + `a42470971` C4/C6）。C1 marketplace 仿冒防护（skeleton 归一化 case/homograph/affix 检测+可注入信任列表，警告进 trust-gate review 不硬拦——vendor-neutral，CC 硬编码官方列表）；C2 六种 source 全部拉进 quarantine（github Contents API/generic git 加固浅 clone[hooks+fsmonitor 禁用、ext:// 禁、credential.helper 置空、剥 .git、timeout+size/count/depth 上限]/npm 只读 tarball 永不跑 lifecycle scripts[**超前 CC：其 npm 仍是 TODO**]/url=marketplace.json 清单语义照 CC/file+directory 拒 symlink 且限 allowed_roots）；C3 插件体 materialize 进 agent workspace+`${CLAUDE_PLUGIN_ROOT}`/`${user_config.KEY}` CC-faithful 替换（函数式+敏感掩码）接进组件激活；C5 adapter 全格式矩阵（directory+manifest MERGE 非互斥、inline command、agents/skills 清单形态、三种 hook 形状、mcpServers path/dict/array）；**C4 孤儿转正 E2E**：`plugin_source_adapter.stage_cc_plugin_import`（fetch→quarantine→`load_cc_plugin_bundle` 首个生产调用者→Trust Gate review）+ `POST /enterprise/external-capabilities/plugin-imports` 真实 admin 入口——**接线时堵掉一个 fail-open**（`_within_allowed_roots(None)`=True，路由必须显式 `allowed_roots=[AGENT_DATA_DIR]`，否则租户 admin 可经 file 源读任意服务器文件进 review 证据；**根修 `aee5d0084`**：守卫本体翻 fail-closed——无 roots 即拒本地读，缺省路径补红测钉死[此前零覆盖]，全量 5732 passed）；C6 slash_command→单文件 skill 包投影（复用 `install_active_skill_package`，经 `_dynamic_skill_commands` 自动成 / 命令，零新 runtime 基建）+ hook 激活/清单行显式 `runtime_execution="not_yet_supported"`（registration→executor 链未建，明示而非装死）。验证：reviews API+adapter E2E+activation 套件 23 passed；grep：`load_cc_plugin_bundle` 唯一生产调用者=adapter:93、adapter 唯一生产调用者=API 路由:31、router 挂 main.py:853 |
| 2026-07-09 | §7.2-D Codex 工程 delta 三件 | ✅ 已闭环 | commit `119d71261`。D3 声明式 execpolicy 规则引擎替换 governance 手写危险命令 regex 块——规则即数据（pattern/capability/decision/reason+自校验 match examples），first-match 语义与 capability 路由逐字节保持（governance 套件全绿）；D1 SandboxProfile 四档真接 `subprocess_sandbox` builder（read_only 拒写、workspace_write 只绑声明的可写根、network 开关真生效、full_access_local_only 保持 legacy 形状；行为级测试在 macOS sandbox-exec 下真跑；Railway 仍锁 vercel_sandbox，provider 选择不动）；D2 单命令 shell 升权流 `command_escalation_service` 复用现有 approval 基建——精确命令、run-once、无常设授权，审计带原文命令与 resolver（`request_shell_escalation` 工具挂载+taxonomy 注册随 goal 工具批落地） |
| 2026-07-09 | §7.2-E 复核残留两件 | ✅ 已闭环 | E1（commit `972a3b593`）：**事实修正——07-09 凌晨并行线已实装主体，本轮验收+收尾**。`process_import_jobs` 消费者转真后台：chat/knowledge 上传入队 `KnowledgeIndexJob` 后立即返回，索引后台完成，单 job 失败不再阻塞后续批次（`upload_file` 签名加 `BackgroundTasks`，安全回归测试同步于 `878891be3`）。E2 K 侧写投影：已随 §7.3.1 Q-shrink 退役（`reference_index.py` activation_keys 写侧+读侧 272L 删除+rebuild `DROP TABLE IF EXISTS` 钉死），不另立行 |
