...
> 状态：设计权威；Group 2/3/4 substrate 已分别形成并部署，但 2026-07-17 live-entry/path 复核重新打开 `P1-004`、`SES-CONSUMER-001`、`A2A-TERMINAL-001`、`TEAM-FANOUT-001`、`ROOT-TREE-001` 五个回归 leaf。`EVID-G2-016` 已随 commit `11001f133` 三服务同源部署，Task 实时/折叠/reload 与 Peer A2A read-only browser canary 已通过；真实 MiniMax canary 同时证明无 commentary Provider 即使收到强提示也可能不产生 public progress。当前 §28.5 / `EVID-G2-017` 以 model-authored `report_progress` 补齐这一 seam，新 source 发布和 live/reload canary 前 `SES-CONSUMER-001` 仍为 `in_progress-deployed-canary-partial`。完整 Session V2 继续由 Group 6/7/8/9/10 验收。
>
...
| Frontend typed projection | `in_progress-deployed-canary-partial:EVID-G2-016/017` | 三类协作、Composer Task、query invalidation、final artifact 与 read-only A2A 已部署/canary；无 commentary Provider 的 `report_progress` exact-message consumer 已 local Green，待发布后 live/reload 验收 |
...
| 自动化黄金轨迹验收 | 既有 Group 4 保持；`EVID-G2-016` 已发布，`017` local Green | `016`：backend full=`7569 passed, 2 skipped`、frontend full=`720`、Playwright=`12`；`017`：backend focused=`125+30`、full=`7574 passed, 2 skipped`，frontend focused=`26`/full=`722`、build/budget 与 architecture ledger 绿；production canary 待写回 |
| Railway 行为修复与生产验收 | `EVID-G2-016` 已部署/canary partial；`017` 待发布 | `11001f133` deployments：backend=`16228d84-e601-4c1f-a1f1-7d5e0274d7a6`、backend-api=`af01ed4a-d085-4efc-8d52-bf531ea44405`、frontend=`cd99db27-d1fe-4791-833f-ce01a5c13d3a`，均 `SUCCESS`。Task/live/reload/read-only A2A 已绿；MiniMax 无 public text 反证进入 `EVID-G2-017` |
...
**七原子：** Input=provider public text、model-authored `report_progress.message`、canonical task/tool events、current-turn artifact declarations；Authority=Session visibility + exact RuntimeTask/ChatArtifact ownership，progress expression 不被 effect capability 接管；Execution=round seal/outbox→socket、core progress tool、single Session reducer、ledger query 与 terminal outcome；Evidence=SessionEventOutbox、stable event/item/tool-call ID、原始 progress arguments、RuntimeTask metadata、artifact manifest hash；Recovery=outbox replay、query invalidation+polling、authority drift reconciliation、reload/reconnect reducer；Consumption=Run disclosure Markdown、Composer Task dock、right-rail deliverables；Acceptance=Red→Green、full frontend/backend、production build、browser live/reload/canary。`EVID-G2-016` 为 deployed-canary-partial，`017` 发布和真实 progress live/reload canary 前 `SES-CONSUMER-001` 仍不 closed。
