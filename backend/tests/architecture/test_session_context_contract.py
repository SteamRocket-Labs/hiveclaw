...
def test_runtime_entrypoints_pass_session_context_to_invoker() -> None:
    ...
    assert 'listed_surface="task_updates"' in task_executor
...
def test_internal_execution_channels_are_excluded_from_normal_chat_listing() -> None:
    ...
    assert '_LEGACY_HIDDEN_CHAT_SOURCES = ("trigger", "task", "heartbeat")' in chat_sessions_api
    assert 'ChatSession.listed_surface == "chat"' in chat_sessions_api
    assert 'ChatSession.listed_surface == "chat"' in session_recall
