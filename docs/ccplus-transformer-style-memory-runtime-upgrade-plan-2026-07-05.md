...
| F-02 | T2 gatherer 输出 evidence candidates | `reference_index.py`、`retriever.py` | T2 candidate tests | 已完成：本提交；Red targeted test -> missing `MemoryRetriever.gather_t2_evidence_candidates`；Green targeted -> 1 passed, retrieval pipeline file -> 12 passed；Lint `ruff check app/memory/retriever.py tests/memory/test_retrieval_pipeline.py` -> All checks passed |
