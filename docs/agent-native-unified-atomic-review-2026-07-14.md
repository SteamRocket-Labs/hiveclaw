...
>
> terminal-fold commit `d60a669bd` 三服务发布后的 canary 已证明“成功完成后默认收起过程、final 独立、展开恢复”本身成立；但同一未刷新页面的运行中 commentary 仍会停住。`EVID-G2-020` 用生产 DB 与 WebSocket 帧把最后根因锁定为 canonical sequence 被 bridge 主动打断：durable `assistant_commentary` 在 final 前 62–95 秒已提交，而 live 帧从 sequence `3229` 跳到 `3240/3243`；`web_chat_stream_bus._listen_web_chat_stream_once()` 对 `private_provider/operator` 直接 `continue`，严格 highest-contiguous reducer 因缺失中间序号而合法缓冲全部后续 public commentary/final，直到 REST reload。当前修复在 Redis publication 前生成 standard user projection、删除 explicit redaction paths 指向的私有 bytes、保留 event identity/sequence，并兼容滚动部署期间的旧 raw publisher；所有 canonical sequence 都送入浏览器，private event 只推进 cursor、不生成用户正文。新 source 三服务发布与 authenticated no-refresh/reload canary 前状态为 `in_progress-local-green-live-sequence`，不增加 canonical leaf。
>
...
**当前回归状态（2026-07-18）**：13/14 保持 closed；`SES-CONSUMER-001` 已依次关闭三类协作 typed consumer、公开 commentary 持久化、Task 专用消费、terminal folding、reload replay、Redis→socket bridge、newest suffix 与 typed live-tail admission。`EVID-G2-020` 又用生产 DB/live-frame correlation 证明 bridge 丢弃非 public canonical event 会制造 sequence gap，使后续 public commentary/final 必须等 reload；当前 user-projected continuity 修复已本地 Green。三服务发布并通过 authenticated no-refresh、successful terminal 默认折叠、展开恢复、hard reload 等价和 private-byte absence canary 前不得标 closed。
...
- P1 | SES-CONSUMER-001 | in_progress-local-green-live-sequence:EVID-G2-016/017/018/019/020 | 三类协作、固定 Task、final artifact、Peer A2A read-only、canonical commentary persist/render、rolling ownership、exact Session store、live bridge、suffix/tail/live-tail 与 terminal fold 已绿；`020` 修复 user-projected sequence continuity，待三服务发布与 no-refresh/reload/ordering/private-byte canary
...
| 2 | `EVID-G2-*` | 14 leaf / 0 Missing | `in_progress`：13/14 closed；`EVID-G2-016/017` 已证明 Task、commentary persistence/replay 与 A2A read-only；`018/019` 已证明 bridge 与 typed live-tail；terminal-fold commit `d60a669bd` 的 canary 证明完成态折叠矩阵成立。`EVID-G2-020` 继续证伪 bridge 丢弃 private/operator sequence，并完成 user-projected continuity local Green | 发布 `020` source；真实模型必须在 running 时无刷新显示 progress/milestone，completed 默认折叠只留 final，展开恢复完整过程，raw task/progress tool 与 private bytes 均为零，hard reload 后顺序和 render ownership 等价；不得用 fixture、REST reload 或 deployment success 代替默认产品路径 proof |
...
context_read_receipt:
  ...
  source_baselines:
    hive_head: "d60a669bd830344d7b61a1c7e36a93e23fbf1f99 + owned live-sequence correction"
    freecode_head: "7dc15d6c8fb0c40c7fcc02ce9b58204324252632"
    codex_head: "5c19155cbd93bfa099016e7487259f61669823ff"
...
- verified root cause：`web_chat_stream_bus._listen_web_chat_stream_once()` 只把 audience=`direct_user|participants` 的 canonical envelope 交给 browser，对 `private_provider/operator` 直接 `continue`。Session V2 sequence 是同一 Session 的全局连续机械事实；前端 highest-contiguous reducer 收到缺口后必须缓存所有更高 sequence。private event 即使不产生可见行，也必须以 user projection 进入 reducer，否则 public commentary/final 只能等 REST replay 修复。
- implementation：`publish_canonical_session_event()` 先验证 durable source envelope hash，再调用 canonical `serialize_session_event(..., audience="user")`；explicit `redaction_paths` 指向的 provider-private bytes 在进入 Redis 前删除，wire envelope 记录 source/projected hash 与 `projection_audience=user`，但保留原 `event_id/session_id/sequence`。API forwarder 验证 projected hash并投递全部 sequence；滚动部署期间若收到旧 runtime publisher 的 raw envelope，则在 forwarder 内执行同一 user projection，避免新旧二进制窗口重新产生 gap 或泄露 private bytes。public commentary/final 不做语义重写。
- Red→Green：backend 新回归首次为 `2 failed / 2 passed`：一条证明私有正文仍进入 Redis，另一条证明 forwarder 只产生 `[1,3]` 而不是 `[1,2,3]`。实现后 stream bus + Session contract + terminal outcome + live wiring=`57 passed`；frontend 新 acceptance 证明 redacted private continuity event 推进 cursor 到 3、正文为空、后续 public delta 立即投影，相关 Session disclosure/reducer/hydration=`4 files / 54 tests`。frontend full=`122 files / 746 tests`；`tsc + vite build + bundle budgets` 通过，AgentDetail=`339876/380000` bytes、gzip=`93784/115000`，vendor=`591449/620000`、gzip=`186474/200000`；scoped Ruff/format 通过。
- broader regression ownership：共享工作树 backend full=`7631 passed, 2 skipped, 7 failed`。7 个失败全部位于另一 session 的未提交 IM-channel scope：1 个临时 router shape、5 个 untracked `im_channel_identity_closure_0718` head 与旧 0717 断言冲突、1 个新 Feishu RLS bypass 尚未登记；均不与本证据 owned paths 相交，未修改、暂存或伪装为本项通过。
- migration/backfill/rollback：无 schema migration、无数据回填或 rewrite；既有 canonical events 已完整保留，reload 一直可恢复。rollback 不损坏 durable evidence，但会重新打开 live sequence gap 与“只有刷新才更新”；部署时用 `projection_audience` 保持 old-publisher/new-forwarder 滚动兼容。
- status：`in_progress-local-green-live-sequence`。只有同一 exact source 三服务发布后，authenticated no-refresh canary 同时证明 running progress/milestone 实时出现、private bytes/raw task tools 为零、successful terminal 自动折叠只留 final、展开恢复完整过程、hard reload 顺序与字节等价，才关闭 `SES-CONSUMER-001`。
