...
> 状态：设计权威；Group 2/3/4 substrate 已分别形成并部署，但 2026-07-17 live-entry/path 复核重新打开 `P1-004`、`SES-CONSUMER-001`、`A2A-TERMINAL-001`、`TEAM-FANOUT-001`、`ROOT-TREE-001` 五个回归 leaf。`EVID-G2-016/017` 已证明 Task 专用消费、公开 commentary 持久化/reload、rolling render ownership 与 Peer A2A read-only。`EVID-G2-018` 随 commit `9ebb51f1b` 三服务发布后证明 runtime forwarder 与 live Task/final 接线；§28.5 / `EVID-G2-019` 的 suffix baseline、tail-watermark、first-snapshot `liveReady` 与 typed `cursor_mode=live_tail` 又分别随 commit `70c4a12f2`、`d29f52493`、`9ad195fc3`、`a1be2dfbe` 三服务发布。authenticated canary G 已在首个 transcript REST page 可继续后台恢复的同一未刷新页面实时得到固定 Task 与 final，raw `track_todo/report_progress` row 精确为零；手动展开 terminal Run 后，两条 exact public progress/milestone 同样存在，证明 persistence、live delivery 与 replay 已接线。剩余缺口被收敛为 presentation：成功 Run 默认折叠时错误地连公开 commentary 一起移出 DOM。当前 local Red→Green 已改为公开 commentary/prose 永久可见，只折叠 private reasoning、普通工具历史与 compaction；新 source 发布及 no-refresh/hard-reload 等价 canary 前 `SES-CONSUMER-001` 仍为 `in_progress-local-green-terminal-commentary`。完整 Session V2 继续由 Group 6/7/8/9/10 验收。
>
...
|---|---|---:|---|
| 用户输入、模型公开 `assistant_text/commentary`、final | 正文，保持原字节与顺序 | 否；completed Run 也不得把它们藏进 process disclosure | 不得改写成 `Thinking`；`report_progress.message` 显示为 commentary，不显示其 tool call/ACK |
...
| 当前 Hive 七原子审计 | 已完成并持续更新；5 leaf 仍 open | §28.4 五个协作回归已部署但行为验收 open；§28.5 已用 authenticated canary G 证明 live-tail、Task 专用消费和 final 实时送达，并把 `SES-CONSUMER-001` 收敛为 completed Run 错折叠公开 commentary。terminal commentary local Green 尚待三服务发布与 no-refresh/hard-reload 终验，总报告继续维持 38/103 |
...
| Frontend typed projection | `in_progress-local-green-terminal-commentary:EVID-G2-016/017/018/019` | 三类协作、Composer Task、query invalidation、final artifact、read-only A2A、commentary persist/reload、live bridge 与 typed `live_tail` 已部署/canary；公开 commentary 的 terminal persistence 已 local Green，待三服务发布与 no-refresh + hard reload 终验 |
...
| 自动化黄金轨迹验收 | 既有 Group 4 保持；`EVID-G2-016/017/018` 保持，`019` local Green | `019`：live-tail source 已部署；terminal commentary Red 首次 `1 failed / 9 passed`，Green focused=`4 files / 155 tests`、frontend full=`122 files / 745 tests`、build/budget 绿；production terminal-visible + hard-reload canary 待写回 |
| Railway 行为修复与生产验收 | `EVID-G2-016/017/018/019` 已提供 durable/reload/live-bridge/live-tail proof；terminal commentary 待发布 | commit `a1be2dfbe` 三服务均 `SUCCESS`，health 的 runtime/RLS/forwarder gate 与 frontend 200 通过；authenticated canary G 无刷新得到 Task/final，展开 completed Run 后 exact progress/milestone 可见。新 presentation source 需再次三服务同源发布并验证折叠状态下 commentary 仍可见、hard reload 等价 |
