...
| CC 机制 | 代码证据 | 语义 |
| --- | --- | --- |
| System context cached per conversation | `/Users/example-owner/vc-saas/free-code-main/src/context.ts:116` `getSystemContext` | git status / cache breaker 等会话级系统上下文可缓存 |
| User context cached per conversation | `/Users/example-owner/vc-saas/free-code-main/src/context.ts:155` `getUserContext` | CLAUDE.md / memory files / current date 单独进入 user context |
| Skill `paths` frontmatter | `/Users/example-owner/vc-saas/free-code-main/src/skills/loadSkillsDir.ts:159` `parseSkillPaths` | Skill 可声明路径触发条件 |
| 条件 Skill 动态激活 | `/Users/example-owner/vc-saas/free-code-main/src/skills/loadSkillsDir.ts:997` `activateConditionalSkillsForPaths` | 读写相关路径后，把 conditional skill 移入 dynamic skills 并通知缓存清理 |
...
| 代码位置 | `/Users/example-owner/Context Engineering/codex/codex-rs/hooks/src/schema.rs`；`hooks/src/events/compact.rs`；`analytics/src/events.rs`；`execpolicy/src/decision.rs`；`codex-api/src/common.rs` |
...
37 个 RTD 原子项全部提交后，执行两轮大逻辑召回：

1. Runtime 全量回归：`cd backend && source .venv/bin/activate && pytest tests/runtime -q`。第一次有效运行暴露 4 个断点：`spawn_subagent` schema 与自定义 definition 兼容边界冲突；`task_eval` 仍期待旧 `web_pack` 术语，和 RTD-01/RTD-02 的 capability/deferred group 清债冲突。最终修复结论：`subagent_type` 必须保持开放 string，不能用 enum 封死自定义 active subagent；内置类型通过 description 暴露，推荐自定义定义使用 `definition_name`，但不破坏 AgentTool 兼容入口；`backend/app/runtime/task_eval.py` 将 research benchmark expectation 从旧 `web_pack` 改为当前 `Likely Deferred Tool Groups` 中的 `- web`。
2. Backend 全量回归：`cd backend && source .venv/bin/activate && pytest tests -q`。第一次全量暴露 21 个断点，集中在 5 类：`agent_tools.is_pack_enabled` 兼容出口缺失；`ToolRuntimeService` Plan Gate mock/旧 decision 缺 `reason` 字段兼容；`app/agents/subagent.py` 静态 import `execute_tool` 破坏 ToolRuntimeService 单入口架构测试；Dream 模板缺精确禁止 `memory/t3/**` 直写语句；local-agent delegation 在无 budget run 时也传 `budget_run_id`，破坏旧 mock/无预算路径。收尾提交：`799492db0 Restore agent tool pack compatibility shim`、`0ef82fec8 Make plan gate reason fallback compatible`、`22604de04 Preserve subagent custom definition compatibility`、`948222aa3 Close dream and local agent compatibility gaps`、`ed958c301 Align coordinator subagent schema test with custom definitions`。

最终验证：冲突契约用例 `pytest tests/runtime/test_coordinator_force_async_acceptance.py::test_coordinator_agenttool_surface_is_session_worker_variant tests/tools/test_agent_tool_cc_compat.py::test_spawn_subagent_schema_exposes_agenttool_compatible_fields tests/tools/test_agent_tool_cc_compat.py::test_spawn_subagent_schema_allows_custom_agent_names_in_subagent_type -q` -> `3 passed, 4 warnings`；失败集合回归 `pytest tests/architecture/test_tool_runtime_single_entry.py tests/services/test_agent_mcp_gating.py tests/services/test_mcp_tool_discovery.py tests/tools/test_service.py::test_live_intercept_returns_confirmation_required_without_plan_mode_activation tests/tools/test_service.py::test_unattended_intercept_returns_confirmation_required_without_plan_mode_activation tests/tools/test_service.py::test_non_eligible_source_intercept_returns_confirmation_required_fail_closed tests/services/test_subagent_run_service.py::test_spawn_schema_exposes_run_in_background_and_check_tool_registered tests/tools/test_agent_tool_cc_compat.py::test_spawn_subagent_schema_exposes_agenttool_compatible_fields tests/services/test_distillation_boundary_contracts.py::test_dream_template_does_not_instruct_direct_writes_to_platform_managed_evolution_files tests/services/test_local_agent_a2a.py::test_delegate_to_agent_routes_execution_target_local_agent -q` -> `27 passed, 4 warnings`；Runtime 全量 `pytest tests/runtime -q` -> `664 passed, 19 skipped, 4 warnings`；Backend 全量 `pytest tests -q` -> `5341 passed, 206 skipped, 4 warnings`；Lint：`ruff check app/services/agent_tools.py app/tools/service.py app/agents/subagent.py app/tools/handlers/subagent.py app/templates/DREAM.md app/services/agent_tool_domains/messaging.py tests/runtime/test_coordinator_force_async_acceptance.py tests/architecture/test_tool_runtime_single_entry.py tests/services/test_agent_mcp_gating.py tests/services/test_mcp_tool_discovery.py tests/tools/test_service.py tests/services/test_subagent_run_service.py tests/tools/test_agent_tool_cc_compat.py tests/services/test_distillation_boundary_contracts.py tests/services/test_local_agent_a2a.py` -> `All checks passed!`；项目内 `test_trigger_daemon.py` 的 unawaited coroutine warning 已通过关闭 `run_bounded` 内层 awaitable 清掉，剩余 4 个 warning 均来自第三方依赖 deprecation。
