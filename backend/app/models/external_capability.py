...
class ExternalMarketplaceSource(Base):
    ...
    sync_status: Mapped[str] = mapped_column(String(30), nullable=False, default="never_synced", index=True)
...
class ExternalMarketplaceEntry(Base):
    """One discoverable external capability entry from a marketplace source."""

    __tablename__ = "external_marketplace_entries"
    __table_args__ = (
        UniqueConstraint("tenant_id", "source_id", "external_key", name="uq_external_marketplace_entry_key"),
    )
