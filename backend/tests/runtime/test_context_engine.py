...
def test_context_engine_fences_context_and_records_reference_artifact():
    ...
    assert '<context_block kind="memory_snapshot" source="memory_provider:snapshot">' in fenced
    assert "- user prefers concise answers" in fenced
    artifacts = session_context.metadata["context_artifacts"]
    assert artifacts[0]["kind"] == "memory_snapshot"
    assert artifacts[0]["source"] == "memory_provider:snapshot"
...
def test_context_engine_ignores_blank_context_blocks():
    ...
    assert engine.inject(session_context, kind="memory_snapshot", source="memory", content="  ") == ""
