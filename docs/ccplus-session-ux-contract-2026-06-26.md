...
| 替我批准 | `auto` | 低风险动作自动执行；高风险或不明确的动作询问我。 |
| 完全访问 | `bypassPermissions` | 尽量自主执行；删除、外发、sandbox 限制和企业硬规则仍然生效。 |

新 Agent 和没有显式配置的 Agent 初始使用“请求批准”。Agent 管理者可以在 Agent Detail → Settings 中把未来新会话的默认值改成三者任意一个；修改默认值不反向改变既有会话。当前会话操作者仍可从 composer 单独覆盖本会话。

“完全访问”是 session-local 审批模式，不是企业权限、管理员授权或 break-glass。选择它可以有一次风险确认，但不要求管理员、原因或 TTL，也不能扩大用户/Agent 已有的 tenant、resource、capability、network、secret 或 sandbox authority。
...
- `default` 是初始缺省模式，但不是硬编码模式；已设置过的 Agent 默认值和 session metadata 必须按各自作用域执行。
...
- 企业治理没有面向普通用户的开启/关闭选项；只有规则命中时，session 才显示简洁的阻断或企业审批状态与恢复动作。
