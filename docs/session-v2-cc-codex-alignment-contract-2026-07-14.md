...
> 状态：设计权威；Group 2/3/4 substrate 已分别形成并部署，但 2026-07-17 live-entry/path 复核重新打开 `P1-004`、`SES-CONSUMER-001`、`A2A-TERMINAL-001`、`TEAM-FANOUT-001`、`ROOT-TREE-001` 五个回归 leaf。`EVID-G2-016`–`019` 已证明 Task 专用消费、公开 commentary 持久化/reload、rolling render ownership、Peer A2A read-only、runtime forwarder、suffix/tail 与 typed live-tail；`EVID-G2-020` 的 private/V1 sequence continuity 已随 `37599d2ca`、`5e6d47b06` 发布，authenticated canary M 证明 active progress、Task、milestone 无刷新到达。该 canary 同时证伪 live terminal projection：final 已显示且过程已隐藏，但当前 Run 仍停在 `处理中`，hard reload 后才生成 collapsed `已处理`。`EVID-G2-021` 已本地修正高序号 suffix terminal replay、RuntimePhase 同步与后续 metadata overwrite。最终产品合同保持：运行中公开 commentary/prose 实时展开；成功完成后与 Thinking、工具、compaction 一起进入同一个可恢复的 `Processed` disclosure，默认只留 final；Task ledger 独立固定；整体 failed/blocked 继续自动展开。新 source 发布及 no-refresh terminal fold/expand/collapse/reload canary 前 `SES-CONSUMER-001` 仍为 `in_progress-local-green-live-terminal-seal`。完整 Session V2 继续由 Group 6/7/8/9/10 验收。
>
...
| 当前 Hive 七原子审计 | 已完成并持续更新；5 leaf 仍 open | §28.4 五个协作回归已部署但行为验收 open；§28.5 与总报告 `EVID-G2-016`–`021` 已证明 live-tail、Task 专用消费、active commentary 与 final 实时送达、terminal fold/reload substrate。canary M 又证伪 live terminal 高序号 suffix replay；`021` local Green 尚待三服务发布与 no-refresh/hard-reload 终验，总报告继续维持 38/103 |
...
| Frontend typed projection | `in_progress-local-green-live-terminal-seal:EVID-G2-016/017/018/019/020/021` | 三类协作、Composer Task、query invalidation、final artifact、read-only A2A、commentary persist/reload、live bridge、typed `live_tail`、private/V1 continuity 与 active no-refresh 已部署/canary；`021` 修正 terminal suffix replay/phase/metadata overwrite，待三服务发布与 fold/expand/collapse/reload 终验 |
...
| 自动化黄金轨迹验收 | 既有 Group 4 保持；`EVID-G2-016/017/018/019/020/021` 保持 | `020` active canary 已通过；`021` high-sequence terminal Red=`1 failed / 18 passed`，Green targeted=`19 passed`、related=`8 files / 211 tests`、frontend full=`122 files / 747 tests`，build/typecheck/bundle budgets 通过 |
| Railway 行为修复与生产验收 | `EVID-G2-016`–`020` 已提供 durable/reload/live-bridge/live-tail/private/V1 continuity/active no-refresh proof；`021` 待发布 | `5e6d47b06` 三服务均 `SUCCESS`，canary M 证明 running progress/Task/milestone 无刷新可见，并精确证伪 terminal live projection；`021` 必须三服务同源发布，验证 completed 自动只留 final、展开恢复、再次折回、hard reload 等价与 raw/private bytes 为零 |
