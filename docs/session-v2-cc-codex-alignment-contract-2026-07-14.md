...
> 状态：设计权威；Group 2/3/4 substrate 已分别形成并部署，但 2026-07-17 live-entry/path 复核重新打开 `P1-004`、`SES-CONSUMER-001`、`A2A-TERMINAL-001`、`TEAM-FANOUT-001`、`ROOT-TREE-001` 五个回归 leaf。`EVID-G2-016` 已随 commit `11001f133` 三服务同源部署，Task 实时/折叠/reload 与 Peer A2A read-only browser canary 已通过；真实 MiniMax canary 同时证明无 commentary Provider 即使收到强提示也可能不产生 public progress。第一版 `report_progress` commit `69b4127b7` 已发布并证明模型调用可达，但 hard reload 又坐实 canonical tool event 没有 public message；当前 §28.5 / `EVID-G2-017` follow-up 已改为 pre-effect 原生 `assistant_commentary` + V1 exact-envelope 恢复。新 source 发布和 live/reload canary 前 `SES-CONSUMER-001` 仍为 `in_progress-deployed-canary-partial`。完整 Session V2 继续由 Group 6/7/8/9/10 验收。
>
...
| Frontend typed projection | `in_progress-deployed-canary-partial:EVID-G2-016/017` | 三类协作、Composer Task、query invalidation、final artifact 与 read-only A2A 已部署/canary；第一版 progress hard-reload 失败已形成 path proof，原生 commentary + V1 rolling replay follow-up 已 local Green，待发布后 live/reload 验收 |
...
| 自动化黄金轨迹验收 | 既有 Group 4 保持；`EVID-G2-016` 已发布，`017` follow-up local Green | `016`：backend full=`7569 passed, 2 skipped`、frontend full=`720`、Playwright=`12`；`017` first=`7574 passed, 2 skipped` / frontend=`722`，follow-up backend full=`7577 passed, 2 skipped`、frontend full=`724`、build/budget 绿；新 production canary 待写回 |
| Railway 行为修复与生产验收 | `EVID-G2-016` 已部署/canary partial；`017` first 已部署且 canary 失败，follow-up 待发布 | first `69b4127b7` deployments：backend=`a7c9f65a-e980-4957-9b5e-781095847bac`、backend-api=`6063f003-9137-4429-820d-e6184cbee18f`、frontend=`3034548c-f1f2-4eff-aa5e-d724db54f5f0`，均 `SUCCESS`；模型调用可达但 hard reload 丢 message。follow-up exact-source/live/reload 尚未验收 |
