...
| E-03 | 旧 T3 机械回填 activation keys | `reference_index.py`、`plane_read.py` | legacy T3 fixture tests | 已完成：本提交；Red targeted test -> missing `t3_profile` / `t3_knowledge` activation rows；Green targeted -> 1 passed, derived table + plane metadata regression -> 13 passed；Lint `ruff check app/memory/reference_index.py app/memory/plane_read.py tests/memory/test_c8_derived_tables.py tests/memory/test_retrieval_pipeline.py` -> All checks passed |
