...
backend/app/memory/md_store.py
  写 T2 / T3 markdown 段；rebuild_index 维护唯一 generated Memory Wiki map：`memory/indexes/wiki_map.md`。旧 `memory/INDEX.md` / `memory/index.md` / `memory/.derived/t3_index.md` 均已退役，不能作为兼容镜像继续生成。
...
Status update 2026-06-19: this phase is superseded by the Agent Markdown Wiki
single-path design. Relationship-shaped knowledge is no longer a first-class
runtime prompt node or durable write path. `understandings.md` may remain as a
read-only legacy projection / offline eval artifact, but all semantic promotion
must go through T0 append-only ledger -> T2 Segment Package -> accepted T3 files.

Historical goal: relationship-shaped knowledge becomes a durable node with
evidence refs, confidence, and contradiction history, instead of being scattered
across T2/T3 free-form bullets.
...
- Current boundary: `record` / `contradict` are disabled; `get` / `query` may
  inspect old files for migration and audit only.
- Runtime gate: dream / retriever must not consume this file as accepted
  semantic memory. If relationship knowledge matters, a T3 Writer must synthesize
  it into `episodes.md`, `user.md`, `worker.md`, or `capabilities.md` with
  source refs and Memory Gate review.
...
- Do not wire `UnderstandingStore` into prompt retrieval. The 2026-06-19
  single-path closure retired relationship projections as a prompt source;
  relationship views are rebuildable read models from accepted T3 entries and
  source refs.
- Persist lifecycle state to `memory/control/lifecycle.json` and record every
  new T2/T3 active write against the same `entry_id` carried in the
  markdown metadata.
- Add an explicit approved-proposal apply path for
  `CharterProposalStore`: pending / rejected proposals remain inert,
  while approved owner-charter calibration appends to the relevant
  frozen owner agency charter section in `soul.md` and writes
  `memory/charter_calibration.md` audit evidence.
- Runtime gate: relationship memory, lifecycle state, and approved
  charter calibration are no longer out-of-band notes. They now have
  concrete read / write / apply paths and tests.
- Utility smoke: legacy `understandings.md` can still be read for audit, but
  no `source_type=understanding_store` item should appear in runtime prompt
  retrieval. A T2/T3 memory write creates a matching lifecycle entry keyed by
  the same `entry_id`; an approved `consider_full_authority` proposal adds the
  action to `soul.md` once and records an audit line.
...
| 6 First-Person Soul and HR Creation | Completed 2026-05-22 | Code paths: `backend/app/services/agent_manager.py`, `backend/app/tools/handlers/hr.py`, `backend/tests/services/test_agent_file_generation.py`, `backend/tests/tools/test_hr_handler.py`. Runtime gate wired: HR schema and preview accept `company_charter` and `owner_agency_charter`; `create_digital_employee` passes company/owner identity and charter blueprint into `initialize_agent_files()`; soul rendering always emits `First-Person Accountability`, `Frozen Company Charter`, and `Frozen Owner Agency Charter` sections. Tests: red run failed because soul had no charter sections and HR schema/preview did not expose charter fields; green run `pytest tests/services/test_agent_file_generation.py tests/tools/test_hr_handler.py tests/tools/handlers/test_hr_soul_prompt.py` -> `31 passed in 2.69s`; lint `ruff check ... && ruff format --check ...` -> passed. Utility smoke: a new research assistant soul says it directly supports Example Owner inside Acme Capital, lists Full Authority / Confirm First / Never Do, and marks charter sections frozen so dream/heartbeat can propose but not mutate. Residual risk: owner/admin approval UI for charter edits is not implemented yet; runtime mutation still remains blocked by convention and future checkpoint flows. |
...
| 19 Memory Governance Layer Closure | Completed 2026-05-22; superseded 2026-06-19 for relationship prompt retrieval | Code paths: `backend/app/memory/retriever.py`, `backend/app/memory/lifecycle_store.py`, `backend/app/memory/t2_store.py`, `backend/app/memory/md_store.py`, `backend/app/services/charter_proposals.py`, `backend/tests/memory/test_retrieval_pipeline.py`, `backend/tests/memory/test_lifecycle_state_machine.py`, `backend/tests/memory/test_t2_store.py`, `backend/tests/tools/test_memory_handler.py`, `backend/tests/services/test_charter_proposals.py`. Current runtime gate: `UnderstandingStore` is read-only legacy and must not enter prompt retrieval; relationship views are rebuildable from accepted T3 and source refs. Durable lifecycle state and approved charter calibration remain valid: every new T2/T3 active write records a `memory/control/lifecycle.json` entry keyed by the markdown `entry_id`; approved charter proposals can be explicitly applied to `soul.md` frozen owner agency charter and emit `memory/charter_calibration.md` audit rows. |
...
| 11 Understanding Store + Relationship Memory | Completed 2026-05-22; retired to read-only legacy 2026-06-19 | Code paths: `backend/app/memory/understanding_store.py`, `backend/tests/memory/test_understanding_store.py`. Current runtime gate: `UnderstandingStore` may read old `understandings.md` for audit/migration, but `record()` and `contradict()` are disabled and retriever must not inject relationship-shaped projections into prompt memory. Any relationship knowledge that matters must be synthesized by the T3 Writer into accepted T3 files with source refs and Memory Gate review. |
