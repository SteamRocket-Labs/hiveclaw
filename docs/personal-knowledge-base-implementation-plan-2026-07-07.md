...
ingest request
  ...
  -> source acquisition
  -> write canonical Markdown or raw payload spool
  -> document upsert by scope + source_sha256, status=queued
  -> create knowledge_index_jobs row, stage/status=queued
  -> return document_id + job_id to caller

background worker
  -> claim queued job under tenant scoped session
  -> load queued Markdown / raw bytes / URL metadata
...
| --- | --- | --- | --- |
| M1-G1 多跳 PPR 检索 | 已闭环 | `search_personal()` 的 graph channel 必须从命中实体出发构建 person-scope 子图，复用 `personalized_pagerank()` 做多跳扩散；结果仍经过 document/segment ACL 过滤并返回 `score_trace.channels.graph`。 | 单测覆盖二跳命中；Personal KB service 测试通过。 |
| M1-G2 异步 ingestion job | 已闭环 | upload / URL / paste 入口先创建 `knowledge_index_jobs(status=queued)` 并返回 job；后台 worker 负责转换、切片、抽取、索引、失败重试和状态推进。同步 helper 只能作为 worker 内部执行器或测试便利路径。 | API 测试证明入口不阻塞抽取；service 测试覆盖 queued/running/ready/failed/degraded/retry；现有导入测试通过。 |
| M1-G3 optional vector boundary | 已闭环 | Personal M1 不强依赖 pgvector，但必须有可插拔 vector provider 边界：未配置时可观测降级；配置后从 canonical segment 输入生成/查询 vector lane，且不成为 source of truth。 | 单测覆盖无 provider 降级和 provider 命中融合；迁移仍不强制 `CREATE EXTENSION vector`。 |
| M1-G4 extractor 独立测试 | 已闭环 | `PersonalKnowledgeLLMExtractor` 的 JSON 提取、schema 解析、敏感级别跳过、空响应/坏 JSON 失败路径必须有独立测试文件。 | 新增 `test_personal_knowledge_extractor.py` 并通过。 |
| M1-G5 SAG benchmark scorecard | 已闭环 | 新增 Hive-native vs SAG-trace 的轻量 benchmark harness；SAG 只作为陪练输入/对照，不进入生产 truth 或检索 provider。输出 recall@k、citation accuracy、ACL leakage、latency、cost 的 scorecard。 | 新增 scorecard 单测和可执行脚本/模块；文档记录命令与通过结果。 |
...
| M1-G2 异步 ingestion job | 已完成 | `complete personal kb async ingestion jobs` | 红测：`queue_source_bytes_import` 缺失、`process_import_jobs` 不消费 queued payload、API 缺 background worker 均先失败。转绿证据：`pytest tests/services/test_personal_knowledge_service.py -q` -> 27 passed；`pytest tests/api/test_agent_personal_knowledge_api.py tests/api/test_chat_upload_conversion.py -q` -> 14 passed；`ruff check app/services/personal_knowledge_service.py app/api/agent_knowledge.py app/api/upload.py tests/services/test_personal_knowledge_service.py tests/api/test_agent_personal_knowledge_api.py tests/api/test_chat_upload_conversion.py` -> All checks passed。实现：paste/upload/URL/chat attachment 均先返回 `status=queued`，后台 `tenant_scoped_session` worker 推进 queued job，旧同步 helper 仅作为 worker 执行器保留。 |
| M1-G3 optional vector boundary | 已完成 | `complete personal kb optional vector lane` | 红测：无 provider 时 job metadata 缺 `optional_vector`、`vector_provider` 不能注入、search 无 vector lane。转绿证据：`pytest tests/services/test_personal_knowledge_service.py -q` -> 30 passed；`ruff check app/services/personal_knowledge_service.py tests/services/test_personal_knowledge_service.py` -> All checks passed；`rg -n "CREATE EXTENSION.*vector|pgvector|Vector\\(" backend/alembic backend/app/models backend/pyproject.toml backend/requirements.txt backend/requirements-dev.txt` -> 仅命中 `knowledge.py` 注释。实现：`vector_provider` 可选注入，ingest 从 `title + heading_path + segment content` 生成 `index_text` 给 provider，search provider 只返回 segment 候选，最终仍经 SQL ACL fetch 后进入 `score_trace.channels.optional_vector`。 |
| M1-G4 extractor 独立测试 | 已完成 | `complete personal kb extractor tests` | `pytest tests/services/test_personal_knowledge_extractor.py -q` -> 4 passed；`ruff check app/services/personal_knowledge_extractor.py tests/services/test_personal_knowledge_extractor.py` -> All checks passed。覆盖 schema 清洗/无效项丢弃、fenced JSON 解析、4096 输出预算、usage context、client close、敏感级别跳过、空响应和坏 JSON 失败路径。 |
| M1-G5 SAG benchmark scorecard | 已完成 | `complete personal kb sag scorecard` | 红测：`pytest tests/evals/test_personal_kb_scorecard.py -q` -> 3 failed，缺 `app.evals.personal_kb_scorecard` 与 `app.scripts.run_personal_kb_scorecard`。转绿证据：`pytest tests/evals/test_personal_kb_scorecard.py -q` -> 3 passed；`ruff check app/evals/personal_kb_scorecard.py app/scripts/run_personal_kb_scorecard.py tests/evals/test_personal_kb_scorecard.py` -> All checks passed；`python -m app.scripts.run_personal_kb_scorecard --input <fixture> --output <report> --fail-on-acl-leakage` -> exit 0，report 输出 `benchmark=personal_kb_sag_scorecard`、`providers.hive.provider_kind=hive_native`、`providers.sag_trace.provider_kind=comparison_trace`、`hard_gates.acl_leakage_zero=true`。实现：SAG 只作为离线 comparison trace 评分输入，不进入 Personal KB provider 或 source-of-truth。 |
...
| D1a Personal KB daemon 兜底 | 已完成 | `complete personal kb import job drain` | 红测：缺 `build_personal_knowledge_job_claim_statement`、`process_import_jobs(max_attempts=...)`、`_heartbeat_loop()` 未调用 `_drain_personal_kb_jobs`，3 tests 先失败。转绿证据：`pytest tests/services/test_personal_knowledge_service.py::test_personal_import_job_claim_statement_uses_skip_locked_and_time_guards tests/services/test_personal_knowledge_service.py::test_process_import_jobs_marks_attempt_limit_as_failed_without_rebuild tests/services/test_personal_knowledge_service.py::test_process_import_jobs_consumes_queued_and_failed_personal_jobs tests/services/test_personal_knowledge_service.py::test_process_import_jobs_consumes_queued_source_bytes_payload tests/services/test_evolution_daemon.py::test_heartbeat_loop_drains_personal_kb_jobs_per_iteration -q` -> 5 passed；`pytest tests/services/test_personal_knowledge_service.py tests/services/test_evolution_daemon.py -q` -> 46 passed；`ruff check app/services/personal_knowledge_service.py app/services/evolution_daemon.py tests/services/test_personal_knowledge_service.py tests/services/test_evolution_daemon.py` -> All checks passed。实现：`SELECT ... FOR UPDATE SKIP LOCKED` claim，BackgroundTasks 与 daemon 共用 claim/process 路径，queued grace、running timeout、attempt 上限 poison 均在 service 层处理。 |
| D1b 并发闸与 token 记账 | 已完成 | `complete personal kb extraction metering` + `complete personal kb usage unavailable metering` | 红测：`KnowledgeExtractionResult` 不返回 usage/usage_tokens；ingest job metadata 无 `extraction_usage`；service 无租户级 `_extract_segment_with_tenant_guard` 与 semaphore；二轮复核又补出 provider 无 usage 时缺 `usage_unavailable_count`，均先失败。转绿证据：`pytest tests/services/test_personal_knowledge_extractor.py::test_llm_extractor_returns_usage_tokens_for_job_accounting -q` -> 1 passed；`pytest tests/services/test_personal_knowledge_service.py::test_ingest_markdown_records_extraction_usage_in_job_metadata tests/services/test_personal_knowledge_service.py::test_extract_segment_guard_limits_same_tenant_concurrency -q` -> 2 passed；`pytest tests/services/test_personal_knowledge_service.py::test_ingest_markdown_records_usage_unavailable_when_provider_omits_usage -q` -> 1 passed；`pytest tests/services/test_personal_knowledge_service.py -q` -> 37 passed；`pytest tests/services/test_personal_knowledge_extractor.py tests/services/test_personal_knowledge_service.py -q` -> 41 passed；`ruff check app/services/personal_knowledge_extractor.py app/services/personal_knowledge_service.py tests/services/test_personal_knowledge_extractor.py tests/services/test_personal_knowledge_service.py` -> All checks passed。实现：LLM extractor 保留原始 provider usage 并按全局 `extract_usage_tokens` 口径产出有效 token；Personal KB service 使用每 tenant semaphore（默认 4）包住 segment extraction；document/job metadata 写入 `extraction_usage.segment_count/segments_with_usage/usage_unavailable_count/tokens/provider_usage`，只记录聚合数字，不用估算值冒充真实 token。 |
| A1 activation cleanup 安全洞复核 | 已完成 | `harden external capability activation cleanup` | 复核现有 dirty diff 后接管提交。红测覆盖真实文件系统：正常 skill/subagent 删除、`../../` 相对逃逸、绝对路径、`.`/`..`、symlink 指向 workspace 外部、缺失组件、MCP manual revoke。转绿证据：`pytest tests/services/test_activation_cleanup.py tests/services/test_external_capability_activation.py::test_deactivate_external_extension_removes_files_and_marks_mcp_manual_revoke tests/services/test_external_capability_trust_gate.py::test_revoke_external_capability_snapshot_revokes_active_agent_activations -q` -> 9 passed, 4 warnings；`ruff check app/services/external_capabilities/activation_cleanup.py tests/services/test_activation_cleanup.py` -> All checks passed。实现：cleanup 只接受单段 safe component name；target/base 均 resolve 后要求 target.parent 等于 base，阻断相对路径、绝对路径和 symlink 逃逸。 |
| L1 legacy 绕过路径复核 | 已完成 | `document external capability legacy bypass review` | 复核范围：`rg "deactivate_activation_components|ExternalExtensionActivation|legacy_pack|source kind .*not installable|runtime_writes"`。结论：live external activation/revoke 只有 `activation.py` 与 `trust_gate.py` 两个 cleanup 调用点，均走 A1 已加固 helper；legacy `/enterprise/plugins/install` 是 builtin/local pack projection，不创建 `ExternalExtensionActivation`，`legacy_pack_adapter` 标记 `migration_only=true` 且 `runtime_writes=[]`；remote source 在 `_assert_installable` fail-closed。证据：`pytest tests/services/test_legacy_pack_adapter.py tests/services/test_plugin_install_service.py::test_source_policy_fail_closed_on_remote tests/services/test_plugin_install_service.py::test_assert_installable_accepts_builtin tests/services/test_external_capability_activation.py::test_deactivate_external_extension_removes_files_and_marks_mcp_manual_revoke tests/services/test_external_capability_trust_gate.py::test_revoke_external_capability_snapshot_revokes_active_agent_activations -q` -> 7 passed, 4 warnings；`ruff check app/services/external_capabilities/legacy_pack_adapter.py app/services/plugin_install_service.py app/services/external_capabilities/activation.py app/services/external_capabilities/trust_gate.py tests/services/test_legacy_pack_adapter.py tests/services/test_plugin_install_service.py tests/services/test_external_capability_activation.py tests/services/test_external_capability_trust_gate.py` -> All checks passed。 |
