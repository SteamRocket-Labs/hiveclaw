...
def _soul_auditable_entries(soul_text: str) -> list[str]:
    lines = []
    in_learned = False
    for raw in soul_text.splitlines():
        line = raw.strip()
        lowered = line.lower()
        if lowered.startswith("<soul_") and 'frozen="true"' not in lowered:
            lines.append(line)
            continue
        if line.startswith("## "):
...
def run_self_evolution_audit(
    *,
    ...
    t2_packages, _mtimes = load_t2_package_snapshots(Path(data_root), agent_id)
    t2_without_evidence = [
        package for package in t2_packages
        if not t2_package_has_source_refs(package)
    ]
    ...
    if soul_path.exists():
        soul_lines = _soul_auditable_entries(soul_path.read_text(encoding="utf-8", errors="replace"))
