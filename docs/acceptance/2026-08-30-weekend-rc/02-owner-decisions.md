...
### 2026-09-09 最新裁决：PDEC-016

Owner 原话：“最终标准不是已经改成 8 步了吗？已经不是所谓的 0-96 了。”以及“剩下的部分你需要什么权限我都可以给你。麻烦把你刚说的这几类全部搞定，commit、git push，跟新文档，全部落地”。

- 八步功能闭环是本任务最终标准，替代旧 96 条分母与全量双遍/D/E 完成门；旧记录保留为历史，不伪造通过、不删除失败。
- 单一 Codex 继续全部剩余修复、必要真实验收、文档更新、commit/push 和已授权三服务部署，不恢复 zCode/CC、Goal 或 heartbeat。
- 授权覆盖已说明的合成账号资产移交/停用，以及 Local 已验证修复的上游 push/npm 正式发行。只经真实认证支持路径，核对精确资源和当前运行，保留恢复材料；不影响真实用户、其他租户、凭据或无关全局政策。
- Growth 两次错误后改用原始证据和检索/上下文对照定位并修复，不靠第三次改写问题或静默换模型取得 PASS。Dynamic、Hook、自动化恢复和清理仍需对应实际证据。
- 以下旧权限表、PDEC-015 及更早执行条款与本裁决冲突时，以本裁决为准；不把已授权的同范围动作重新列为等待授权。
