...
backend/app/memory/md_store.py
  写 T2 / T3 markdown 段；rebuild_index 维护唯一 generated Memory Wiki map：`memory/wiki_map.md`。旧 `memory/INDEX.md` / `memory/index.md` / `memory/.derived/t3_index.md` 均已退役，不能作为兼容镜像继续生成。
...
- Do not wire `UnderstandingStore` into prompt retrieval. The 2026-06-19
  ...
  source refs.
- Persist lifecycle state to `memory/lifecycle.json` and record every
