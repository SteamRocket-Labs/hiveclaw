...
当前可读源码事实表明：FreeCode 主要围绕物理 context window 做 AutoCompact，保留 output/recovery 空间，provider 413 后可 reactive compact，并用 circuit breaker 与 stop hook 避免 compact 自循环。Skill 预算不足时退化描述，但保留全部名称可发现。这是 Hive 应保持的“模型先看见、超窗再恢复”语义下限。源码落点：`${LOCAL_HOME}/vc-saas/free-code-main/src/services/compact/autoCompact.ts:30-91,241-351`、`src/query.ts:592-647,1065-1182,1258-1265,1292-1297`、`src/tools/SkillTool/prompt.ts:20-40,70-171`。
