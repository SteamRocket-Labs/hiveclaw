...
日期：2026-06-24
状态：CCPlus V2 / Hive Connect 总纲，建立在 CCPlus V1 00-08 已闭环基底之上
...
| Company Knowledge / Ontology | 建设公司级 Knowledge / Ontology Plane，接入 Graphiti / SAG 作为 provider | 公司知识是组织资产，不是 Agent Memory，也不是外部检索索引 |
...
- SAG 的定位是 Markdown 文档语料上的 chunk、vector、event、entity、多跳检索 provider；它消费 Hive canonical Markdown artifact，不负责原始文件解析、ACL、审批或 prompt 注入。
- Graphiti 的定位是 temporal context graph provider；它输出候选 facts/entities/relationships，不负责公司知识最终提交。
- ACL / sensitivity filter 必须发生在任何 Knowledge prompt section 构建之前。
- 每条注入到 prompt 的公司知识都必须 source-bound，可追溯到 document/segment/assertion/proposal/review/audit。
...
| 企业知识库 / Ontology / SAG | `company-knowledge-ontology-plane-plan-2026-06-20.md` | Hive Knowledge Core、Graphiti/SAG provider、ACL/citation、company knowledge candidate lane |
...
| Company Knowledge / Ontology | `company-knowledge-ontology-plane-plan-2026-06-20.md` 已定义 Hive Knowledge Core、DocumentConversionService canonical Markdown、Graphiti/SAG provider 边界 | 还缺 Knowledge Core schema、provider adapters、KnowledgeSearchService fusion、ACL/citation/prompt injection 测试 |
...
### Step 5 - CompanyKnowledgeOntologyPlaneV1

把公司知识库和 Ontology 从“文档设计”提升成 V2 的公司级知识权威层。

交付：

- Knowledge Core schema / API
- canonical Markdown ingestion binding
- Graphiti provider adapter
- SAG provider adapter
- KnowledgeSearchService fusion
- ACL / sensitivity prompt filter
- citation validation
- proposal / review / publish / retire / rollback lane
...
| Company Knowledge / Ontology | Memory 不能直接成为公司事实；SAG/Graphiti 不能绕过 Hive Knowledge Core；每条 prompt 注入知识都经过 ACL 过滤并带 source refs |
