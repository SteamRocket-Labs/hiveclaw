...
> 状态：设计权威；Group 2/3/4 substrate 已分别形成并部署，但 2026-07-17 live-entry/path 复核重新打开 `P1-004`、`SES-CONSUMER-001`、`A2A-TERMINAL-001`、`TEAM-FANOUT-001`、`ROOT-TREE-001` 五个回归 leaf。`EVID-G2-016/017` 已证明 Task 专用消费、公开 commentary 持久化/reload、rolling render ownership 与 Peer A2A read-only。`EVID-G2-018` 随 commit `9ebb51f1b` 三服务发布后，production health 已证明 runtime forwarder `running=true/forwarded=111/last_sequence=2340/last_error=null`，同页 canary 也无刷新得到 Task/final 且 raw task/progress row 为零。§28.5 / `EVID-G2-019` 的 suffix baseline 与 tail-watermark 修复已分别随 commit `70c4a12f2`、`d29f52493` 三服务发布；authenticated canary D 证明 Task 专用面和 raw-row suppression 成立，但首个 transport backfill 仍在 store 产生前复用完整历史 promise，公开 progress 继续等待 reload。当前修复用独立 `liveReady` 在首个安全 newest snapshot 后立即放行 subscription、完整旧历史继续后台恢复；新 source 发布和 no-refresh/hard-reload 等价 canary 前 `SES-CONSUMER-001` 仍为 `in_progress-local-green-first-snapshot-live-ready`。完整 Session V2 继续由 Group 6/7/8/9/10 验收。
>
...
| Frontend typed projection | `in_progress-local-green-first-snapshot-live-ready:EVID-G2-016/017/018/019` | 三类协作、Composer Task、query invalidation、final artifact、read-only A2A、commentary persist/reload 与 live bridge 已部署/canary；suffix/tail-watermark 已发布，首个安全 snapshot readiness 与完整历史 completion 解耦已 Green，待 no-refresh + hard reload 终验 |
