...
@pytest.mark.asyncio
async def test_kernel_threads_typed_402_failure_code_across_invocation_result() -> None:
    """DAY1-PROVIDER-402-TERMINAL-CONSUMPTION-001: the status-first typed
    failure facts (quota_exhausted / rejected) must ride the InvocationResult
    losslessly so the web terminal path can persist and broadcast a machine
    failure code.  Downstream layers must never re-derive quota from
    natural-language text, the provider must still be called exactly once,
    and no fallback/replay may happen for a typed 402 rejection."""
    ...
    assert len(primary_client.calls) == 1
    assert fallback_client.calls == []
    assert failures[0]["error_class"] == "quota_exhausted"
