...
def test_skill_catalog_includes_session_extension_overlay_only_for_matching_session(tmp_path, monkeypatch):
    ...
    overlay_skill = (
        tmp_path / "data" / str(agent_id) / "session_extensions" / str(session_id) / "skills" / "trial" / "SKILL.md"
    )
    ...
    other_session_catalog = agent_context_mod.build_skill_catalog_section_for_agent(
        agent_id, session_id=other_session_id
    )
