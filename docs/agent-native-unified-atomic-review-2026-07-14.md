...
- first Green/deploy：`report_progress(message)` 是 core/default、Plan Mode 可用、ordered、governance-safe 的模型表达工具；handler 只回 `{ok, acknowledged}`，`agent.session.progress` 只作为 registry/audit taxonomy，admin policy definitions 不展示假开关，capability gate 不把当前 Session 表达误当外部 effect。commit `69b4127b7` 已部署为 backend=`a7c9f65a-e980-4957-9b5e-781095847bac`、backend-api=`6063f003-9137-4429-820d-e6184cbee18f`、frontend=`3034548c-f1f2-4eff-aa5e-d724db54f5f0`，三者 `SUCCESS`；health/readiness 与 frontend HTTP 绿。
...
- deployed commentary path proof：commit `a456932ec` 已部署为 backend=`dd3c6417-7bba-401d-bd4f-de2caa84dc85`、backend-api=`27fc25a8-7cf7-4665-9d39-2c1717d3c26c`、frontend=`7473d4d4-f112-4f4e-93ff-bc743b9b69b1`，三者 `SUCCESS`。production sequence `1839` 是 exact `assistant_commentary.completed`，带 stable item/run/parent/invocation identity 与模型原字节；authenticated browser 在 completed Run 展开后 exact message 唯一可见，证明 persistence/serializer/replay/render 已接线，不是 fixture。
...
- production path proof：`9ebb51f1b` 发布后 health 为 `process_role=runtime`、forwarder `running=true/forwarded=111/last_sequence=2340/last_error=null/restart_count=0`，证明 Redis→process-local WebSocket bridge 已接线。authenticated 同页 canary 无刷新看到 Task add/start/complete、final 与 terminal folding，raw `track_todo/report_progress` row 均为零；但 accepted input 与两条 model-authored public progress 在 live 和 reload 后都缺失。suffix 修复 commit `70c4a12f2` 随后由 backend=`e662497e-1d2a-4c97-ab6c-de10e0b0a209`、backend-api=`f3c0b6de-674c-47a1-abf3-940e30ddac7e`、frontend=`42c446b0-a945-4383-b400-5e95b1e26b73` 三个 production deployment 发布；tail-watermark commit `d29f52493` 又由 backend=`61bf292c-c32b-40c1-aa28-dfc58bd119d0`、backend-api=`a138b28c-067c-48db-84a9-40c729feb5e5`、frontend=`73d7bdfd-1b02-4908-be19-36137856078d` 发布，六个 deployment 均为 `SUCCESS`；health 的 schema/RLS/daemon/forwarder gate 与 frontend HTTP 200 均通过。
...
- production Red/path proof：terminal-fold source `d60a669bd` 已由 backend=`d83531f6-69c0-4f12-9e89-51bf9349502d`、backend-api=`e1ccd312-6fa1-4e1c-8044-e0eab851dcf7`、frontend=`26dc855a-8031-4b8d-8172-b50dda5e5f43` 发布且均 `SUCCESS`。fresh/no-refresh canary 的完成态能默认折叠、final 独立且展开恢复，证明 presentation contract 已接线；但 running commentary 仍需 reload。production PostgreSQL 对同一 run `fcb06d68-aabf-58a2-86f2-d37d057939fa` 已在 sequence `3156/3181` 提交两条 exact `assistant_commentary.completed`，分别早于 final 95/62 秒；CDP live capture 却从 canonical sequence `3229` 直接跳到 `3240/3243`。因此故障不在模型、持久化、折叠 reducer 或 final ownership，而在 live sequence delivery。
- production canary / verified root cause 2：private continuity commit `37599d2ca` 已由 backend=`a4a429bd-ab37-4fc8-8ab3-ab5ccb2fc9b0`、backend-api=`dc318d6b-4c82-49ee-aac7-a4288bb8268c`、frontend=`256f037e-55fb-4e81-b05e-4fd4d83d718c` 发布且均 `SUCCESS`；health=`ok`、forwarder=`running`、frontend HTTP 200。authenticated canary L 的 WebSocket 收到 21 条 `assistant_reasoning_private` event，均保留 sequence 且 `payload.content/parts` 不存在；public progress/milestone/final 分别为 sequence `3310/3347/3362`。终态默认 exact progress/milestone=`0/0`、final=`1`、Task dock=`1`；展开后 progress/milestone=`1/1` 且 raw `track_todo/report_progress=0/0`；折回及 hard reload 后再次只留 final，证明 terminal contract 已闭环。但 active snapshot 已 live 更新 Task 时 progress 仍未出现。production DB 对账证明缺失序号 `3293/3295/3296/3309/3319/3320/3331/3336/3337/3346/3356/3357/3368` 全是 schema V1 runtime row；它们与 V2 共用 Session cursor，却没有 `SessionEventOutbox`，所以 strict reducer 仍不能越过这些 V1 gap。
...
- deployment/active canary：V1 outbox follow-up commit `5e6d47b06` 已由 backend=`94836ad4-ddb1-45a1-93c2-385899343d98`、backend-api=`e43be680-30a4-4b1a-b588-9fc33c34f68d`、frontend=`cc49f826-11c0-4d69-a4b5-ddfa277eb865` 发布且均 `SUCCESS`。authenticated canary M 在同一未刷新 active Run 中看到 exact progress、Task 与 milestone，raw task/progress tool 仍为零，证明 `EVID-G2-020` 的 live shared-sequence closure 已真实接线。
...
- deployment：exact source commit `ee95cd98d` 已由 backend=`ace7f88d-d174-4036-87fa-f43669771588`、backend-api=`9e76b88e-0486-4504-b1ee-57bf0892aa77`、frontend=`78844f6d-e834-4de6-8647-523f633a0c16` 发布，三者均 `SUCCESS`；backend health=`ok`，frontend HTTP 200。
