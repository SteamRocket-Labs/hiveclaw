...
(证据:`${LOCAL_HOME}/Context Engineering/claude-code-org`)
...
(`AgentKernel.handle()`,常规 agent loop 默认 200 轮；heartbeat 已退为 direct T3 core，不再消费 tool-round budget),不是"一次性调用"。因此**无需启动新 run**——
