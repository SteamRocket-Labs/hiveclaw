...
|---|---|---|---|
| **A** | 内置类型零 prompt（`_TYPE_PRESETS` 只有工具白名单；运行时 spawn 即裸 LLM + 任务文本 = L1 违例） | 内置 agent 是代码常量但每个都带完整打磨 prompt（`exploreAgent.ts` ~35 行 / `generalPurposeAgent.ts` / `verificationAgent.ts` 含失败模式预判 + rationalization 自识别） | `_TYPE_PROMPTS` 三段 CC 移植文本（explorer←Explore 工具名落地 Hive 面；general-purpose←CC general-purpose；critic←verification 适配 read-only 工具面，保留 VERDICT: PASS/FAIL/PARTIAL 输出契约；历史 worker 仅作为 alias 归一到 general-purpose）。运行时 fallback 与 `resolve_subagent_tools` 的 preset fallback 完全同构：显式 body 替换基线，空 body 落基线 |
