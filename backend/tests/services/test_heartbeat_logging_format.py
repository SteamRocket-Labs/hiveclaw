...
HEARTBEAT_PATH = Path(__file__).resolve().parents[2] / "app" / "services" / "heartbeat.py"
LOG_METHODS = {"debug", "info", "warning", "error", "critical", "exception"}
PERCENT_PLACEHOLDERS = ("%s", "%d", "%r", "%f")


def test_heartbeat_loguru_calls_use_brace_formatting() -> None:
    tree = ast.parse(HEARTBEAT_PATH.read_text(encoding="utf-8"))
    offenders: list[str] = []
