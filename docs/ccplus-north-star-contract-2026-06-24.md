...
1. `${LOCAL_HOME}/vc-saas/free-code-main` as the runnable CC / FreeCode semantic baseline.
2. `${LOCAL_HOME}/Context Engineering/claw-code/src` as Python-port reference only, not a full parity source.
3. `${LOCAL_HOME}/Context Engineering/claw-code/rust` for session hygiene, workspace partition, JSONL rotation, resume, fork, and compact lessons.
4. `${LOCAL_HOME}/Context Engineering/claude-code-org` as a cross-check against FreeCode.
5. `${LOCAL_HOME}/Context Engineering/codex/codex-rs` as Codex delta only.
