...
> 状态：设计权威；Group 2/3/4 substrate 已分别形成并部署，但 2026-07-17 live-entry/path 复核重新打开 `P1-004`、`SES-CONSUMER-001`、`A2A-TERMINAL-001`、`TEAM-FANOUT-001`、`ROOT-TREE-001` 五个回归 leaf。`EVID-G2-016/017` 已证明 Task 专用消费、公开 commentary 持久化/reload、rolling render ownership 与 Peer A2A read-only。`EVID-G2-018` 随 commit `9ebb51f1b` 三服务发布后，production health 已证明 runtime forwarder `running=true/forwarded=111/last_sequence=2340/last_error=null`，同页 canary 也无刷新得到 Task/final 且 raw task/progress row 为零。§28.5 / `EVID-G2-019` 的 suffix baseline、tail-watermark 与 first-snapshot `liveReady` 已分别随 commit `70c4a12f2`、`d29f52493`、`9ad195fc3` 三服务发布；authenticated canary F 证明 Task 专用面和 raw-row suppression 成立，但首个 transcript REST page 返回 `HTTP 504` 时，WebSocket 仍没有安全 cursor，公开 progress 继续等待 reload。当前修复增加 typed `cursor_mode=live_tail`：服务端在注册 live buffer 后选择当前 durable watermark，客户端从该 fence 接收新事件，完整旧历史继续由 REST 后台恢复；新 source 发布和 no-refresh/hard-reload 等价 canary 前 `SES-CONSUMER-001` 仍为 `in_progress-local-green-live-tail`。完整 Session V2 继续由 Group 6/7/8/9/10 验收。
>
...
| 当前 Hive 七原子审计 | 已完成并持续更新；5 leaf 仍 open | §28.4 五个协作回归已部署但行为验收 open；§28.5 又以真实截图和连续三轮 production canary 坐实 `SES-CONSUMER-001` 的 first-snapshot live presentation 缺口。独立 `liveReady` 仅 local Green，三服务发布与 authenticated browser canary 前总报告仍维持 38/103 |
...
| Frontend typed projection | `in_progress-local-green-live-tail:EVID-G2-016/017/018/019` | 三类协作、Composer Task、query invalidation、final artifact、read-only A2A、commentary persist/reload 与 live bridge 已部署/canary；suffix/tail-watermark/first-snapshot readiness 已发布，typed `live_tail` 已 Green，待三服务发布与 no-refresh + hard reload 终验 |
...
| 自动化黄金轨迹验收 | 既有 Group 4 保持；`EVID-G2-016/017/018` 保持，`019` local Green | `019`：production-shaped suffix Red 首次 `2 failed / 3 passed`；Green focused=`6 files / 194 tests`、clean exact-source frontend full=`120 files / 730 tests`、build/budget 绿；production live/reload canary 待写回 |
| Railway 行为修复与生产验收 | `EVID-G2-016/017/018` 已提供 durable/reload/live-bridge proof；`019` 待发布 | post-bridge health=`process_role:runtime`、forwarder `running=true/forwarded=111/last_sequence=2340`；production DB 有 accepted input/commentary/final，但 latest-page projection 丢前两者。新 source 需三服务同源发布并用 authenticated no-refresh/hard-reload canary 关闭 |
