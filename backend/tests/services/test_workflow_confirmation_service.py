...
def _preview(*, now: datetime | None = None) -> WorkflowPreviewArtifact:
    ...
    return WorkflowPreviewArtifact(
        ...
        preview_json={"confirmation_required": True},
        expires_at=created_at + timedelta(hours=1),
    )


def test_claim_binds_identity_confirmation_and_deterministic_run() -> None:
    ...
    assert preview.confirmation_source == "agent_user_turn"
...
def test_failed_preview_retries_same_run_identity_after_lease_release() -> None:
    ...
    identity = {
        ...
        "user_id": preview.requested_by_user_id,
        "confirmation_source": "agent_user_turn",
        "confirmation_evidence_id": "turn-9",
    }
    assert claim_workflow_preview_record(preview, now=now, **identity) == "claimed"
