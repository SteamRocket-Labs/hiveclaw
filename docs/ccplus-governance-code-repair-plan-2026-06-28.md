...
| D10 killed-process E2E | `tests/e2e/test_tool_call_recovery_closure.py` 原本只有 recovery manifest 序列化和 request-preflight compaction 事件测试，不是 crash/fork/denial 恢复矩阵；本轮补测同时发现 D4 新增的 `pending_skill_handoffs/executed_skill_handoffs` 没进入 manifest | 已落地：`RecoveryManifest` 增加 `pending_skill_handoffs`、`executed_skill_handoffs`、`continuation_records`；从 live `SessionContext.metadata` 和 persisted `runtime_artifacts/recovery_manifest.json` 两条路径恢复，覆盖 session permission pending frame、compaction lifecycle、MCP assignments、truth evidence、Skill fork handoff、denial continuation metadata。unit runner 中不伪称 OS-level kill，而以持久化 artifact 重建路径模拟 process crash/restart | 红线：`cd backend && .venv/bin/python -m pytest tests/e2e/test_tool_call_recovery_closure.py::test_recovery_manifest_preserves_skill_fork_and_denial_continuation_shape tests/e2e/test_tool_call_recovery_closure.py::test_persisted_recovery_manifest_restores_full_crash_matrix -q` 旧实现 2 failed，修复后 `2 passed, 4 warnings`。回归：`cd backend && .venv/bin/python -m pytest tests/e2e/test_tool_call_recovery_closure.py tests/runtime/test_recovery_manifest_persistence.py -q` -> `12 passed, 4 warnings` |
