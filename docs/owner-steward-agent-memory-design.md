...
### Phase 14: Persistent Coordination + Approval Queue

Goal: Lease, Signal, Checkpoint state survives process restart and is
visible across two Hive workers, so confirm-first approvals are durable
and duplicate cross-process delegations cannot race for the same task.

- Add `agents/coordination_store.py` with `SqliteCoordinationStore`
  backed by stdlib `sqlite3`. Lease, signal, checkpoint tables live in
  a single shared db file (typically under `workspace/<tenant>/
  coordination.db`).
- API mirrors the in-process `CoordinationRuntime`: `acquire_lease`,
  `send_signal`, `read_signals`, `create_checkpoint`, `get_checkpoint`,
  `escalate_expired_checkpoints`.
- Lease acquisition is serialized via the table `PRIMARY KEY(task_key)`
  with expiry check inside the same transaction.
- Sentinel state is intentionally not persisted — it is re-derived per
  proactive-loop tick and would otherwise need a `last_fire_at` column
  for every owner-defined predicate.
- Runtime gate: code paths that need cross-process correctness instantiate
  `SqliteCoordinationStore(db_path)` instead of constructing a fresh
  `CoordinationRuntime()`. Existing in-process callers are unaffected.
- Utility smoke: two `SqliteCoordinationStore` instances pointing at the
  same file behave like two processes — the second one cannot acquire a
  live lease, sees signals written by the first, and observes a
  checkpoint escalation after the first one fires
  `escalate_expired_checkpoints`.

### Phase 15: Charter Calibration Approval Surface
...
- Add `services/charter_proposals.py` with `CharterProposal` dataclass,
  `ProposalKind` enum (`consider_full_authority`, `tighten_to_confirm_first`),
  `ProposalStatus` enum (`pending`, `approved`, `rejected`, `expired`),
  and `CharterProposalStore` backed by stdlib `sqlite3`.
...
| 15 Charter Calibration Approval Surface | Completed 2026-05-22 | Code paths: `backend/app/services/charter_proposals.py`, `backend/tests/services/test_charter_proposals.py`. Runtime gate available: dream's `propose_charter_calibrations_from_feedback()` output now has a durable home where the owner can `approve` / `reject` each proposal with a recorded `decided_by` + `decision_reason`. Tests: red run failed with `ModuleNotFoundError: No module named 'app.services.charter_proposals'`; green run `pytest tests/services/test_charter_proposals.py` -> `10 passed in 0.03s`; lint `ruff check ... && ruff format ...` -> passed. Utility smoke: a `consider_full_authority` proposal submitted by dream is later approved by `alice` with reason "LGTM"; a 14-day-old pending proposal moves to `expired` after `expire_stale(max_age_days=7)`; reopening the store from disk preserves all decisions. Residual risk: applying an approved proposal back to the agent's frozen charter still requires a downstream `Phase 6 sketch->active` step which lives in HR / soul rendering; the proposal store itself does not mutate any charter. Closes the Phase 5/6 missing-approval-surface residual. |
| 14 Persistent Coordination + Approval Queue | Completed 2026-05-22 | Code paths: `backend/app/agents/coordination_store.py`, `backend/tests/agents/test_coordination_store_sqlite.py`. Runtime gate available: callers needing cross-process correctness instantiate `SqliteCoordinationStore(db_path)` instead of the in-process `CoordinationRuntime`; lease acquisition is serialized by `PRIMARY KEY(task_key)` with expiry recheck in the same transaction. Tests: red run failed with `ModuleNotFoundError: No module named 'app.agents.coordination_store'`; green run `pytest tests/agents/test_coordination_store_sqlite.py` -> `5 passed in 0.09s`; lint `ruff check ... && ruff format ...` -> passed. Utility smoke: two store instances pointing at the same sqlite file behave like two processes — duplicate `acquire_lease(task-1)` returns `acquired=False` with the existing lease id, signals/checkpoints created by one store are immediately visible to the other, and `escalate_expired_checkpoints` mutations are observed across instances. Residual risk: production wiring (which call-sites should opt into the sqlite store vs in-memory runtime) is a follow-on integration; Sentinel state remains process-local because it is re-derived per proactive-loop tick. Closes the Phase 7 process-local residual. |
