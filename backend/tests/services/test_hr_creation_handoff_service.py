...
async def _seed_handoff_authority(owner_sessionmaker, *, include_hr: bool = True):
    ...
    tenant_id, user_id, source_agent_id, source_session_id, source_run_id = (
...
async def test_handoff_creates_one_hr_session_and_one_run_across_replay(owner_sessionmaker) -> None:
    ...
    async with owner_sessionmaker() as db:
        ...
        assert await db.scalar(select(func.count()).select_from(Agent).where(Agent.tenant_id == authority["tenant_id"])) == 2
        assert await db.scalar(select(func.count()).select_from(ChatSession).where(ChatSession.id == hr_session_id)) == 1
