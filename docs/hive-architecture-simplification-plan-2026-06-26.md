...
Date: 2026-06-26
Status: architecture diagnosis + consolidation plan(讨论稿,实施需主理人逐项拍板; 2026-07-02 按当前 HEAD 复核修订)
Method: 6 个并行只读勘察 agent 全量测绘(2026-06-26 快照:services 207 / memory 30+自进化 / kernel-runtime-tools 核心 84 / api 75+models 53+core 12 / 前端 / 兼容-死面全局普查)+ 主理人复核; 2026-07-02 复核当前 HEAD:services root 214 / services total 254 / memory 38 / api 72 / models 52,`legacy`/`compatibility`/`deferred` 命中文件数为 110/45/30
...
> **真正能"删"的死代码 < 1% LOC;重量是认知负担——当前 110 个 legacy 命中文件不是 110 份垃圾,是 110 处"还有另一条路"。**
...
| # | 模式 | 代表证据 | 性质 |
|---|---|---|---|
| **P1** | **Legacy 双轨从不退役** | 记忆死 curator(`scene_curator`+`wiki_curator` 真孤儿;`memory_curation` 是 evolution_daemon 接线但永返 `disabled` 的死 stub。注:`distillation_audit` **不在此列,它 LIVE**);extract-first 老管线(`extract_agent` 62KB+`t0_logger` 51KB+`t2_store`);no-op 空壳(`enhancement.py`/`backend.py`) | 主体,当前约 110 个 legacy 命中文件来源 |
...
| **P6** | **错放 + 地图过期** | `prompt_eval`+`task_eval`(1400 行 eval 工具)错放 `runtime/` 核心目录;前端 `Dashboard.tsx`(855 行)纯孤儿;CLAUDE.md 计数全过期(services 写 163,2026-07-02 当前 root 214 / total 254) | 杂项 |
...
**基础 canonical 面必须保留**:T0/T2/T3/soul 四层分层、`ccplus_contracts` live 契约族、`hooks.py` 的 Hook wire standard、`tools/audit.py` 这类三源对账/startup 断言、`invocation_spans` DB trace、Memory Gate + Platform Gate、RLS/权限治理都是有意结构,不是"兼容包袱"。本方案砍的是并行机制和过期入口,不是这些北极星能力。

**近期已合入的 Session UX / permission / artifact canonical 面也要保留(2026-07-02 复核)**:`docs/ccplus-session-ux-contract-2026-06-26.md` 是 session 用户体验契约;`services/chat_artifact_delivery.py` 是当前单一路径的 session artifact 交付 helper;`ChatArtifact` + `artifact_delivery` transcript event + 前端 artifact card / inspector 是活的交付物路径;`ChatSession.transcript_metadata_json` / `RuntimeTask.metadata_json` / `runtime_session_context.metadata` 的 permission profile 三层同步是当前运行面,不能在清理时当成重复字段随手删。它们需要另走 §5.4 SUNSET/收口纪律,不是 §5.1 死代码删除。

**Hook runner 当前也必须保留(2026-07-02 复核)**:`runtime/hook_runner.py::GovernedHookRunner` 已不再是 2026-06-26 的 deferred orphan。当前生产接线为 `main.py` startup → `services/plugin_hook_service.py::register_installed_plugin_hooks` → `HookRegistry` → `GovernedHookRunner`;`hook.command` / `hook.prompt` / `hook.http` / `hook.agent` 通过 plugin hook registration 唯一路径进入。它应归入 Hook/plugin canonical 路径,不能再列为删除候选。

### 3.3 services/ 根目录 214 个 py → 13 个 domain 包(= "重打包",见 §7)
...
### 4.1 近期已合入基线先决条件(Phase 0)

2026-06-29 到 2026-07-01,Session UX / permission / artifact / workflow/chat UI 主线已经合入 `main`(例如 2026-06-30 单次 session/continuation/workflow/chat UI 改动 2400+ 行,2026-07-01 又继续修 writable chat sessions)。这条主线不推翻本精简计划,但它把 Phase 0 从"当前工作树未稳定 diff"改成了"当前 HEAD 基线回归闸":删除旧代码前先证明新 session 基线稳定,否则 A 阶段失败会被误判成删除导致。

**新的执行前提**:先完成 Phase 0(当前 HEAD 的 session 目标测试 + contract 核对),再动 §5.1 的删除候选。Phase 0 的验收重点:
...
- 当前基线回归绿后,再进入 §5.1 删除,避免把"新 session 行为失败"误判成"删除死代码失败"。
...
**穷举核实结果(2026-06-26,只读,逐项过闸; 2026-07-02 按当前 HEAD 修订)** — grep 覆盖 `app/**/*.py` 静态+函数内动态 import+字符串引用;带测试的删除连其测试一起删。注意:后续提交已把 #12 反转为 live wired,#8 也有测试保护 alias,所以本表不能再按"13 项一张执行单"使用。
...
| 8 | `runtime_guidance_catalog.py` 两 alias(`RuntimeGuidanceCatalogEntry`/`ATTACHMENT_ALIGNMENT_CATALOG`) | ⚠️ 仍是删除候选,但非"测试零引用" | app 零;`ATTACHMENT_ALIGNMENT_CATALOG` 被 `backend/tests/kernel/test_runtime_guidance_catalog.py` 显式保护。删除前必须先判断是否继续保留 backwards-compatible export,并同步改测试;**留底层真名** `CC_NATIVE_ATTACHMENT_CATALOG`(live) |
...
| 12 | `runtime/hook_runner.py`(`GovernedHookRunner`,=§9) | ❌ 2026-07-02 反转为 LIVE,不删 | 当前生产接线已存在:`main.py:416-418` startup 调 `register_installed_plugin_hooks`;`plugin_hook_service.py:24/200/311-319` import/build/register `GovernedHookRunner`;`hooks.py:844/1288` 把 `governed_hook_runner` 纳入 trust/runtime catalog;`hook_runner.py` docstring 明写 production wiring 入口 |
...
**确认未过闸、保留(LIVE,本次复核坐实)**:`distillation_audit.py`(`evolution_daemon:196`+`auto_dream:417/1170` 在写)、`search_smithery`(`web_mcp:2096` 在调)、`agent_manager.py`(hr/advanced/agents/seeder 在依赖)、`hook_runner.py`/`GovernedHookRunner`(plugin hook production path 在用)→ 全留;`container_candidate`(`memory.py:77` public schema)→ §5.4 SUNSET;`enhancement.py`/`backend.py`(`main.py:540`/`memory.py:371`/`evolution_daemon:221` 在调,no-op 但被接线)→ §5.3 改 3 处调用点的小重构,非删。

**净结果(2026-07-02 当前 HEAD)**:原"13 项过闸可删"已失效。扣除 #12 `hook_runner.py` 反转为 live 后,最多剩 12 项删除候选;其中 #6 是局部删,#13 需连删 `evolution_daemon` 整个 try/except 块,#8 需先处理测试保护/compat export 决策。A 阶段验收不再按"数量"算,而是逐项重新过 caller 闸 + docs/tests 同步清理。

**删除验收补充(2026-07-02 当前 HEAD)**:A 阶段不只是"删代码 + 跑测试"。凡删除本表项目,必须同步移除/改写仍把旧路径当当前实现的 truth docs 和保护旧实现的 tests。否则会重新制造"地图对不上地形"。尤其要核 `extract_queue_replay`、`heartbeat_reflection_backfill`、`understanding_store`、`PromotionRouter`、`scene_curator`/`wiki_curator`、`Dashboard.tsx` 在 docs 下的当前 truth 引用;历史/archive 允许保留,当前 truth doc 必须改成"已退役"或移除执行入口。`hook_runner` 反过来要清理的是旧 docs 中"零生产调用/未接线/默认删"这类过期判断,不是删除代码。

**执行前核验命令(删任何东西前必跑)**:

```bash
# 0) 先确认当前 HEAD 的 Hook runner 是 live wired,不能误删
rg -n "GovernedHookRunner|hook\\.command|hook\\.prompt|hook\\.http|hook\\.agent|register_installed_plugin_hooks|governed_hook_runner" backend/app backend/tests docs --glob '!docs/archive/**'
...
rg -n "extract_queue_replay|heartbeat_reflection_backfill|understanding_store|PromotionRouter|scene_curator|wiki_curator|Dashboard\.tsx" docs --glob '!docs/archive/**'
# 2b) 复核当前 truth docs 不再把 Hook runner 写成 deferred/dead
rg -n "hook_runner|GovernedHookRunner|零生产调用|未接线|deferred-only|默认删" docs --glob '!docs/archive/**'
# 3) 后端全量回归(删后必须仍绿)
...
**现状(2026-07-02 当前 HEAD)**:`app/services/` 根目录仍有 **214 个 Python 文件平铺**(含子目录总计 254 个 Python 文件)。已有 `agent_tool_domains/`、`code_execution/`、`skill_creator_files/` 等局部子目录,但主体仍在根目录堆叠;要找"workflow 相关"仍得在 200+ 个根文件名里靠前缀认(`workflow_runtime_service.py`/`workflow_daemon.py`/`workflow_launch.py`…)。
...
**它改的只是文件位置(import 路径),行为零变化**(T1 低风险,纯 git mv + 改 import,测试守门)。**收益**:打开 `services/` 一眼看到 13 个 domain 文件夹而不是 214 个根文件名;新代码"该放哪"有明确归属,不会再随手平铺加文件。
...
4. **没有 live 消费者不准种契约**;确需 deferred 必须显式 fence + 标期限;一旦接入生产路径,必须反向更新清理计划,不能继续按 deferred/dead 处理。
...
## 9. CC Hook 现状(2026-07-02 当前 HEAD)

此前 2026-06-26 的判断是:`cc_hook_contract.py` 删除、CC wire standard 折叠进 `hooks.py`,`hook_runner.py` 仍是 deferred 外部 runner,可在"退役 vs 接通"之间二选一。

这个判断已经被后续提交反转。当前事实是:`hook_runner.py` 已经接通为 plugin hook 的外部 hook runner,生产路径为:

```
pack manifest hook.*
  → PluginHookRegistration
  → services/plugin_hook_service.py
  → HookRegistry.register_spec(... handler_name="governed_hook_runner")
  → runtime/hook_runner.py::GovernedHookRunner
```

因此 `GovernedHookRunner` 不再是可删项。正确收口方向是:**保留一个入口**(plugin hook registration → shared `HookRegistry` → `GovernedHookRunner`),禁止再新增第二套 external hook runner / 第二张 hook invocation truth table。任何 cleanup 只清理过期 docs/tests 中"未接线、默认删"的旧判断,不删 live runner。
...
| 阶段 | 内容 | Tier | 何时 |
|---|---|---|---|
| **0. Session 基线回归** | 已合入的 Session UX / permission profile / artifact delivery 改动先跑目标测试并核对契约:permission 三层 metadata 不漂移、artifact replay 可见、destructive delete one-shot、UI 只暴露三种 permission mode | T1/T2 | **A 之前** |
| **A. 逐项核实退役**(非"一把清") | §5.1 候选**逐项过"穷举 caller 闸"**后才删 + §5.5 删 Dashboard/归位 eval;`hook_runner` 已反转为 live,不属于 A 删除项 | T0/T1 | 先做穷举审计,只删全过闸的项;Codex 已证 census grep 不够 |
...
**先跑 Phase 0 当前 HEAD session 基线回归,再动 A 中仍过闸的删除候选(逐项重跑 caller 证据,见 §5.1),低风险但删后仍需全量回归兜底,非"零风险一把清";C 第 1 步(freeze 纪律)今天就能立,从此债不再长大。D/F 是碰生产的部分,逐个走 expand→verify→contract,不 big-bang。**
...
- 不删 T0/T2/T3/soul 分层、ccplus_contracts live 族、Hook/plugin live runner、audit 三源对账等有意结构。
