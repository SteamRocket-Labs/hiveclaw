"""Canonical schedule API surface on top of AgentTrigger."""
...
def is_schedule_surface_trigger(trigger: AgentTrigger | object) -> bool:
...
def schedule_response_payload(trigger: AgentTrigger | object, *, creator_username: str | None = None) -> dict:
    """Map a schedule-surface trigger into the schedule API response shape."""
    cfg = getattr(trigger, "config", None) or {}
...
def mark_schedule_manual_pending(config: dict | None) -> dict:
    """Mark a schedule-surface trigger for immediate one-shot execution."""
    cfg = dict(config or {})
...
def schedule_manual_evaluation(trigger: AgentTrigger | object) -> dict | None:
    """Return a manual fire event key when a schedule-surface trigger is pending manual execution."""
    if not is_schedule_surface_trigger(trigger):
        return None
    cfg = getattr(trigger, "config", None) or {}
...
def build_schedule_activity_entries(triggers: list[AgentTrigger | object], reply: str | None) -> list[dict]:
    """Build schedule activity log entries for schedule-surface triggers."""
    entries: list[dict] = []
    reply_text = reply or ""
