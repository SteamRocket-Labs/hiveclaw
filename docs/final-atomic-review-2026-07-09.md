...
2. **⚠️ 脆弱点（本机绝对路径入生产 payload）**：`codex_optimization_ledger.py:50` 内嵌 `${LOCAL_HOME}/Context Engineering/codex/...` 本机路径，被 `session_control_plane.py:1728` 消费进控制面 payload → 换环境即无意义。应改为相对引用或移除。
