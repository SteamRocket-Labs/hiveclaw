...
| G3 | 长期记忆 SOTA | governed write gate、activation context、principal stripping、memory hygiene | 双时态 KG、multi-hop/PPR、TTL、引用重校验、conflict ledger | retrieval tests + memory sidecar + live task recall eval |
| G4 | Durable execution | RuntimeTask、web-chat resume、workflow completion side-effect dedup、orphan reconcile | mutating delegation/subagent step-level journal replay；Temporal-like completion boundary 全覆盖 | restart tests + duplicate side-effect tests + production traces |
...
| G7 | Subagent / Delegation | `spawn_subagent`、peer delegation、worker profiles、tenant-scoped definitions/memory、coordination signals、replay-safe resume | mutating subagent/delegation step journal；live multi-agent behavior eval；subagent memory evolution 证据 | subagent runtime tests + orchestration traces + behavior eval |
