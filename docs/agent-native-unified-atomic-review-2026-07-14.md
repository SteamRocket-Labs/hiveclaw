...
- P1 | SES-CONSUMER-001 | in_progress-local-green-live-tail:EVID-G2-016/017/018/019 | 三类协作、固定 Task、final artifact、Peer A2A read-only、canonical commentary persist/render、rolling ownership、exact Session store 与 live bridge 已绿；suffix/tail-watermark/first-snapshot readiness 已发布，typed `live_tail` 已本地 Green，待三服务发布与 no-refresh/reload/ordering canary
...
| 2 | `EVID-G2-*` | 14 leaf / 0 Missing | `in_progress`：13/14 closed；`EVID-G2-016/017` 已证明 Task、commentary persistence/replay 与 A2A read-only；`EVID-G2-018` 已证明 live bridge；`EVID-G2-019` 的 typed live-tail 已由 `a1be2dfbe` 三服务发布。canary G 证明 Task/final/commentary 无刷新到达且 terminal 展开可恢复；canary H 又证伪 `bdb68fbd3` 把 commentary 设为 terminal 常驻，最终 terminal-fold presentation 已本地 Green | 发布最终 terminal-fold source；真实模型必须在 running 时实时显示 progress/milestone，completed Run 默认折叠后只留 final，展开恢复完整过程，raw task/progress tool 不出现，Task/final live，hard reload 后顺序和 render ownership 等价；不得用 fixture、REST reload 或 deployment success 代替默认产品路径 proof |
