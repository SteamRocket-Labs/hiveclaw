...
- Final integration pass：coding pack 保持默认不开、只由 agent/plugin assignment 手动启用；`SessionIndex` 已暴露 `/agents/{agent_id}/sessions/{session_id}/index` 并接入 frontend adapter；`PermissionsPrompt` 已进入 dynamic suffix；Codex-style `CompactionTraceContext` 已接入 kernel 所有 `maybe_compress_messages` call site；Hook runtime config 已 agent-scoped durable persisted，并在 startup load。
- Prompt Text Contract：`docs/prompt-text-contracts-cc-codex-2026-06-22.md` 已把 Frozen Prefix、Dynamic Suffix、Tool Use、Tool Search、Skill、Sub-agent、Workflow、Hooks、Plan Mode、Goal、Permissions、Session Continuity、Team、Work Ledger 等 prompt surface 收束成可回归文本契约；Dream/Memory 明确留作下一阶段机制重构，不混入 prompt 层。
...
| Source | Role |
| --- | --- |
| `/Users/example-owner/vc-saas/free-code-main` | 第一参考源；用于判定 CC / FreeCode 的真实 runnable semantics。 |
| `/Users/example-owner/Context Engineering/claude-code-org` | 与 FreeCode 交叉确认 CC 语义，尤其 hooks / session / command / recovery。 |
| `/Users/example-owner/Context Engineering/claw-code` | Python 化方向和已有端口边界参考，不作为完整 parity 判定源。 |
| `/Users/example-owner/Context Engineering/codex/codex-rs` | Codex delta；只吸收更好的 thread / compaction / prompt / telemetry / policy 机制，不覆盖 CC baseline。 |
...
| --- | --- | --- | --- | --- |
| Full-suite evidence | 定向 backend parity tests、frontend command/chat tests、ruff、本轮 final integration tests 已跑。 | 还没有本轮 clean env 的全量 `pytest tests -q` 与 live browser pass。 | 跑全量 backend/frontend；补 live smoke：command palette、Team member session switching、Goal continuation、hook visibility。 | 是 release gate，不是代码功能缺口。 |
...
| Hook runner mechanics | `HookEvent` 已覆盖 CC core + FreeCode remaining events；governed Hook Runner 支持 command/prompt/http/agent type 的治理执行、transcript attachment/progress/summary、typed span fact；runtime config 已 agent-scoped durable persisted + startup load。 | 与 CC 本地 raw shell hook 仍有安全形态差异：Hive 不允许绕过治理直接跑 shell；这是 intentional divergence。 | 后续只需 live smoke 验证 Stop/TaskCompleted/TeammateIdle 产品路径。 | Code-level closed；live evidence pending。 |
...
| Session index / picker UX | Session T0、branch、resume、rollback、export 已有命令路径；`SessionIndex` read model 已暴露 API 并接入 frontend adapter。 | UI picker/search 的产品体验还需要 browser pass。 | 后续 UI/live smoke 验证。 | Code-level closed；UX evidence pending。 |
...
| Prompt parity regression | Hive prompt stack已经很强，且有 runtime guidance / plan / work ledger / memory boundary 文档；effective permissions prompt 已进入 dynamic suffix；`prompt-text-contracts-cc-codex-2026-06-22.md` 已补齐统一文本契约。 | 现在差距从“没有统一文本契约”收窄为“尚未把全部 surface 做成 golden fixture/snapshot”。 | 按该契约建 prompt module + golden tests；每个生命周期 surface 有固定 template、fixture、diff review。Dream/Memory 另起机制重构，不塞进 prompt 层。 | 是 quality gate。 |
...
5. **剩余值得补的不是大功能，而是 quality gate。** Prompt 文本契约已写入 `docs/prompt-text-contracts-cc-codex-2026-06-22.md`；接下来需要 prompt golden tests 钉住上下文段落顺序和关键约束；需要 provider-neutral auto-defer threshold，根据 tool schema token 占比自动决定是否只暴露 core + tool_search；需要 live trace 证明 deferred tool discovery 在 resume/compact/team member session 中不丢失。
...
| --- | --- | --- | --- | --- |
| First-class compaction input | Codex `CompactionInput` 明确包含 model、input、instructions、tools、parallel tool calls、reasoning、service tier、prompt cache key、text controls。见 `/Users/example-owner/Context Engineering/codex/codex-rs/codex-api/src/common.rs:23-40`。 | 本轮新增 provider-neutral `CompactionRequest`。 | 已补 code-level substrate；后续可把所有 kernel compact call sites 逐步改为该 typed contract。 | Code-level closed for substrate。 |
| Compaction attempt + checkpoint trace | Codex 把 compaction request attempt 和 installed checkpoint 分开；checkpoint 记录 input history 与 replacement history。见 `/Users/example-owner/Context Engineering/codex/codex-rs/rollout-trace/src/compaction.rs:30-38`、`:78-87`、`:118-166`。 | 本轮新增 `CompactionTraceContext`，稳定 `compaction_id`，attempt completed 与 installed checkpoint 分离。 | 已补 code-level substrate；后续接入全部 kernel compact call sites。 | Code-level closed for substrate。 |
| Unified thread/session persistence metadata | Codex `CreateThreadParams` / `ResumeThreadParams` 保存 `thread_id`、`forked_from_id`、`parent_thread_id`、source、dynamic tools、metadata、event persistence mode、rollout path/history。见 `/Users/example-owner/Context Engineering/codex/codex-rs/thread-store/src/types.rs:45-106`。 | 本轮新增 `SessionIndex` read model。 | 已补 backend substrate；后续接 UI picker/search。 | Code-level closed。 |
| Goal prompt variants | Codex goals prompt 模块拆出 continuation、budget limit、objective updated 三种 template，并转义 XML 文本。见 `/Users/example-owner/Context Engineering/codex/codex-rs/prompts/src/goals.rs:5-28`、`:30-99`。 | 本轮新增 `runtime.prompts.goals` 并接入 Goal continuation；budget-limited prompt 写 metadata。 | 已补。 | Code-level closed。 |
| Policy-derived permission prompt | Codex permissions prompt 从 effective permission profile、approval policy、exec policy、network、writable roots、denied reads 渲染。见 `/Users/example-owner/Context Engineering/codex/codex-rs/prompts/src/permissions_instructions.rs:48-90`、`:112-141`。 | 本轮新增 `runtime.prompts.permissions`。 | 已补 prompt substrate；后续接入动态 prompt suffix。 | Code-level closed for substrate。 |
| Typed analytics facts | Codex typed facts 覆盖 turn resolved config、token usage、turn status、turn steer rejection、skill invocation、subagent thread start、compaction event。见 `/Users/example-owner/Context Engineering/codex/codex-rs/analytics/src/facts.rs:64-136`、`:178-212`、`:214-276`、`:326-330`。 | Hive 有 `InvocationSpan` canonical trace，但 payload 类型仍可更强。 | 在 `InvocationSpan` 上定义 typed fact schemas：turn_config、turn_token_usage、turn_steer、skill_invocation、hook_run、subagent_thread_started、compaction。前端/ops 直接读 typed facts。 | P1 |
| Append-only history hardening | Codex message history 是 JSONL，一行一个对象；并强调 `O_APPEND`、single write、advisory lock、max-byte trimming。见 `/Users/example-owner/Context Engineering/codex/codex-rs/message-history/src/lib.rs:1-15`、`:81-180`、`:183-220`。 | Hive T0 已是 hash-chained JSONL truth；本轮已把 JSONL append 改为 `O_APPEND` + advisory lock + single `os.write` + fsync。 | 已补核心 append hardening；segment max-byte rotation/partial-line quarantine 可作为后续 durability enhancement。 | Code-level closed for append path。 |
| Turn steer rejection semantics | Codex 把 mid-turn steering 拒绝原因类型化：no active turn、expected turn mismatch、non-steerable compact/review、empty/too large。见 `/Users/example-owner/Context Engineering/codex/codex-rs/analytics/src/facts.rs:110-176`。 | Hive 有 active run、queued input、plan/goal/team command，但用户输入在 non-steerable 状态下的拒绝理由可以更明确。 | 对 queued message / command / team member input 增加 typed rejection reason，并写入 T0 + UI toast + InvocationSpan。 | P1 |
...
8. Prompt Text Contract：CC/Codex prompt surface 文本契约。**已补文档契约 + architecture test；golden fixture/snapshot implementation 是下一步 quality gate。**
...
1. 按 `docs/prompt-text-contracts-cc-codex-2026-06-22.md` 建 CC/Codex prompt golden suite，进入 Prompt 层对齐。
2. 单独重构 Dream/Memory 机制：把多 session T0 corpus、T2 Segment Package、T3 semantic layer、`soul.md` promotion 的关系重新定义清楚，避免把 Dream 误当 prompt 总结。
3. 做 Stop/TaskCompleted/TeammateIdle、Team enter/close、Goal continuation、killed-process resume 的 live smoke。
4. 跑全量测试和 live smoke，把 code-level closed 升级成 production-ready closed。
