...
async def test_trigger_end_writes_t0_session_ledger_not_legacy_logs(monkeypatch, tmp_path) -> None:
    ...
    append_t0_session_event(
        ...
        role="user",
        content="trigger accepted user work",
        source="trigger",
        data_root=tmp_path,
    )
    append_t0_session_event(
        ...
        role="tool",
        content='{"name":"web_search","result":"full tool payload"}',
        source="trigger",
        data_root=tmp_path,
    )
    append_t0_session_event(
    ...
    await _t0_trigger_end(
        HookContext(
            ...
            session_id=session_id,
            source="trigger",
            messages=[{"role": "assistant", "content": "HOOK SUMMARY MUST NOT BECOME T0"}],
    ...
    assert [(event.event_type, event.role, event.content) for event in events] == [
        ("user_message", "user", "trigger accepted user work"),
        ("tool_result", "tool", '{"name":"web_search","result":"full tool payload"}'),
        ("assistant_message", "assistant", "trigger completed"),
        ("segment_boundary", "system", "trigger_end"),
    ]
    assert all("HOOK SUMMARY MUST NOT BECOME T0" not in event.content for event in events)
    assert events[-1].metadata["trigger_id"] == "daily-brief"
...
async def test_delegation_end_writes_t0_session_ledger_not_legacy_logs(monkeypatch, tmp_path) -> None:
    ...
    await _t0_delegation_end(
        HookContext(
            ...
            session_id=session_id,
            source="delegation",
            messages=[{"role": "assistant", "content": "DELEGATION HOOK SUMMARY MUST NOT BECOME T0"}],
    ...
    events = replay_t0_session_events(agent_id=agent_id, session_id=session_id, data_root=tmp_path)
    assert [(event.event_type, event.role, event.content) for event in events] == [
        ("user_message", "user", "delegated task prompt"),
        ("assistant_message", "assistant", "handoff result"),
        ("segment_boundary", "system", "delegation_end"),
    ]
    assert all("DELEGATION HOOK SUMMARY MUST NOT BECOME T0" not in event.content for event in events)
    assert events[-1].metadata["delegation_id"] == "handoff-1"
    assert not list((tmp_path / str(agent_id) / "logs").glob("**/*.md"))
...
async def test_heartbeat_and_dream_write_t0_session_ledger_not_legacy_logs(monkeypatch, tmp_path) -> None:
    ...
    await _t0_heartbeat_tick_end(
        HookContext(
            ...
            source="heartbeat",
            messages=[{"role": "assistant", "content": "DISTILLER CONVERSATION MUST NOT ENTER AGENT T0"}],
    ...
    assert all("DISTILLER CONVERSATION MUST NOT ENTER AGENT T0" not in event.content for event in heartbeat_events)
