...
- Task/live Green：`ChatWorkLedgerDock` 在 `pending/queued/running/waiting` 常驻 Composer 上方并显示全量 authorized ledger，terminal 保留摘要且用原生 `<details>` 折叠；成功 `task_create/update/stop` 进入 tool history，failed/blocked 继续 surface。canonical task call、committed tool result、runtime kind、terminal run 使 Session/Runtime/Work-Ledger query 失效；live 时另有 5 秒 polling 作恢复兜底，不作为唯一更新机制。
...
- final local tests/build：backend commentary substrate full=`7577 passed, 2 skipped`；final frontend Session focused=`7 files / 61 tests`，full=`120 files / 726 tests`；production `tsc + vite build + bundle budget` 通过，AgentDetail=`338415/380000` bytes、gzip=`93254/115000`，vendor=`591449/620000`、gzip=`186474/200000`。architecture ledger 既有=`11 passed`，diff check 全绿；final consumer commit、三服务 exact-source 与 authenticated no-refresh/hard-reload ordering canary 尚待写回。
...
- status：`in_progress-local-green-final-consumer`；必须在 final consumer exact source 上看到真实模型调用、无需刷新显示 exact commentary、raw tool 不出现、Task/final 继续 live，且 hard reload 后同一 message 顺序稳定、同 run final 唯一，才能关闭 `SES-CONSUMER-001`。
