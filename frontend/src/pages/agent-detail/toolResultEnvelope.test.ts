...
describe('parseCreateEmployeeToolResult', () => {
  ...
  it('parses preview blueprint payloads into structured preview metadata', () => {
    const parsed = parsePreviewAgentBlueprintResult(JSON.stringify({
      ...
      blueprint: {
        ...
        core_outputs: ['Competitor brief'],
        boundaries: 'Never fabricate sources.',
        permission_scope: 'company',
        source_attributions: [{ field: 'mission', source_type: 'confirmed_by_user' }],
      ...
      knowledge_debt: ['Company KB is unavailable.'],
      confirmation_requirements: ['Confirm the exact canonical blueprint.'],
    ...
    expect(parsed).toEqual({
      ...
      missingGates: [],
      knowledgeDebt: ['Company KB is unavailable.'],
      confirmationRequirements: ['Confirm the exact canonical blueprint.'],
  ...
  it('normalizes preview blueprint payloads into a structured tool card payload', () => {
    const normalized = normalizeToolCallResult(
      ...
      JSON.stringify({
        status: 'preview',
        blueprint_id: 'draft-2',
        blueprint_version: 1,
        blueprint_hash: 'sha256:preview-2',
        draft_status: 'awaiting_confirmation',
        blueprint: {
          name: 'Research Bot',
          primary_users: ['Research team'],
          core_outputs: ['Landscape brief'],
    ...
    expect(normalized.toolMeta).toEqual({
      ...
      blueprintId: 'draft-2',
      blueprintVersion: 1,
      blueprintHash: 'sha256:preview-2',
      status: 'awaiting_confirmation',
      name: 'Research Bot',
      mission: 'Research competitors and write briefs.',
      firstMission: 'Create the first competitor landscape brief.',
