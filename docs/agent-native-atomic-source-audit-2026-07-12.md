...
- 终局 legacy quarantine 补正（2026-07-12）：边界复核发现已保留的只读导出仍在 async API 内同步读取并把全部源文件 bytes 留在内存，status 查询又把 loading、403、503/网络失败与真实空目录折叠为同一“无卡片”结果。修复不开发 Company KB 本体：status/ZIP 构建与分块读取统一进入 AnyIO worker；源文件按 1 MiB chunk 做 SHA-256，ZIP 使用磁盘临时文件，不再建立全量 `file_payloads`；scan receipt 绑定 device/inode/size/mtime/hash，根目录或子文件 symlink、同内容 inode 替换、扫描期间变化一律 fail-closed 为 409，权限/IO/临时盘失败稳定映射 503 且不泄漏宿主路径；取消、审计提交失败或客户端提前关闭流都会关闭源文件与临时归档。12 MiB fixture 的 Python heap 峰值由 Red `12,905,033` bytes 降至 Green `2,381,764` bytes。管理员 UI 现在明确呈现 loading、403、unavailable、已验证 empty 与可用 export，并由 Retry 重新读取权威状态；Personal KB/Company KB/retired files 的源码说明也不再混称。Red 为 backend `5 failed`、frontend `4 failed`；Green R-008 定向 backend **`18 passed`**，扩展 Company/HR/prompt 边界 **`76 passed`**，frontend **`4 files / 35 passed`**，生产 build、ruff/format 与 diff-check 全绿。独立提交主题：`fix(R-008): harden legacy quarantine export`。
...
- 终局 strict-RLS 验收补正（2026-07-12）：在全新 PostgreSQL database、schema owner=`example-owner`、runtime=`app_rls` 的真实启动中，J-01 先后机械暴露四个此前 fixture 未覆盖的 bootstrap seam：第二个公开注册用户在 BYPASS 退出后写 tenantless `users` 返回 500；tenantless 签名身份在 Join 前以空 scope 查询返回 401；Join 与 Self-create 在 pin 目标 tenant 后更新旧 NULL-tenant User 时被 RLS `USING` 拒绝并返回 500。同时启动日志证明 default Tenant 查询不可见后重复 INSERT、global builtin Skill INSERT、`server_startup` NULL-tenant AuditLog INSERT 均被 strict RLS 拒绝。修复没有放宽任何 policy：注册的 User+Participant 留在既有受审计 scope 内；tenantless token 只按签名 `sub` 做单行 audited lookup；Join/Self-create 在已验证 invitation/new-tenant authority 内先原子完成 NULL→tenant transition，再 pin 普通消费 scope；default tenant seed 改为单条 `INSERT ... ON CONFLICT DO NOTHING RETURNING`，两个真实 `app_rls` session 竞争 exact-one 且均无异常；global Skill 与 operator audit 各自进入静态 allowlist 约束的唯一 bootstrap owner。Red 证据为 auth scope `1 failed`、真实 PG matrix `4 failed`、双 session race `1 failed`、tenantless identity/Join/Self-create 各 `1 failed`；Green 合并定向 **`35 passed`**，RLS bypass exact-shape gate 全绿，变更文件 ruff/format 全绿。最终在另一全新 database `hive_atomic_terminal_0712_200321`、独立 Redis DB 10 重跑 `npm run test:e2e:journeys` → **`15 passed (43.7s)`**；owner 侧事实核对为 default tenant=`1`、global builtin Skills=`9`、NULL-tenant `server_startup` audit=`1`、users/user Participants=`3/3`。本补正不新增 R 编号、不改变 28 项统计；独立提交主题：`fix(R-011): close strict-RLS bootstrap journeys`。
...
| Railway生产门 | **上一候选三服务通过；本次候选待部署** | `ac19ee17b` 的 backend `000001b6…`、backend-api `00000203…`、frontend `00000029…` 均 `SUCCESS`；P-002/P-003 的 Sidebar 与权限默认模式修复尚未部署，不能沿用上一候选的部署证据冒充当前 checkout 已上线。 |
...
| R-008 | 边界闭环（Company KB 本体已知缺失） | `fix(R-008): isolate the missing Company KB boundary` + `fix(R-008): harden legacy quarantine export` | 首轮关闭假 route/consumer 与 typed legacy quarantine；终局 Red backend 5 + frontend 4，Green backend 18/扩展 76、frontend 35 + build；worker offload、2.38 MiB heap peak、inode/hash race fence、symlink/IO/cancel/409/503、loading/403/empty/Retry；ruff/format/diff 绿 |
...
| R-011 | 闭环 | `fix(R-011): gate fifteen real user journeys` + 3 个 acceptance 补正 | Red architecture 2 failed + CI collection Red 1 failed；真实全链 15 passed；终局 strict-RLS 补正关闭 public registration/tenantless identity/Join/Self-create/default tenant/global Skill/operator audit 六条 bootstrap authority seam，定向 35 passed，fresh DB 全链 15 passed；backend 全量 6562 passed；frontend 639 passed + build；CI 独立收集与永久证据上传；ruff/format/diff 绿 |
