...
> 状态：设计权威；Group 2/3/4 substrate 已分别形成并部署，但 2026-07-17 live-entry/path 复核重新打开 `P1-004`、`SES-CONSUMER-001`、`A2A-TERMINAL-001`、`TEAM-FANOUT-001`、`ROOT-TREE-001` 五个回归 leaf。`EVID-G2-016`–`019` 已证明 Task 专用消费、公开 commentary 持久化/reload、rolling render ownership、Peer A2A read-only、runtime forwarder、suffix/tail 与 typed live-tail。terminal-fold commit `d60a669bd` 的 canary 证明 successful Run 默认收起、final 独立、展开恢复均成立。private-continuity commit `37599d2ca` 又由三服务发布并在 authenticated canary L 证明 private sequence 无正文、terminal 默认 progress/milestone=0、final=1、展开恢复、折回与 hard reload 等价；同一 canary 继续证伪 schema V1 runtime row 共用 Session sequence 却没有 live outbox，导致 active commentary 仍等 REST/terminal。`EVID-G2-020` follow-up 已在同一事务补齐 V1 user-safe compatibility outbox，并让 claim/publish/forward 接受 canonical/compatibility 两类 envelope。最终产品合同保持：运行中公开 commentary/prose 实时展开；成功完成后与 Thinking、工具、compaction 一起进入同一个可恢复的 `Processed` disclosure，默认只留 final；Task ledger 独立固定；整体 failed/blocked 继续自动展开。新 source 发布及 active no-refresh canary 前 `SES-CONSUMER-001` 仍为 `in_progress-local-green-v1-outbox`。完整 Session V2 继续由 Group 6/7/8/9/10 验收。
>
