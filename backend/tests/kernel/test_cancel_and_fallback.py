...
async def test_agent_kernel_uses_fallback_for_provider_overloaded_529():
    ...
    primary_client = _SwitchableClient(error=LLMError("HTTP 529: overloaded"))
    fallback_client = _SwitchableClient(
        responses=[
            SimpleNamespace(
...
@pytest.mark.parametrize(
    ...
    [
        (
            'HTTP 403: {"error":{"message":"Your token-plan quota has been exhausted."}}',
            "AI 模型额度已耗尽",
        ),
        (
            "HTTP 429: Too Many Requests: 您的账户已达到速率限制，请您控制请求频率",
            "AI 模型服务方已限流",
        ),
        (
...
async def test_agent_kernel_does_not_fallback_for_provider_account_errors(error, expected):
    ...
    primary_client = _SwitchableClient(error=LLMError(error))
    fallback_client = _SwitchableClient(
        responses=[
            SimpleNamespace(
                content="fallback success",
