...
- `ccplus-session-native-closure-gap-ledger-2026-06-25.md` — 当前最重要的 Session 全闭环差距总账：把 CC 在 Session 内的完整体系拆成 accepted input、Hook、permission、tool、artifact、Deep Research、Workflow、Sub-agent / Team、Goal / Once / Schedule、Memory / Evolution、composer、timeline、inspector、export；逐项标出当前 Hive 代码事实、缺口、Codex 可叠加的 typed invocation / replay / inspector / session export 优化，以及验收标准。
- `ccplus-session-permission-and-enterprise-hard-rules-2026-06-25.md` — 当前生产修复裁决与 session-native 边界账本：CCPlus session 权限菜单只暴露 `default/auto/bypassPermissions`，新 session 默认 `auto`；`dontAsk`、Plan Mode 与 `acceptEdits` 不进入权限菜单；企业第一硬规则是员工不能删除 Agent；当前 Hook 目标是一次性建出完整 CC 27-event contract，并叠加 Codex 风格 typed invocation / replay / inspector / session export；企业审批 / 治理配置留在控制面，但会话触发的权限、Hook 决策、Deep Research / Workflow / Sub-agent / 定时任务执行、文档产物和继续执行点必须投影回 Session/T0；tenant-wide enterprise Hook policy / assignment / rule DSL 延后到 CC Hook substrate 之后讨论。
- `ccplus-session-ux-contract-2026-06-26.md` — CCPlus Session UX 契约：把 CC 能力内核、Codex 式 session 信息分层、Hive 企业治理边界收成一个体验准绳；规定用户默认看到 Agent 工作判断而非工具流水账，Plan Mode 必须是“观察事实 + 关键判断 + 执行范围 + 验证方式 + 风险”的执行前提案，权限菜单只用请求批准 / 替我批准 / 完全访问，最终交付物必须作为 session 内 artifact 卡片出现并在右侧 inspector 预览。
