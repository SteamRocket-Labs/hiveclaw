...
| HR `create_digital_employee`(内部直写 first mission/boot trigger) | 半 | 新 agent 即带自主 wake |
...
```text
早拦层(体验:尽早提示用户去确认计划)
  tool(tagged):set_trigger / update_trigger / delegate_to_agent / manage_tasks / create_digital_employee / deep_research_start
...
create_digital_employee           # first mission / boot trigger
deep_research_start               # 已有 plan_confirmed(RC11-RC15 刚修好);MVP 只桥接登记 PlanRequest,不重构
```

**不要**把 `send_feishu_message` / `write_file` / `edit_file` / `delete_file` 放进第一批强拦——当前轮明确授权的低风险同步动作不该进 Plan Mode,它们继续走现有 ActionPreflight / capability approval。只有"未来自动发送、批量覆盖、改生产配置"这类才升级到 plan。
...
2. **早拦层**:
   - tool gate(代码级 ToolMeta 打标):`set_trigger`/`update_trigger`/`delegate_to_agent`/`manage_tasks`/`create_digital_employee`/`deep_research_start`,在 `execute`/`execute_direct`/`execute_approved` 全入口生效。
