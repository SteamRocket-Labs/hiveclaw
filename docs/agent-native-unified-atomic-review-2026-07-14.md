...
| 2 | `EVID-G2-*` | 14 leaf / 0 Missing | `in_progress`：13/14 closed；`EVID-G2-016/017` 已证明 Task、commentary persistence/replay 与 A2A read-only；`EVID-G2-018` 已证明 live bridge；`EVID-G2-019` 的 typed live-tail 已由 `a1be2dfbe` 三服务发布，canary G 证明 Task/final/commentary 无刷新到达并把最后缺口收敛为 terminal Run 错折叠公开 commentary，presentation 已本地 Green | 发布 terminal commentary source；真实模型必须在 completed Run 折叠状态仍显示 progress/milestone，raw task/progress tool 不出现，Task/final live，hard reload 后顺序和 render ownership 等价；不得用手动展开、fixture、REST reload 或 deployment success 代替默认产品路径 proof |
...
- CC/Codex/产品裁决：FreeCode 成功 Todo/Task mutation 不重复渲染 result row，Codex `PlanUpdateCell` 把计划更新投到 typed checklist；Hive 因此保持 successful `track_todo/task_create/task_update/task_stop` 只进入 Composer Task ledger，失败/blocked/cancelled 才回主时间线。普通成功工具在 active Run 展开、terminal Run 折叠；model-authored public progress/final、approval/question、typed failure/recovery 与 deliverable 始终是用户可见事实。private CoT、raw ACK/JSON/内部 ID 与重复 receipt 不进入产品时间线。
...
context_read_receipt:
  ...
  source_baselines:
    hive_head: "a1be2dfbe6ef + owned terminal-commentary presentation diff; deployed bridge source=9ebb51f1bac1"
    freecode_head: "7dc15d6c8fb0c40c7fcc02ce9b58204324252632"
    codex_head: "5c19155cbd93bfa099016e7487259f61669823ff"
...
- local acceptance：baseline 首次 Red=`2 failed / 3 passed`；baseline Green focused Session disclosure/hydration=`6 files / 194 tests`；commit `70c4a12f2` 的 clean exact-source archive（含 `.github` harness contract）=`120 files / 730 tests`。tail-watermark follow-up Red 同样为 `2 failed / 3 passed`，Green focused transport/hydration=`6 files / 36 tests`；commit `d29f52493` clean exact-source frontend full=`120 files / 730 tests`。first-snapshot readiness Red=`2 failed / 12 passed`，Green focused=`2 files / 14 tests`、相关 Session suite=`4 files / 133 tests`、共享工作树 frontend full=`122 files / 739 tests`，commit `9ad195fc3` 已三服务发布。live-tail Red=backend collection error + frontend `2 failed / 11 passed`；Green backend Session/WebSocket=`43 passed`、frontend focused=`4 files / 27 tests`、当前共享工作树 frontend full=`122 files / 744 tests`，ruff/format 与 architecture line budget 通过。terminal commentary Red=`1 failed / 9 passed`，并反转一条把“成功 turn 全部折叠”钉成契约的旧测试；Green focused disclosure/reducer=`4 files / 155 tests`、frontend full=`122 files / 745 tests`。`npm run build` 通过，AgentDetail=`340062/380000` bytes、gzip=`93847/115000`，vendor=`591449/620000`、gzip=`186474/200000`；`git diff --check` 通过。
...
- status：`in_progress-local-green-terminal-commentary`。typed `live_tail` 已由 exact commit `a1be2dfbe` 三服务发布，canary G 已证明真实模型的公开 progress/milestone、Task 和 final 都无刷新到达；但两条公开 commentary 只有手动展开 completed Run 才可见。只有 terminal commentary source 三服务发布后，在默认折叠状态无需交互即可看见公开进度、Task/final 继续 live、raw task/progress tool 为零，且 hard reload 后顺序与字节等价，才能关闭 `SES-CONSUMER-001`。
