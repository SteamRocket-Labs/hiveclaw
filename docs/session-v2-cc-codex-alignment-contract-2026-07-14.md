...
> 状态：设计权威；Group 2/3/4 substrate 已分别形成并部署，但 2026-07-17 live-entry/path 复核重新打开 `P1-004`、`SES-CONSUMER-001`、`A2A-TERMINAL-001`、`TEAM-FANOUT-001`、`ROOT-TREE-001` 五个回归 leaf。`EVID-G2-016` 已随 commit `11001f133` 三服务同源部署，Task 实时/折叠/reload 与 Peer A2A read-only browser canary 已通过。`report_progress` follow-up 已由 commit `a456932ec` 三服务同源部署，production DB 与 authenticated browser 证明 exact `assistant_commentary` 可持久化、reload 后展开 completed Run 可见；该 canary 同时暴露 rolling compatibility reducer 会把旧消息拖到最新 sequence，并与 canonical final 重复渲染。§28.5 / `EVID-G2-017` 已用 first-materialization sequence、typed `run_id` render ownership、Task 专用消费和 exact Session store 转绿；2026-07-18 production health 又坐实 runtime 进程未启动 canonical Redis→process-local WebSocket bridge（outbox published 93、forwarded 0）。`EVID-G2-018` 已本地恢复 socket-role bridge 并锁定 initial hydration exact-Session route；三服务发布和 no-refresh/hard-reload canary 前 `SES-CONSUMER-001` 仍为 `in_progress-local-green-live-bridge`。完整 Session V2 继续由 Group 6/7/8/9/10 验收。
>
...
| Frontend typed projection | `in_progress-local-green-live-bridge:EVID-G2-016/017/018` | 三类协作、Composer Task、query invalidation、final artifact 与 read-only A2A 已部署/canary；原生 commentary 已证明 persist/reload/render，rolling sequence/render owner 与 exact Session store 已 Green；production runtime 缺 canonical Redis→WebSocket bridge 的根因已本地修复，待 no-refresh + hard reload 终验 |
...
| 自动化黄金轨迹验收 | 既有 Group 4 保持；`EVID-G2-016/017` 保持，`018` local Green | `018`：backend startup/stream/health/ingress/ledger=`46 passed`；frontend focused=`20`、full=`120 files / 728 tests`、build/budget 绿；仓级 backend 与 production live canary 待写回 |
| Railway 行为修复与生产验收 | `EVID-G2-016/017` 已提供 durable/reload proof；`018` 待发布 | pre-fix production health=`process_role:runtime`、outbox `93/93 published`、forwarder `running=false/forwarded=0`，证明 bridge 未接线；新 source 需三服务同源发布并用 authenticated no-refresh/hard-reload canary 关闭 |
