...
| B-02 | 定义 `ActivationCandidate` / score / hard mask schema | `backend/app/runtime/activation_candidates.py` | `tests/runtime/test_activation_candidates.py` | 已完成：本提交；Red `pytest tests/runtime/test_activation_candidates.py -q` -> 3 failed / ModuleNotFoundError；Green -> 3 passed, 4 third-party warnings；Lint `ruff check app/runtime/activation_candidates.py tests/runtime/test_activation_candidates.py` -> All checks passed |
...
| B-04 | 扩展 `RuntimeAssemblyState` 以承载 Q/Candidates/Router output | `backend/app/runtime/context.py` | `tests/runtime/test_runtime_context_composition.py` | 待执行 |
| C-01 | 在 `USER_PROMPT_SUBMIT` 后构造 Q | `runtime/invoker.py::invoke_agent` | `tests/runtime/test_invoker.py` | 待执行 |
| C-02 | `ActivationQuery` 引用 `turn_id` / `intent_id` | `runtime/invoker.py::_ensure_turn_metadata` | `tests/runtime/test_activation_query.py` | 待执行 |
| C-03 | 接入 `TaskProfile` 为 Q 子结构 | `runtime/context_budget.py`、`activation_query.py` | `tests/runtime/test_context_budget.py` | 待执行 |
| C-04 | 增加机械 Q parser | `activation_query.py` | URL、文件名、时间词、风险词测试 | 待执行 |
| C-05 | 增加条件 LLM Q parser 与降级 | `activation_query.py`、LLM client seam | 超时/失败 fallback 测试 | 待执行 |
| D-01 | T0 事件补齐 runtime metadata discipline | `services/t0_logger.py`、T0 writers | T0 event schema tests | 待执行 |
| D-02 | T2 labels prompt 输出 `<activation_keys>` | `memory/t2/prompts.py` | labels prompt contract test | 待执行 |
| D-03 | T2 segment package 校验 activation keys | `memory/t2/segment_package.py` | invalid/legacy package tests | 待执行 |
| D-04 | T2 manifest 写入 activation key preview | `memory/t2/segment_package.py` | manifest rebuild tests | 待执行 |
| D-05 | T3 profile/knowledge 解析 aliases/tags/lifecycle | `memory/plane_read.py` | T3 parsing tests | 待执行 |
| D-06 | explicit overlay 产生 candidate keys | `memory/explicit_overlay.py` | explicit key projection tests | 待执行 |
| D-07 | Skill/Tool/Subagent 元数据产生 candidate keys | `skills/*`、`services/agent_tools.py`、`subagent_listing.py` | key projection tests | 待执行 |
| E-01 | `reference_index` 新增 `activation_keys` 派生表 | `memory/reference_index.py` | migration-free rebuild tests | 待执行 |
| E-02 | 旧 T2 机械回填 activation keys | `reference_index.py` | legacy T2 fixture tests | 待执行 |
| E-03 | 旧 T3 机械回填 activation keys | `reference_index.py`、`plane_read.py` | legacy T3 fixture tests | 待执行 |
| E-04 | 提供 `query_activation_keys()` / `candidate_refs_for_keys()` | `reference_index.py` | query API tests | 待执行 |
| E-05 | 删除派生 index 后全量重建验证 | rebuild script / tests | rebuild smoke + source_refs test | 待执行 |
| F-01 | Memory gatherer 输出 `agent_memory` candidates | `memory/retriever.py` | memory candidate tests | 待执行 |
| F-02 | T2 gatherer 输出 evidence candidates | `reference_index.py`、`retriever.py` | T2 candidate tests | 待执行 |
| F-03 | T3 gatherer 输出 profile/knowledge candidates | `plane_read.py`、`retriever.py` | T3 candidate tests | 待执行 |
| F-04 | explicit overlay gatherer | `explicit_overlay.py`、`retriever.py` | explicit candidate tests | 待执行 |
| F-05 | Skill gatherer | `services/skill_catalog_ranker.py` | skill candidate tests | 待执行 |
| F-06 | Tool / deferred tool gatherer | `services/agent_tools.py` | tool candidate tests | 待执行 |
| F-07 | Subagent gatherer | `runtime/prompt_sections/subagent_listing.py` | subagent candidate tests | 待执行 |
| F-08 | Personal / Company KB gatherer 接口 | `runtime/retrieval` / KB service seam | ACL + empty implementation tests | 待执行 |
| G-01 | Router hard mask：ACL / sensitivity / policy | `activation_router.py` | deny/suppress tests | 待执行 |
| G-02 | Router multi-head score | `activation_router.py` | semantic/entity/temporal/profile tests | 待执行 |
| G-03 | Router budget-aware top-k | `activation_router.py`、`context_usage` | budget pressure tests | 待执行 |
| G-04 | Router suppression reasons | `activation_router.py` | suppressed reason manifest tests | 待执行 |
| G-05 | Router output 写入 `RuntimeAssemblyState` | `runtime/context.py`、`kernel/engine.py` | state mirror tests | 待执行 |
| H-01 | Memory ValueLoader 只加载入选 V | `memory/plane_read.py`、`memory/assembler.py` | no eager body load tests | 待执行 |
| H-02 | Skill / Tool / Subagent hint 渲染 | `prompt_sections/activation_hints.py` | prompt section tests | 待执行 |
| H-03 | dynamic suffix 接入 activation hints | `runtime/prompt_builder.py` | prompt builder tests | 待执行 |
| H-04 | manifest 写 Q/K/V trace | `runtime/turn_envelope.py` | manifest tests | 待执行 |
| H-05 | ContextUsageLedger 归账 selected V | `turn_envelope.py`、`context_usage` | context usage tests | 待执行 |
| I-01 | Tool success/failure 写 `ActivationEvent` | `kernel/engine.py::_execute_tool_with_hooks` | POST_TOOL_USE/FAILURE tests | 待执行 |
