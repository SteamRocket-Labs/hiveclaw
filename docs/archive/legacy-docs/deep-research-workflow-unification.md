...
| **DR-6 旧路径退役** ✅ 完成（2026-06-05，用户拍板"无论如何退役"后实跑 gate 改为切换后验证） | I1 兑现 | **6a 搬家（零行为变化，独立 commit）**：`synthesis_gates.py` 单一正源（RC12/RC13/footnotes/envelope/coverage/密度计数 + digest instruction + claim 压缩 + `_depth_expectation`，逐字搬移）；plan_mode 内嵌 `_worker_topics` 族；worker 的 source 精炼函数搬进 leaf_presets。**6b 删除**：`orchestrator.py`（1263 行）`controller.py` `reflector.py` `worker.py` `searcher.py` `reader.py` `writer.py` `reasoner.py` 八文件整删；`WORKFLOW_DEEP_RESEARCH_ENABLED` flag + `deep_research_workflow_enabled()` 删；handler 单路径化 — `deep_research_run` 同步驱动 workflow、`deep_research_start`/plan-handoff `start_deep_research_background_run` 预生成 run_id + **后台调度 workflow**（保真旧异步契约：立即返回可查 id + 防轮询指引原文保留；新 `_schedule_deep_research_workflow_background` runner 只注记结果与清 in-flight，RuntimeTask 行/状态归 workflow runtime 所有）；`RuntimeTask(task_type="deep_research")` 创建点零残留（历史 task 经 check 旧分支仍可读）；`__init__` 导出 workflow-native 面；修掉 dedup id 形态不一致（hex vs str）真 bug；definition `per_item_task` 补 `{{args.output_language}}` 语言行（旧 worker prompt 语言锁的等价承载） | ✅ 旧路由测试 8 文件整删（orchestrator/orchestrator_v2/controller/worker/reflector/searcher/reasoner/synthesis_quality）；保价值改写：plan_gate 守卫面换 workflow 启动、citation/devils_advocate/extractor_writer/plan_contract/source_grading 的退役整链 case 删（等价锚由 DR-2/3/5b 测试 + `test_workflow_args_use_confirmed_worker_topics` + `test_explorer_surface_pins_integration_contract_and_language_rides_args` 承载）；handler 测试改写为单路径契约（同步 run 透传 / start 后台调度 + 异步 id / full-depth 路由守卫） | ✅ 全量 **3750 passed, 7 skipped**；ruff clean；**验收 grep 零残留**：`deep_research_workflow_enabled`/`WORKFLOW_DEEP_RESEARCH_ENABLED`/`task_type="deep_research"` 创建点均为零。`deep_research_run/start` 唯一路径 = workflow；runbook flag 段同步退役 |
