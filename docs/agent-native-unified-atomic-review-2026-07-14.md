...
- final local tests/build：首次 follow-up 新增/相邻 backend=`187 passed`、real-PG/contract focused=`9 passed`；补入 rolling-recovery Red 后 final focused=`6 passed`、相邻整组=`167 passed`，仓级 full=`7577 passed, 2 skipped in 375.17s`；scoped Ruff check/format 与 diff check 全绿。frontend replay/disclosure 首次 focused=`5 files / 52 tests`，最终相关=`3 files / 43 tests`，full=`120 files / 724 tests`；production build/budget 通过，AgentDetail=`337666/380000` bytes、gzip=`93006/115000`，vendor=`591449/620000`、gzip=`186474/200000`。architecture ledger=`11 passed`；follow-up commit、三服务 exact-source 与 authenticated live/reload canary 尚待写回。
...
- status：`in_progress-local-green-followup`；必须在新 exact source 上看到真实模型调用、无需刷新显示 exact commentary、raw tool 不出现、Task/final 继续 live，且 hard reload 后同一 message 仍在，才能关闭 `SES-CONSUMER-001`。
