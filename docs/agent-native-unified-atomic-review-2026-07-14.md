...
>
> 修复账本滚动更新：2026-07-17；Group 0 与 Group 4 的既有证据门保持关闭。对当前 checkout 和线上 A2A 故障截图做 wiring/path 复核后，旧的 43/103 完成声明被主动撤销：`P1-004`、`SES-CONSUMER-001`、`A2A-TERMINAL-001`、`TEAM-FANOUT-001`、`ROOT-TREE-001` 五个 leaf 因 server-side read-only 未执行、Peer A2A 仍被并入 Sub-agent、terminal outcome 未关闭 root item、Team member model 未进入 worker、Team member implementation Session 暴露到普通列表而重开。五个回归的代码、迁移、typed consumer 与测试已由 commit `b9852f37f` 进入 production；backend=`00000325-0000-4000-8000-000000000000`、backend-api=`00000344-0000-4000-8000-000000000000`、frontend=`00000147-0000-4000-8000-000000000000` 均 `SUCCESS`。production migration actual/expected head 均为 `collaboration_runtime_closure_0717`，148-table/4-trigger readiness `issues=[]/ready=true`，backend health=`ok`、RLS=`app_rls/strict/non-superuser/non-BYPASSRLS`、三个 daemon 与 sandbox healthy，frontend HTTP 200。首次 backend-api deployment `00000133-0000-4000-8000-000000000000` 在 writer migration 前按设计 fail-closed；schema ready 后从同一 commit archive 重提成功，没有放宽 readiness 或改写业务数据。由于 authenticated deny/read、真实 Team model route、terminal/root reconciliation 与三类协作 browser canary 尚未执行，五个 leaf 当前统一记为 `in_progress-deployed-pending-canary`，业务分母仍为 **38/103 closed**，不是 43/103；本轮未新增 canonical ID，也未把“部署成功”冒充行为验收完成。Group 4 exact-source commit=`4e385d423` 的既有 return-storm/recovery 证据仍记录在 `EVID-G4-*`。2026-07-15T13:37Z 因 production `backend-volume` 在重启批次中从约 24.8 GB 急升到 28.65 GB，Group 1 曾显式暂停；`EVID-G8-PRE-001/002/003` 已分别关闭继续写放大、transaction lifecycle 与当次容量事故处置，核心数据停止门成立。B-01 发布后同挂载点 `df -B1 /data/agents` 为 used=`11,360,583,680` bytes（24%），未执行新的清理或核心数据删除。三个 Group 8 前置证据仍不关闭任何 Group 8 leaf 或 `MISS-RETENTION-001`；Object Storage、snapshot CAS、sealed T0 archive、T2 authority/replay 与跨资产 retention 仍未完成。
>
> 最新 Session live presentation 复核继续重开同一个 `SES-CONSUMER-001`，不增加分母：`EVID-G2-016` 已由 commit `11001f133` 同源发布，backend=`00000071-0000-4000-8000-000000000000`、backend-api=`00000358-0000-4000-8000-000000000000`、frontend=`000003cf-0000-4000-8000-000000000000` 均 `SUCCESS`。authenticated browser canary 已证明 Task 无刷新实时变化、live final、terminal 原生折叠、reload canonical replay 和 Peer A2A read-only window；但 MiniMax M3 在“先公开进度”强指令下仍只产生 provider-private reasoning 后直接调用工具，证明无 commentary Provider 还缺模型可用的公开表达通道。第一版 `report_progress` commit `69b4127b7` 已三服务发布并证明模型会调用该工具；同一 canary 又发现实时 raw tool 可见、hard reload 后 public message 消失，因为新 Session V2 只把参数保存在 `SessionToolInvocation`，canonical `tool_call` 仅携带 hash。`EVID-G2-017` follow-up 已在 pre-effect 同事务生成原生 `assistant_commentary` 并补 V1 rolling-read 兼容；新 source 发布和 live/reload canary 前状态仍为 `in_progress-deployed-canary-partial`，38/103 分母不变。
>
...
>
> `EVID-G2-018` 随 commit `9ebb51f1b` 三服务发布后，production health 已证明 runtime forwarder `running=true/forwarded=111/last_sequence=2340/last_error=null`；同页 canary 也无刷新得到 Task add/start/complete、final、terminal folding，且 raw `track_todo/report_progress` row 为零。该 canary 仍证伪同一 `SES-CONSUMER-001` 的 newest-page hydration：production PostgreSQL 已按 sequence `2192` 持久化 accepted input、`2239/2289` 持久化两条 exact `assistant_commentary`、`2337` 持久化 final，但浏览器只显示 Task/final。`EVID-G2-019` 先把首个 newest-page suffix 的 projection baseline 从固定 0 修成“首个已取 sequence - 1”，并由 commit `70c4a12f2` 发布；tail-watermark follow-up `d29f52493` 与 first-snapshot `liveReady` commit `9ad195fc3` 也已分别三服务发布且均 `SUCCESS`。连续 canary 依次坐实：完整旧历史 promise 不能充当 live admission、首个 transport waiter 不能在 store 建立前复用 full completion、而首个 transcript REST page 本身返回 `HTTP 504` 时仍完全没有安全 cursor。最终修复增加 typed `cursor_mode=live_tail`：服务端先注册 buffer、捕获 durable watermark `W`、返回 `accepted_after_sequence=W`，客户端从 `W+1` 接收 live event 并以 `W` 建立空基线；fence 前的 canonical evidence 继续由 REST 后台恢复，不能被解释成空历史或丢弃。新 source 发布与无刷新/reload 等价 canary 前状态为 `in_progress-local-green-live-tail`，业务分母仍为 38/103。
>
> terminal-fold commit `d60a669bd` 三服务发布后的 canary 已证明“成功完成后默认收起过程、final 独立、展开恢复”本身成立；但同一未刷新页面的运行中 commentary 仍会停住。`EVID-G2-020` 先用生产 DB 与 WebSocket 帧证明 bridge 丢弃 `private_provider/operator` canonical event 会打断 sequence；commit `37599d2ca` 随后由 backend=`00000319-0000-4000-8000-000000000000`、backend-api=`00000413-0000-4000-8000-000000000000`、frontend=`000000be-0000-4000-8000-000000000000` 发布且均 `SUCCESS`。authenticated canary L 证明 private event 已带原 sequence 且无 private content，successful terminal 默认 progress/milestone=0、final=1，展开后 progress/milestone=1、raw tool=0，折回与 hard reload 均等价；但 running 时 Task 已 live、public progress 仍在 REST/terminal 前不可见。进一步的 DB/WS 对账发现同一连续序号里仍混有没有 outbox 的 V1 runtime row（如 `model_route/context_window_status/provider_call_ledger`），它们占用 sequence 却不 live publish，继续合法阻塞后续 V2 commentary。当前 follow-up 让每个 V1 append 在同事务生成 user-safe compatibility outbox，并让 publisher/forwarder/claim validation 接受同一 sequence plane；新 source 发布与 authenticated running-time canary 前状态为 `in_progress-local-green-v1-outbox`，不增加 canonical leaf。
>
> V1 continuity commit `5e6d47b06` 已由 backend=`000002cf-0000-4000-8000-000000000000`、backend-api=`00000440-0000-4000-8000-000000000000`、frontend=`000003c9-0000-4000-8000-000000000000` 三服务发布且均 `SUCCESS`。authenticated canary M 首次在未刷新 active Run 中同时看到 exact progress、Task 和 milestone，证明 event/outbox/bridge/contiguous reducer 已接通；但 terminal 到达后 final 已出现、过程已从正文隐藏，当前 Run 的 disclosure 仍停在 `处理中`，只有 hard reload 才出现新的 collapsed `已处理`。第一轮 `EVID-G2-021` 用 suffix baseline replay 修正该症状，并随 commit `165682a20` 三服务发布；authenticated canary N 随即证明 durable final/run/turn/outcome 已完整到达，但当前 Run 的 progress/milestone 仍没有进入新的 `已处理`。最终修正不再在 terminal 重建全历史：保留 latest user 后的 live process，仅从 exact `run_id` canonical store 合并非重复过程与唯一 `assistant_final` render owner；完整历史恢复继续独立运行。commit `ee95cd98d` 已由 backend=`0000034d-0000-4000-8000-000000000000`、backend-api=`000002fd-0000-4000-8000-000000000000`、frontend=`0000024b-0000-4000-8000-000000000000` 三服务发布且均 `SUCCESS`。authenticated canary P 在同一未刷新 Run 中先显示 `处理中` 且 exact progress/milestone/Task 各 1，成功后自动形成 collapsed `已处理（7 个步骤）`、正文只留唯一 final；展开/折回与 hard reload 均保持字节和可见性等价，raw task/progress row 为零。`SES-CONSUMER-001` 因此由 `EVID-G2-021` 关闭。
>
...
- 当前本机 convenience root：`${LOCAL_HOME}/vc-saas/hive-connect`（不进入 `@` 路由，不作为 CI 必需路径）
...
`claude-code-org` 的 `${LOCAL_HOME}/Context Engineering/claude-code-org/src/tools/AgentTool/**`、`${LOCAL_HOME}/Context Engineering/claude-code-org/src/utils/forkedAgent.ts`、`${LOCAL_HOME}/Context Engineering/claude-code-org/src/utils/sessionStorage.ts` 对 fork/resume/background/transcript 语义给出同向交叉证据。claw-code Python port 只用于识别现有移植边界；其中按固定 turn 数保留尾部的简化实现不能反向定义 CC 语义。claw-code Rust 的 JSONL session、fork、resume、compact 与 health-probe 只作为低层 session hygiene 参考。发生冲突时仍以 FreeCode 为 CC semantic floor。
...
- `${LOCAL_HOME}/Context Engineering/codex/codex-rs/core/src/tools/handlers/agent_jobs.rs`
- `${LOCAL_HOME}/Context Engineering/codex/codex-rs/core/src/tools/handlers/agent_jobs/spawn_agents_on_csv.rs`
- `${LOCAL_HOME}/Context Engineering/codex/codex-rs/core/src/tools/handlers/agent_jobs/report_agent_job_result.rs`
- `${LOCAL_HOME}/Context Engineering/codex/codex-rs/protocol/src/protocol.rs`
...
| 2 | 14 | 0 | closed：14/14；Task/折叠/持久 replay、Redis→runtime bridge、suffix/tail/live-tail、private/V1 continuity 与 exact-run terminal merge 均已部署；canary P 证明同一 Run active→terminal→fold/expand/collapse→hard reload 全链 |
...
**当前回归状态（2026-07-18）**：14/14 closed。`SES-CONSUMER-001` 已依次关闭三类协作 typed consumer、公开 commentary 持久化、Task 专用消费、terminal folding、reload replay、Redis→socket bridge、newest suffix、typed live-tail admission、private/V1 continuity 和 exact-run terminal visible-process merge。`EVID-G2-021` 的第一轮 terminal suffix replay 随 `165682a20` 发布后被 canary N 证伪；最终 `ee95cd98d` 已三服务发布，canary P 证明同一未刷新 Run 的 active progress/Task/milestone、terminal 自动折叠、final-only、展开/折回及 hard-reload 等价。Group 2 不再保留本 leaf 的生产 gate。
...
- P1 | SES-CONSUMER-001 | closed:EVID-G2-016/017/018/019/020/021 | 三类协作、固定 Task、final artifact、Peer A2A read-only、canonical commentary persist/render、rolling ownership、exact Session store、live bridge、suffix/tail/live-tail、private/V1 continuity 与 terminal fold 已闭环；`ee95cd98d` 三服务发布，canary P 证明同一 Run active 实时、terminal 自动折叠只留 final、展开/折回与 hard reload 等价
...
| 2 | `EVID-G2-*` | 14 leaf / 0 Missing | `closed`：14/14；`EVID-G2-016/017` 证明 Task、commentary persistence/replay 与 A2A read-only；`018/019` 证明 bridge 与 typed live-tail；`020` 证明 private/V1 continuity；`021` 证明 exact-run visible-process terminal merge | `ee95cd98d` 三服务发布，canary P 已完成真实模型 no-refresh running、completed final-only、fold/expand/collapse 与 hard-reload 等价；后续 Group 9 只继续完整历史/V1 cleanup 与长时重连，不重开本 Group |
...
- commit / deploy / production canary：独立 P0 commit=`10b74360a`，随 source=`1b822eb766` 完成三服务部署：backend=`000003f1-0000-4000-8000-000000000000`、backend-api=`000001b2-0000-4000-8000-000000000000`、frontend=`00000277-0000-4000-8000-000000000000`，均 `SUCCESS`。生产容器通过同一 governed transport 得到：`https://example.com`=`200/559 bytes`；同源 redirect 最终 `https://httpbin.org/get`=`200/304 bytes`；direct metadata 与 redirect-to-metadata 均 `network_target_denied`；HTTPS→HTTP downgrade=`network_redirect_denied`。本地 fault matrix 继续覆盖不可安全注入 production 的 peer mismatch/DNS rebinding/compression bomb 等故障。
...
- commit / deploy / recovery：独立 code commit=`53287e716`。同一 Git archive 部署后 backend=`000004a7-0000-4000-8000-000000000000`、frontend=`0000040d-0000-4000-8000-000000000000` 均 `SUCCESS`；backend-api 首次 deployment=`000003b7-0000-4000-8000-000000000000` 在 backend migration 尚未完成时耗尽 restart 并被移除，证明 startup schema gate fail closed；待 schema ready 后从同一 archive 重试 `00000094-0000-4000-8000-000000000000` 为 `SUCCESS`。2026-07-15 当前 Railway latest 仍是上述三项，backend `/api/health`=200、RLS runtime role=`app_rls` strict、三个 daemon healthy、frontend=200。
...
- commit/deploy/health：code/evidence commit=`637a56395`。同一 commit 的 Git archive 部署 backend=`00000022-0000-4000-8000-000000000000`、frontend=`0000034e-0000-4000-8000-000000000000` 均 `SUCCESS`；backend-api 首次 deployment=`00000264-0000-4000-8000-000000000000` 在主 migration 前按 schema readiness fail closed 并耗尽 10 次 restart，schema ready 后从同一 commit 重提 `000000e3-0000-4000-8000-000000000000` 为 `SUCCESS`。最终 backend `/api/health`=`status=ok`，runtime role=`app_rls / strict / non-superuser / non-bypassrls`，evolution/trigger/workflow daemon 均 running+healthy；frontend=`HTTP/2 200`。该证据关闭时 Group 1 为 4/16 closed、3/16 deployed-but-open、9/16 pending；当前滚动状态只以 §9 与 §12.3 为准。103 分母、severity、owner 与 5 个 Missing 均不变，下一 Knowledge leaf 为 `KB-PROP-001`。
...
${REPO_ROOT}/backend/.venv/bin/pytest tests/services/test_knowledge_provenance.py tests/services/test_knowledge_provenance_repair.py tests/services/test_agent_tools_channel_delivery.py tests/services/test_chat_transcript.py tests/services/test_channel_delivery_outbox.py tests/agents/test_subagent.py tests/agents/test_subagent_spawn_tool.py -q
```

- production deploy/health：同一 commit archive 部署 backend=`00000417-0000-4000-8000-000000000000`、backend-api=`0000000d-0000-4000-8000-000000000000`、frontend=`000003b4-0000-4000-8000-000000000000`，三项最终均 `SUCCESS`。backend `/api/health`=`status=ok/version=1.7.0`，runtime role=`app_rls`、`strict`、non-superuser、non-BYPASSRLS，evolution/trigger/workflow daemon 均 healthy；frontend=`HTTP/2 200`。
...
- production deploy/freshness：同一 `3c1998607` archive 部署 backend=`000002ac-0000-4000-8000-000000000000`、backend-api=`00000248-0000-4000-8000-000000000000`、frontend=`000001d8-0000-4000-8000-000000000000`，三项最终均 `SUCCESS`。backend 首次启动完成 migration/schema readiness、RLS/tool audit 与既有任务恢复后通过 `/api/health`：`status=ok/version=1.7.0`，runtime role=`app_rls / strict / non-superuser / non-BYPASSRLS`，evolution/trigger/workflow daemon、RuntimeTask worker、runtime control bus 与 Vercel Sandbox deny-all probe 均健康；frontend=`HTTP/2 200`。`/metrics` 已存在 `runtime_budget_root_failures_total` 的 HELP/TYPE，即使零事件也可被监控发现。
...
- production deploy/freshness：精确 `b4d4446a4` Git archive（关键 migration/privacy 文件与本地 SHA-256 相同）部署 backend=`00000038-0000-4000-8000-000000000000`、frontend=`000002da-0000-4000-8000-000000000000`，均 `SUCCESS`。backend-api 首次 deployment=`0000025e-0000-4000-8000-000000000000` 在 runtime migration 前按 readiness 拒绝旧 head，不计完成；schema ready 后同一 archive 重提 `00000300-0000-4000-8000-000000000000` 为 `SUCCESS`。最终 backend `/api/health`=`status=ok`，runtime role=`app_rls / strict / non-superuser / non-BYPASSRLS`，三 daemon 与 sandbox deny-all probe 健康；frontend=`HTTP/2 200`。
...
- production deploy/freshness：exact `09c3823a0` Git archive 的 `policy.py` SHA-256=`a8300d3160a9ad3840c605df527795770548f467031e281e18511575b55b45a9`、`audit_logger.py`=`b602262d8614956bd30667c970b2a4e2f7f4aac36fcaacab312e7a190d1c3e0e` 与 live backend 容器一致。backend=`00000206-0000-4000-8000-000000000000`、backend-api=`000002b8-0000-4000-8000-000000000000`、frontend=`00000064-0000-4000-8000-000000000000` 均 `SUCCESS`；backend health=`status=ok`、schema head=`audit_evidence_immutability_0715`、runtime role=`app_rls/non-superuser/non-BYPASSRLS/strict`、三 daemon/RuntimeTask worker/sandbox probe 均健康，frontend=`HTTP/2 200`。
- production canary：第一次多行 `python -c` 被 Railway SSH 参数重组，在任何 import/DB call 前以 `Argument expected for -c`、exit `2` 结束，不计证据且没有写 row。随后通过 stdin 执行同一已部署代码，append-only event=`00000387-0000-4000-8000-000000000000`、marker=`AUDIT-TENANT-001-production-canary-1f791d93533940c489d725ec96c8f35c`；回读断言 scope=`platform_operator`、action=`platform_security.auth.login`、schema=`hive.platform_security_audit.v1`、tenant/user/agent 均 NULL、actor/request_id byte-preserved，exit `0`。该一条无 PII canary 是不可变验收证据，按设计不删除。
...
- Exact-source deployment：从 `b805dd67e` Git archive 部署 backend=`000000df-0000-4000-8000-000000000000`、frontend=`00000474-0000-4000-8000-000000000000`，均 `SUCCESS`。backend-api 初次 deployment=`00000280-0000-4000-8000-000000000000` 在 schema owner 完成 migration 前按 readiness fail closed，10 次重试耗尽后为 `REMOVED`；schema ready 后从同一 archive 重提 `0000018e-0000-4000-8000-000000000000` 为 `SUCCESS`。该恢复缺口继续归 Group 8 schema-wait fault matrix，不伪装成 B-01 未关闭。
...
- production/fault/recovery：首轮 exact-source 部署捕获 `writer_epoch_rejected legacy run authority`；没有删事件或放宽所有 writer，而是以 additive projection epoch 区分“新写权威”与“既有 Run evidence projection”。同一 event 后续 `attempts=1`、`error=null`、成功 projected，open projection=`0`。最终 `5ffdb464f` 三服务 deployment：backend=`00000448-0000-4000-8000-000000000000`、backend-api=`00000242-0000-4000-8000-000000000000`、frontend=`00000143-0000-4000-8000-000000000000`，全部 `SUCCESS`；backend health=`ok`，runtime=`app_rls/strict/non-superuser/non-BYPASSRLS`，sandbox passed，三个 daemon healthy，frontend HTTP 200，最终部署日志无 `writer_epoch_rejected`/Traceback/IntegrityError/Exception/ERROR 命中。
...
- production/fault/recovery：从 `01e979bb3` Git archive 发布。backend=`0000037b-0000-4000-8000-000000000000`、frontend=`000000a4-0000-4000-8000-000000000000` 均 `SUCCESS`。backend-api 首次 deployment=`00000469-0000-4000-8000-000000000000` 在主 backend 完成 migration 前按 schema readiness fail closed，10 次重启后为 `REMOVED`；schema ready 后从同一 archive 重提 `0000041b-0000-4000-8000-000000000000` 为 `SUCCESS`。该 rolling 顺序证据不放宽 readiness，也不伪装首轮成功；后续发布先等 schema owner 成功再启动只读 API。
...
- exact-source production：backend=`00000361-0000-4000-8000-000000000000`、backend-api=`00000407-0000-4000-8000-000000000000`、frontend=`000002d4-0000-4000-8000-000000000000`，均 `SUCCESS` 且 deployment message 指向 `4e385d423`。backend/backend-api 的 `runtime_notification_outbox.py`、`runtime_result.py`、migration SHA-256 与本地逐文件完全一致；production actual/expected head=`runtime_result_fanin_0717`、148 tables/4 triggers、issues=`[]`，四张相关表 RLS ENABLE+FORCE、四条 tenant policy、29 个关键 FK/unique/check 约束在位。public backend health=`ok`、runtime role=`app_rls/strict/non-superuser/non-BYPASSRLS`、三 daemon/sandbox/worker healthy，frontend HTTP/2 200；backend-api 日志为 `Application startup complete`。
...
- commit / deploy：原子实现 commit=`b9852f37f`；backend=`00000325-0000-4000-8000-000000000000`、backend-api=`00000344-0000-4000-8000-000000000000`、frontend=`00000147-0000-4000-8000-000000000000` 均 `SUCCESS`。首次 backend-api=`00000133-0000-4000-8000-000000000000` 在 writer migration 前按设计 fail-closed，schema ready 后同 commit archive 重提成功；production deny/read canary 未执行。
...
#### EVID-G2-016：公开 assistant prose、固定 Task ledger、实时 query 与 canonical artifact 消费
...
context_read_receipt:
  ...
  leaf_ids: ["SES-CONSUMER-001"]
  documents:
    - ref: "@docs/session-v2-cc-codex-alignment-contract-2026-07-14.md §17.2 / §17.7 / §18 / §28.5"
      role: "design-and-acceptance"
      decision_consumed: "公开 assistant text、private reasoning、Task durable ledger、live/reload/reconnect 与 final artifact 必须由 canonical Session truth 一致投影"
    - ref: "@docs/runtime-model-agency-constraint-audit-2026-07-13.md"
      role: "model-agency"
      decision_consumed: "平台不能把公开模型正文改写成 Thinking，也不能让 transport/projection failure 反向改写模型 round"
  source_baselines:
    hive_head: "5d9f18fe2"
    freecode_head: "7dc15d6c8fb0c40c7fcc02ce9b58204324252632"
    codex_head: "5c19155cbd93bfa099016e7487259f61669823ff"
  conflicts_or_deltas:
    - "FreeCode/CC 保留并展示公开 assistant text，但不强制 phase；Codex MessagePhase 明确允许 None，只有显式 Commentary 才有 typed commentary 语义"
    - "旧 ChatWorkLedgerDock 依赖 hover/focus、静默最多 10 条；task mutation 同时作为普通工具行重复展示"
    - "canonical Session event 已进 reducer，但 socket projector 未在 authoritative tool_result 后刷新 read model；用户必须刷新才看见 Task/状态变化"
    - "terminal final 只引用文字 source block，没有把已治理 current-turn ChatArtifact 作为 canonical parts 交给 frontend deliverables consumer"
  evidence_sink: "EVID-G2-016"
```

- 原始症状/path proof：production 截图中公开进度全部退化为重复 `Thinking / Provider-private reasoning was used`，`task_create/task_update/A2A step` 成为平铺日志，Composer 上方没有常驻 Task，状态与交付物要刷新后才变化。源码追踪确认 `session_model_round → SessionEventOutbox → web_chat_run_orchestrator → sessionSocketEventProjector → SessionEventStore → chatDisclosureReducer/ChatWorkLedgerDock/right rail` 是唯一应修 live path；只改 fixture/CSS 不算接线。
- Red：canonical `tool_result` 到达后 read-model invalidation 缺失；artifact 在 outcome seal 后从 `owned` 漂移为 `quarantined` 仍可能被 final 投影；已提交 public envelope 的即时 broadcast 抛错会把成功模型 round 反向报错。三条测试分别先得到预期失败，再进入 Green。既有前端 Red 还覆盖 hover-only Task、静默 top-10、无 phase assistant text 被丢/标 Thinking、Task mutation 永久占行与 live socket 必须刷新。
- public prose Green：`seal_model_response()` 只把 direct-user public content snapshot/completed event ID 放入 `live_visible_event_ids`；private reasoning 不进入该集合。round/outbox 与 registry 均 commit 后即时广播同一 durable envelope；即时 transport 失败只记录 warning，由 outbox 以 stable event ID 重放。frontend 把 `assistant_text(unknown)` 渲染为 Markdown prose，显式 commentary 保持 commentary，private reasoning 保持克制 `Thinking`；terminal source item 由 final render owner 去重。
- Task/live Green：`ChatWorkLedgerDock` 在 `pending/queued/running/waiting` 常驻 Composer 上方并显示全量 authorized ledger，terminal 保留摘要且用原生 `<details>` 折叠。按 FreeCode `TodoWrite/TaskCreate/TaskUpdate` 与 Codex `PlanUpdateCell` 的 source-backed 合同，成功 `track_todo/task_create/task_update/task_stop` 只由 Task ledger 消费，不再生成 Run tool row；failed/blocked/cancelled 继续作为 typed recovery surface。canonical task call、committed tool result、runtime kind、terminal run 使 Session/Runtime/Work-Ledger query 失效；live 时另有 5 秒 polling 作恢复兜底，不作为唯一更新机制。
- artifact Green：`_record_canonical_terminal_artifact_selection()` 在 outcome seal 前只绑定模型声明/唯一 current-turn write 且匹配 tenant/agent/session/run/root-user/root-session、`authority_state=owned` 的 artifact；`prepare_and_seal_run_outcome()` 保存 refs/parts/manifest hash，`commit_terminal_outcome()` 重新锁行校验。authority/manifest 漂移进入 typed reconciliation；frontend final 从 canonical `parts` 投影 deliverables，不从文件名或任意历史 write 猜交付物。
- Prompt/北极星：多步任务的 behavior contract 要求首次工具调用前和关键里程碑给简洁公开进度，但明确禁止 hidden reasoning/provider-private CoT。该要求恢复 CC 可见工作过程，不强迫模型暴露推理，也不让平台 authored prose 冒充模型。
...
- first deployment/browser canary：实现已由 commit `11001f133` 进入 production；backend=`00000071-0000-4000-8000-000000000000`、backend-api=`00000358-0000-4000-8000-000000000000`、frontend=`000003cf-0000-4000-8000-000000000000` 均 `SUCCESS`，health=`ok`。同一 authenticated browser 无刷新看到新 prompt、Task 从旧 4 项增长并完成为 6 项、terminal `SESSION_CANARY_OK`；Task summary 是 Composer 上方 closed native `<details>`，reload 后恢复 prompt、7 条 `track_todo` 证据与 final。Peer A2A `delegation_run` 另开 owner 可见 read-only window、无 Composer。
...
context_read_receipt:
  ...
  source_baselines:
    hive_head: "first implementation=69b4127b78b9da5ccc7708a8f4600da9731f4d26; replay follow-up based on that commit"
    freecode_head: "7dc15d6c8fb0c40c7fcc02ce9b58204324252632"
    codex_head: "5c19155cbd93bfa099016e7487259f61669823ff"
  evidence_sink: "EVID-G2-017"
```

- first Red/path proof：真实 MiniMax canary 在强提示下仍无 public text；新增 backend contract 先以 missing handler/registry/core/capability/Plan Mode/prompt 得到 12 个预期失败，frontend reducer 先把 `report_progress` 当普通 surface tool且空 args 也留垃圾行，得到 2 个预期失败。
- first Green/deploy：`report_progress(message)` 是 core/default、Plan Mode 可用、ordered、governance-safe 的模型表达工具；handler 只回 `{ok, acknowledged}`，`agent.session.progress` 只作为 registry/audit taxonomy，admin policy definitions 不展示假开关，capability gate 不把当前 Session 表达误当外部 effect。commit `69b4127b7` 已部署为 backend=`0000032e-0000-4000-8000-000000000000`、backend-api=`000001dd-0000-4000-8000-000000000000`、frontend=`000000e9-0000-4000-8000-000000000000`，三者 `SUCCESS`；health/readiness 与 frontend HTTP 绿。
- production canary 反证：MiniMax M3 确实调用 `report_progress`，Task 与 final 继续 live；旧打开页因仍运行旧 bundle 显示 raw `report_progress`，hard reload 新 bundle 后 raw tool 被正确隐藏，但 `LIVE_PROGRESS_0717` 没有恢复。path proof 锁定两层：canonical V2 `prepare_tool_invocation()` 只把 `args_hash` 放入 `tool_call.started`，原始参数只在受控 `SessionToolInvocation.provider_arguments_json`；V1 rolling row 的 `content` JSON envelope 也未被 `serialize_session_event()` 提升成 `payload.tool_name/args`。因此“工具存在 + fixture 绿 + 三服务 SUCCESS”仍不算 Consumption/Recovery closed。
- follow-up Red→Green：新增 real-PG Red 先证明 `report_progress` prepare 后 `assistant_commentary` 数量为 0；backend legacy serializer 两个 Red 证明 V1 call/result 丢 `tool_name/args`；frontend rolling canonical Red 证明 `toolName/toolArgs` 均为 `undefined`。最终自审又加入 old-binary rolling Red：旧实例先写 `SessionToolInvocation` 但没有 commentary，新实例重放会被 existing-invocation 早退吞掉。Green 后，`SessionToolInvocation` 继续作为通用参数受控事实，不向用户公开任意工具 args；仅 `report_progress.message` 在同一 pre-effect transaction 中按原字节生成 stable `assistant_commentary.completed` + outbox，handler 仍只 ACK。即时返回按 sequence 同时广播 commentary/tool-call envelope；existing invocation 在同一 row lock 下按 stable item ID 幂等补 projection；frontend canonical reducer直接消费 commentary，旧 V1 envelope 则按 exact JSON machine contract 恢复，ACK/raw receipt/空 message 仍不渲染。
- deployed commentary path proof：commit `a456932ec` 已部署为 backend=`0000041a-0000-4000-8000-000000000000`、backend-api=`000000c8-0000-4000-8000-000000000000`、frontend=`00000234-0000-4000-8000-000000000000`，三者 `SUCCESS`。production sequence `1839` 是 exact `assistant_commentary.completed`，带 stable item/run/parent/invocation identity 与模型原字节；authenticated browser 在 completed Run 展开后 exact message 唯一可见，证明 persistence/serializer/replay/render 已接线，不是 fixture。
- final consumer Red→Green：同一 canary 发现旧 `assistant_message` 漂到最新 Turn 且与 canonical final 双渲染。源码根因是每条后续 compatibility event 都给累积 replay messages 重写当前 sequence，且 legacy/canonical projection 没有共享 render ownership。两个 production-shaped Red 分别得到 `OLDER_LEGACY_FINAL` 被排到最新 commentary 之后、同 `run_id` 同时出现 legacy/canonical final；Green 后只在 message 首次物化时冻结 sequence，同 typed `run_id` 的 terminal canonical `assistant_final` 成为唯一 render owner，不扫描或比较正文，unrelated legacy run 保持可见。
- Task 单一消费与 live route follow-up Red→Green：production browser 与 owner 截图证明真实 Hive 工具名 `track_todo` 未进入 task-mutation allowlist，成功调用同时出现在 Composer Task 和 Run raw row；同一 live canary 还证明 Task query 会无刷新变化，但 user input/commentary 直到 terminal/reload 才出现。5 个 Red 分别锁定 `track_todo` 展示/失败例外/query invalidation、所有 task mutation 的 dedicated-surface ownership，以及 legacy stream 必须写入 socket 携带的 exact Session。Green 后 successful task mutation=`external`，失败态仍=`surface`；canonical/legacy/optimistic message update 以 explicit `sessionId` 写入 per-Session store，不再使用可能滞后的 active selection 推断目标。
- final local tests/build：backend commentary substrate full=`7577 passed, 2 skipped`；本轮 task/live-route focused=`149 passed`，frontend full=`120 files / 728 tests`；production `tsc + vite build + bundle budget` 通过，AgentDetail=`338559/380000` bytes、gzip=`93282/115000`，vendor=`591449/620000`、gzip=`186474/200000`。architecture ledger=`11 passed`，diff check 全绿；新 exact-source commit、三服务部署与 authenticated no-refresh/hard-reload ordering canary 尚待写回。
- migration/backfill/rollback：无 schema migration。第一版发布前不存在产品级 `report_progress` 历史；该次 authenticated canary 的参数仍保存在 `SessionToolInvocation`，不把一次测试消息伪造成新的业务迁移。V1 rolling rows由 read adapter 原位恢复，无 DB rewrite；滚动发布期间由旧实例创建、仍会进入 prepare replay 的 invocation 在 row lock 下幂等补齐 commentary，不做全表扫描。rollback 删除 commentary projection/兼容 consumer 可恢复旧行为，但会重新打开 refresh 后丢 public progress 的断点；既有 invocation/event/outbox 不删除。
- status：`in_progress-local-green-live-route`；必须在新 exact source 上看到真实模型调用、无需刷新显示 exact commentary、raw `track_todo/report_progress` 均不出现、Task/final 继续 live，且 hard reload 后同一 message 顺序稳定、同 run final 唯一，才能关闭 `SES-CONSUMER-001`。

#### EVID-G2-018：canonical Redis event 到 process-local WebSocket 的 live bridge

```yaml
context_read_receipt:
  aa_entry: "§9 Group 2 + §12.1/§12.2 SES-TRANSPORT-001/SES-CONSUMER-001"
  leaf_ids: ["SES-CONSUMER-001"]
  documents:
    - ref: "@docs/session-v2-cc-codex-alignment-contract-2026-07-14.md §17.2 / §18 / §28.5"
      role: "session-design-and-acceptance"
      decision_consumed: "durable event、live socket、reload replay 与 exact Session store 必须同源且行为等价"
    - ref: "@docs/runtime-model-agency-constraint-audit-2026-07-13.md §5.5"
      role: "model-agency"
      decision_consumed: "平台只可靠传递模型 authored progress，不生成或改写其语义"
  source_baselines:
    hive_head: "1ff80c4ed + owned live-bridge diff; prior Session source=80711401f"
    freecode_head: "7dc15d6c8fb0c40c7fcc02ce9b58204324252632"
    codex_head: "5c19155cbd93bfa099016e7487259f61669823ff"
  evidence_sink: "EVID-G2-018"
```

- production path proof：authenticated canary 在 hard reload 后能恢复 exact commentary、Task、final 和 terminal folding，证明 DB/outbox/reducer/replay 已接线；但同一 run 在 reload 前不显示 commentary。修复前 production health 的同一 runtime snapshot 为 `process_role=runtime`、`runtime_task_worker.session_event_outbox_claimed=93/published=93/retried=0/dead_lettered=0`，而 `web_chat_stream_forwarder.running=false/forwarded=0/last_error=null/restart_count=0`。这不是 Provider 没写、前端没 render 或 event 丢失，而是 durable Redis publish 与 process-local WebSocket consumer 之间没有运行中的 bridge。
- verified root cause：`web_chat_broker` 的 connection registry 是进程内对象，browser `/ws/chat/{agent_id}` 由 production runtime role 接收；`main.lifespan()` 却只在 `HIVE_PROCESS_ROLE=api` 时启动 `start_web_chat_stream_forwarder()`。outbox worker 发布 `hive:session:event:v2:live` 后，API 进程收到事件但没有该 browser connection，runtime 进程持有 connection 却没有 subscriber，所以只能在 REST hydration/hard reload 后读到 PostgreSQL 事实。
- Red→Green：backend Red 要求每个 websocket-serving role（`runtime`、`api`）启动 forwarder、`read_model` 不启动，修前因缺 helper 确定失败；Green 后 lifespan 只通过 `_web_chat_stream_startup_enabled()` 注册 bridge。frontend architecture Red 又锁定 newest-page snapshot、legacy fallback 与 typed failure 三条 initial hydration path 必须调用 `setChatMessagesAfterQueuedForSession(sessionId, ...)`，禁止从可能已切换的 active selection 推断写入目标。
- CC/Codex/产品裁决：FreeCode 成功 Todo/Task mutation 不重复渲染 result row，Codex `PlanUpdateCell` 把计划更新投到 typed checklist；Hive 因此保持 successful `track_todo/task_create/task_update/task_stop` 只进入 Composer Task ledger，失败/blocked/cancelled 才回主时间线。普通工具与 model-authored public progress 在 active Run 展开，successful terminal 统一归入可恢复的 Run disclosure；final、approval/question、整体 failed/blocked recovery 与 deliverable 保持用户可见。private CoT、raw ACK/JSON/内部 ID 与重复 receipt 不进入产品时间线。
...
context_read_receipt:
  ...
  source_baselines:
    hive_head: "bdb68fbd3c5b + owned terminal-fold correction; deployed bridge source=9ebb51f1bac1"
    freecode_head: "7dc15d6c8fb0c40c7fcc02ce9b58204324252632"
    codex_head: "5c19155cbd93bfa099016e7487259f61669823ff"
  evidence_sink: "EVID-G2-019"
```

- production path proof：`9ebb51f1b` 发布后 health 为 `process_role=runtime`、forwarder `running=true/forwarded=111/last_sequence=2340/last_error=null/restart_count=0`，证明 Redis→process-local WebSocket bridge 已接线。authenticated 同页 canary 无刷新看到 Task add/start/complete、final 与 terminal folding，raw `track_todo/report_progress` row 均为零；但 accepted input 与两条 model-authored public progress 在 live 和 reload 后都缺失。suffix 修复 commit `70c4a12f2` 随后由 backend=`0000044c-0000-4000-8000-000000000000`、backend-api=`0000048b-0000-4000-8000-000000000000`、frontend=`00000157-0000-4000-8000-000000000000` 三个 production deployment 发布；tail-watermark commit `d29f52493` 又由 backend=`000001e3-0000-4000-8000-000000000000`、backend-api=`00000307-0000-4000-8000-000000000000`、frontend=`00000230-0000-4000-8000-000000000000` 发布，六个 deployment 均为 `SUCCESS`；health 的 schema/RLS/daemon/forwarder gate 与 frontend HTTP 200 均通过。
- durable evidence：同一 run `000001b8-0000-4000-8000-000000000000` 的 production PostgreSQL canonical sequence 已包含 `2192 human_input.accepted`、`2239/2289 assistant_commentary.completed`、`2337 assistant_final.completed`、`2338 run.completed`、`2339 turn.completed`、`2340 run_outcome.terminal_committed`。因此断点不在 Provider、写事务、outbox 或 bridge，而在 frontend canonical hydration/projection。
...
- candidate canary / verified root cause 3：`d29f52493` 发布后的 authenticated canary D 在同一未刷新页已实时看到 `SESSION_LIVE_TASK_0718_D` 进入固定 Task dock，raw `track_todo/report_progress` 精确计数为 0，但 exact progress/milestone 仍为 0。调用序列证明第二组修复只覆盖了 store 已存在后的重入：`selectSession()` 先注册 `canonicalHydration.then(full completion)`；transport controller 在首个 newest snapshot 写 store 前进入 `onBackfill`，命中 existing in-flight 后只能拿到完整历史 promise，因而根本到不了 watermark 分支。
- candidate canary / verified root cause 4：first-snapshot `liveReady` commit `9ad195fc3` 已由 backend=`000000b5-0000-4000-8000-000000000000`、backend-api=`000001af-0000-4000-8000-000000000000`、frontend=`00000349-0000-4000-8000-000000000000` 发布，三者 `SUCCESS`。authenticated canary F 在未刷新页继续证明固定 Task dock 会 live 更新且 raw `track_todo/report_progress` row 精确为零，但页面停在 `Loading durable session history...`，公开 progress/milestone 仍缺；首个 transcript REST page 直接返回 `HTTP 504`。`liveReady` 只有在收到首个 snapshot 后才有安全 cursor，因此它不能覆盖“REST 首页本身不可用”。最终修复增加 typed `cursor_mode=live_tail`：服务端先注册 live buffer、捕获 durable watermark `W`、以 `accepted_after_sequence=W` ready、从 `W+1` 投递；客户端以 `W` 建立空 projection baseline，旧历史继续走独立 REST recovery，不能把 504 解释成空历史或丢弃证据。
- production canary / verified root cause 5：typed live-tail commit `a1be2dfbe` 已由 backend=`000000a7-0000-4000-8000-000000000000`、backend-api=`0000015c-0000-4000-8000-000000000000`、frontend=`000000de-0000-4000-8000-000000000000` 发布，三者 `SUCCESS`；health 的 runtime/RLS/forwarder gate 与 frontend HTTP 200 通过。authenticated canary G 在同一未刷新页面实时出现固定 `SESSION_LIVE_TASK_0718_G` 与 `SESSION_LIVE_FINAL_0718_G`，raw `track_todo/report_progress` row 精确为零；手动展开 completed Run 后 `SESSION_LIVE_PROGRESS_0718_G` 与 `SESSION_LIVE_MILESTONE_0718_G` 各唯一出现。由此证明 event persistence、outbox、live-tail transport、canonical reducer、fold recovery 与 Task 专用消费均已接线。
- production canary / verified root cause 6：commit `bdb68fbd3` 已由 backend=`000002cb-0000-4000-8000-000000000000`、backend-api=`00000313-0000-4000-8000-000000000000`、frontend=`00000471-0000-4000-8000-000000000000` 发布且均 `SUCCESS`；health=`ok`、RLS=`app_rls/strict/non-superuser/non-BYPASSRLS`、frontend HTTP 200。fresh cache-bust load 在未点击 Run 的前提下验证 canary G 的 exact progress/milestone/final 各 1、Task dock 1、raw `track_todo/report_progress` 各 0、Run `aria-expanded=false`。随后同页 canary H 无刷新得到 Task/progress/milestone/final 各 1 且 raw tool 各 0，但 Run 已 `aria-expanded=false` 时 progress/milestone 仍常驻，精确证明“公开 commentary 永久可见”阻止了用户收起完整过程。最终修复移除 terminal persistent-step 旁路：running/failed/blocked 仍展开，successful terminal 的 commentary/reasoning/tool/compaction/A2A 过程统一折叠，final 留在 disclosure 外。
- Red→Green：第一组 production-shaped Red 用 `1206 human_input.accepted → 1207 assistant_commentary.completed → 1208 live commentary` 证明首个 suffix 原为空且 live cursor 不前进；第二个 Red 用 `1206 → 1208` 证明修复不能吞掉页内真实 `1207` 缺口。baseline Green 只计算 `min(fetched sequence)-1`；完整历史从 1 开始仍为 baseline=0，suffix 内 gap 继续进入 `gap_detected`。第二组 Red 要求健康 contiguous newest suffix 暴露可订阅 watermark，而真实 gap 必须返回 `null`；Green 新增 `liveSubscriptionWatermark()`，仅在 store 非 `full_hydration` 且 projection 非 `gap_detected/stale` 时返回 `highestContiguousSequence`。第三组 Red 直接阻塞第二个历史页，并要求首个 newest suffix 发布后 `liveReady=2205` 必须先于 full hydration resolve；同时源码合同禁止 `canonicalHydration.then(full completion)` 继续充当 transport readiness。Green 让 `loadCanonicalSessionTranscript()` 返回 `{ completion Promise + liveReady Promise }`：snapshot consumer 只有在 canonical store 安全连续时返回 watermark，真实 gap/stale/full-recovery 返回 `null` 并保持 fail-closed；旧历史继续自动恢复且不变成用户手动分页。第四组 Red 直接覆盖“没有首个 REST snapshot”的路径：backend 首先因缺少 cursor resolver 在 collection 阶段失败，frontend 为 `2 failed / 11 passed`；Green 后 `SessionSubscribeRequest.cursor_mode` 只允许 `resume/live_tail`，`live_tail` 强制 request cursor=0、server accepted cursor=watermark，WebSocket 只在该 typed 模式跳过历史 replay；frontend 不再 await REST backfill 才连接，并验证 tail accepted cursor 必须等于 server watermark。已有 cursor、gap/stale 和 full hydration 都继续 fail-closed 走 `resume`，不允许 tail 掩盖已知缺口。
- CC/Codex/产品裁决：FreeCode `Tool.ts` 与 `TodoWrite/TaskCreate/TaskUpdate` 把成功任务 mutation 交给 todo panel，不渲染 transcript result；Codex `PlanHandler → EventMsg::PlanUpdate → PlanUpdateCell` 只显示 typed checklist，不公开 raw args/ACK。Hive 因此继续保持：successful `track_todo/task_create/task_update/task_stop` 只进 Composer Task ledger（running 常驻、terminal 明细折叠），失败/blocked/cancelled 回 typed recovery；公开 commentary 在 active Run 实时展开、successful terminal 归入同一 Run history，final 独立常驻，private CoT/raw JSON/内部 ID/重复 receipt 不进入产品时间线。
- local acceptance：baseline 首次 Red=`2 failed / 3 passed`；baseline Green focused Session disclosure/hydration=`6 files / 194 tests`；commit `70c4a12f2` 的 clean exact-source archive（含 `.github` harness contract）=`120 files / 730 tests`。tail-watermark follow-up Red 同样为 `2 failed / 3 passed`，Green focused transport/hydration=`6 files / 36 tests`；commit `d29f52493` clean exact-source frontend full=`120 files / 730 tests`。first-snapshot readiness Red=`2 failed / 12 passed`，Green focused=`2 files / 14 tests`、相关 Session suite=`4 files / 133 tests`、共享工作树 frontend full=`122 files / 739 tests`，commit `9ad195fc3` 已三服务发布。live-tail Red=backend collection error + frontend `2 failed / 11 passed`；Green backend Session/WebSocket=`43 passed`、frontend focused=`4 files / 27 tests`、当前共享工作树 frontend full=`122 files / 744 tests`，ruff/format 与 architecture line budget 通过。`bdb68fbd3` 的 terminal-persistent 方案经 canary H 证伪后，最终 terminal-fold Red=`2 failed / 8 passed`；第一轮 Green 又反证两条旧 surface fixture，相关 suite=`2 failed / 153 passed`；最终 component=`10 passed`、focused disclosure/reducer=`4 files / 155 tests`、frontend full=`122 files / 745 tests`。`npm run build` 通过，AgentDetail=`339876/380000` bytes、gzip=`93784/115000`，vendor=`591449/620000`、gzip=`186474/200000`；architecture ledger=`11 passed`、`git diff --check` 通过。
- migration/backfill/rollback：无 schema migration、无历史数据 rewrite。历史与本轮缺失字节始终保留在 canonical Session events；`live_tail` 只建立当前 watermark 之后的 delivery fence，watermark 之前仍由 REST 自动恢复。rollback 不损坏证据，但会重新打开“首个 transcript page 失败即完全没有 live transport”的缺口；normal resume 的 gap/duplicate/out-of-order 保护不可随 rollback 放宽。
- status：`in_progress-local-green-terminal-fold`。typed `live_tail` 已由 exact commit `a1be2dfbe` 三服务发布；canary G 证明真实模型的公开 progress/milestone、Task 和 final 都无刷新到达且 completed Run 展开可恢复；canary H 证明 `bdb68fbd3` 的 terminal 常驻策略无法“只留 final”。只有最终 terminal-fold source 三服务发布后，在 running 时无刷新显示 exact progress/milestone、completed 默认折叠只留 final、展开恢复完整过程、Task/final 继续 live、raw task/progress tool 为零，且 hard reload 后顺序与字节等价，才能关闭 `SES-CONSUMER-001`。

#### EVID-G2-020：user-projected live sequence continuity

```yaml
context_read_receipt:
  aa_entry: "§9 Group 2 + §12.2 SES-PROJECTION-001/SES-TRANSPORT-001/SES-CONSUMER-001 + EVID-G2-019"
  leaf_ids: ["SES-CONSUMER-001"]
  documents:
    - ref: "@docs/session-v2-cc-codex-alignment-contract-2026-07-14.md §10 / §17.1.1 / §18 / §28.5"
      role: "session-live-redaction-and-continuity-contract"
      decision_consumed: "private provider bytes 必须在用户 delivery 前按 explicit JSON pointer 脱敏，但 immutable event identity/sequence 必须继续进入 highest-contiguous reducer"
  source_baselines:
    hive_head: "37599d2ca + owned V1 compatibility-outbox correction"
    freecode_head: "7dc15d6c8fb0c40c7fcc02ce9b58204324252632"
    codex_head: "5c19155cbd93bfa099016e7487259f61669823ff"
  evidence_sink: "EVID-G2-020"
```

- production Red/path proof：terminal-fold source `d60a669bd` 已由 backend=`000003fb-0000-4000-8000-000000000000`、backend-api=`00000435-0000-4000-8000-000000000000`、frontend=`000000c1-0000-4000-8000-000000000000` 发布且均 `SUCCESS`。fresh/no-refresh canary 的完成态能默认折叠、final 独立且展开恢复，证明 presentation contract 已接线；但 running commentary 仍需 reload。production PostgreSQL 对同一 run `000004bf-0000-4000-8000-000000000000` 已在 sequence `3156/3181` 提交两条 exact `assistant_commentary.completed`，分别早于 final 95/62 秒；CDP live capture 却从 canonical sequence `3229` 直接跳到 `3240/3243`。因此故障不在模型、持久化、折叠 reducer 或 final ownership，而在 live sequence delivery。
- production canary / verified root cause 2：private continuity commit `37599d2ca` 已由 backend=`00000319-0000-4000-8000-000000000000`、backend-api=`00000413-0000-4000-8000-000000000000`、frontend=`000000be-0000-4000-8000-000000000000` 发布且均 `SUCCESS`；health=`ok`、forwarder=`running`、frontend HTTP 200。authenticated canary L 的 WebSocket 收到 21 条 `assistant_reasoning_private` event，均保留 sequence 且 `payload.content/parts` 不存在；public progress/milestone/final 分别为 sequence `3310/3347/3362`。终态默认 exact progress/milestone=`0/0`、final=`1`、Task dock=`1`；展开后 progress/milestone=`1/1` 且 raw `track_todo/report_progress=0/0`；折回及 hard reload 后再次只留 final，证明 terminal contract 已闭环。但 active snapshot 已 live 更新 Task 时 progress 仍未出现。production DB 对账证明缺失序号 `3293/3295/3296/3309/3319/3320/3331/3336/3337/3346/3356/3357/3368` 全是 schema V1 runtime row；它们与 V2 共用 Session cursor，却没有 `SessionEventOutbox`，所以 strict reducer 仍不能越过这些 V1 gap。
- verified root cause：第一层是旧 bridge 对 `private_provider/operator` 直接 `continue`；第二层是 `append_session_event()` 写入 V1 `ChatTranscriptEvent` 后只安排 T0 projection，没有为同一 sequence 创建 live outbox。两类 row 都占用统一 per-Session sequence，因此“不可见就不投递”和“V1 只靠 REST”都会阻塞后续 public V2 event。private event 与 V1 compatibility event 都必须以 user-safe envelope 推进 cursor；前者不显示正文，后者按现有 compatibility reducer 恢复精确旧证据。
- implementation：`publish_canonical_session_event()` 先验证 durable source envelope hash，再生成 user projection；explicit `redaction_paths` 指向的 provider-private bytes 在进入 Redis 前删除，wire envelope记录 source/projected hash 与 `projection_audience=user`，但保留原 `event_id/session_id/sequence`。API forwarder验证 projected hash并投递全部 sequence；滚动部署期间收到旧 raw publisher envelope 时执行同一投影。follow-up 在 `append_session_event()` 的同一 DB transaction 为每个 V1 row 生成 `SessionEventOutbox`：可证明的 V2 adapter 保存 operator envelope并在 publish 时投影，无法证明 typed scope 的 compatibility envelope直接保存 user-safe view；`SessionEventOutboxPublisher` 按 schema 从 `scope.session_id` 或 compatibility top-level `session_id` 验证 authority，Redis publisher/forwarder同时接受 canonical V2 与 compatibility V1。public commentary/final 不做语义重写，也不扫描自然语言判断状态。
- Red→Green：private continuity 回归首次为 `2 failed / 2 passed`：一条证明私有正文仍进入 Redis，另一条证明 forwarder 只产生 `[1,3]` 而不是 `[1,2,3]`；Green 后 backend contract/wiring=`57 passed`。V1 follow-up 又以两条 Red 精确复现：compatibility claim 因没有 `scope.session_id` 被拒，forwarder 仍只产生 `[1,3]`；Green 后 V1 append/outbox/claim/publish/forward、Session persistence/contract/terminal 与 tenant-less channel fixture compatibility focused=`84 passed`。frontend acceptance 证明 compatibility/private continuity event 推进 cursor、不生成 private 正文、后续 public delta 立即投影，当前 reducer/consumer/hydration=`3 files / 27 tests`；此前完整 Session disclosure=`4 files / 54 tests`、frontend full=`122 files / 746 tests`、`tsc + vite build + bundle budgets` 均通过，AgentDetail=`339876/380000` bytes、gzip=`93784/115000`，vendor=`591449/620000`、gzip=`186474/200000`；architecture ledger=`11 passed`，scoped Ruff/format/diff check 通过。
- broader regression ownership：最终共享工作树 backend full=`7632 passed, 2 skipped, 7 failed`。7 个失败全部位于另一 session 的未提交 IM-channel scope：1 个临时 router shape、5 个 untracked `im_channel_identity_closure_0718` head 与旧 0717 断言冲突、1 个新 Feishu RLS bypass 尚未登记；本项曾新增的 tenant-less channel fixture 失败已修复并进入 focused 84/84。其余失败均不与本证据 owned paths 相交，未修改、暂存或伪装为本项通过。
- migration/backfill/rollback：无 schema migration、无历史数据 rewrite。既有 V1/V2 rows 已完整保留，REST reload 一直可恢复；新 outbox只覆盖部署后的 append，不伪造历史 publish。rollback 不损坏 durable evidence，但会重新打开 live sequence gap 与“只有刷新才更新”；`projection_audience` 保持 old-publisher/new-forwarder 滚动兼容，active old-run 仍可通过 reconnect catch-up恢复。
- deployment/active canary：V1 outbox follow-up commit `5e6d47b06` 已由 backend=`000002cf-0000-4000-8000-000000000000`、backend-api=`00000440-0000-4000-8000-000000000000`、frontend=`000003c9-0000-4000-8000-000000000000` 发布且均 `SUCCESS`。authenticated canary M 在同一未刷新 active Run 中看到 exact progress、Task 与 milestone，raw task/progress tool 仍为零，证明 `EVID-G2-020` 的 live shared-sequence closure 已真实接线。
- status：`closed-subcontract`。该证据只关闭 private/V1 continuity 与 active delivery 子合同；canary M 在 terminal 时暴露的 live folding 断点转入 `EVID-G2-021`，因此 `SES-CONSUMER-001` 本身仍不 closed。

#### EVID-G2-021：live terminal suffix replay 与最终折叠封口

```yaml
context_read_receipt:
  aa_entry: "§9 Group 2 + §12.2 SES-CONSUMER-001 + EVID-G2-019/020"
  leaf_ids: ["SES-CONSUMER-001"]
  documents:
    - ref: "@docs/session-v2-cc-codex-alignment-contract-2026-07-14.md §3.2 / §17.1.1 / §18 / §28.5"
      role: "session-live-terminal-consumer-contract"
      decision_consumed: "run terminal 是完成态折叠的唯一机械权威；final 保持 disclosure 外，turn/outcome metadata 不得覆盖已封口消息投影"
  source_baselines:
    hive_head: "5e6d47b060d5a55d0dade44f2661ada8e598588a + owned live-terminal correction"
    freecode_head: "7dc15d6c8fb0c40c7fcc02ce9b58204324252632"
    codex_head: "5c19155cbd93bfa099016e7487259f61669823ff"
  evidence_sink: "EVID-G2-021"
```

- production Red/path proof：authenticated canary M 在 active Run 无刷新看到 exact `SESSION_ACTIVE_PROGRESS_0718_M`、`SESSION_ACTIVE_TASK_0718_M` 与 `SESSION_ACTIVE_MILESTONE_0718_M`。terminal 前后 WebSocket 连续收到 `assistant_final.completed(3458) → run.completed(3459) → turn.completed(3460) → run_outcome.terminal_committed(3461)`；final 已唯一显示，progress/milestone 已从正文隐藏，但当前 Run 仍是 `处理中`，没有生成新的 `已处理`。hard reload 后同一 durable transcript 才生成 collapsed `已处理`，证明数据、transport 与 reload reducer 正确，live terminal projection 不等价。
- first implementation / production counterexample：第一轮修复让 terminal replay 使用 retained transcript 的 suffix baseline，并同步 exact Session 的 `RuntimePhase`；commit `165682a20` 已由 backend=`0000032d-0000-4000-8000-000000000000`、backend-api=`00000224-0000-4000-8000-000000000000`、frontend=`000002d1-0000-4000-8000-000000000000` 发布且均 `SUCCESS`。authenticated canary N 的 production DB/frames 已完整提交 progress=`3490`、milestone=`3511`、model final=`3526/3527`、compatibility final=`3533`、canonical `assistant_final=3536`、`run.completed=3537`、`turn.completed=3538`、`run_outcome.terminal_committed=3539`；页面最终只看到 exact final，却没有当前 N 的 progress/milestone 或新 `已处理`。因此第一轮“terminal 时重放 retained transcript”仍不是 live path closure。
- final verified root cause：live-tail delivery 与自动 older-history hydration 是两个独立恢复平面，本地 retained transcript 合法地可能只含历史 prefix、当前 live process 和部分 canonical tail，中间存在尚在 REST recovery 的空洞。terminal 试图从该数组重建整段 Session，既把历史恢复变成当前 Run 封口前置，又可能在 canonical final 到达时丢弃已经实时显示的过程。正确边界不是再猜一个 baseline，而是 terminal 只封口 exact Run：保留 latest user 后已显示的 live process，并让 exact `run_id` 的 canonical item补齐过程、唯一 terminal `assistant_final` 接管 render owner；完整历史继续独立恢复。
- final implementation：新增 `mergeCanonicalTerminalMessages(previous, canonical, terminalRunId)`。它保留当前 user 之前的 Session prefix；从当前 live tail 移除 compatibility/legacy answer owner但保留 commentary、Thinking、普通工具、A2A、compaction 等过程；按 stable message/session-item identity 合并 exact-run canonical process；最后只追加一个 terminal answer，优先 canonical `assistant_final`，否则保留已有 final。`projectCanonicalSessionSnapshot()` 不再接受或重放 retained transcript；`applyCanonicalSessionSnapshot()` 把 typed terminal `run_id` 传给 exact-Session updater。没有文本相似度、没有自然语言终态判断、没有改写 final bytes，也没有删除 durable process evidence。
- final Red→Green：新增 production-shaped Red 要求“locally gapped history + visible live progress + exact canonical final”完成后形成 `done` timeline，并要求 compatibility final 已先到达时 canonical final 成为唯一 render owner；首次 Red=`1 failed / 12 passed`，错误为 `mergeCanonicalTerminalMessages is not a function`。Green 后 targeted consumer/AgentDetail/architecture=`3 files / 30 tests`，Session disclosure/reducer/hydration/transport related=`9 files / 220 tests`，frontend full=`122 files / 749 tests`。`npm run build` 通过 TypeScript/Vite/bundle gate：AgentDetail=`341268/380000` bytes、gzip=`94217/115000`，vendor=`591449/620000`、gzip=`186474/200000`；owned-path `git diff --check` 通过。
- migration/backfill/rollback：无 schema migration、无历史 rewrite。terminal merge 只改变浏览器当前 Run 的 render composition；canonical DB event、server watermark、REST history recovery 与 hard-reload full replay保持机械权威。rollback 不损坏证据，但会重新打开“final 已到而当前过程无法形成 Processed”的消费断点。
- deployment：exact source commit `ee95cd98d` 已由 backend=`0000034d-0000-4000-8000-000000000000`、backend-api=`000002fd-0000-4000-8000-000000000000`、frontend=`0000024b-0000-4000-8000-000000000000` 发布，三者均 `SUCCESS`；backend health=`ok`，frontend HTTP 200。
- authenticated browser acceptance：canary O 首次在未刷新 terminal 后得到 collapsed `已处理（6 个步骤）`、final=1、progress/milestone=0、Task=1、raw `track_todo/report_progress=0/0`；展开后 progress/milestone=1/1、final=1，再折回恢复 0/0，hard reload 后同样默认折叠且展开可恢复。canary P 又在同一个最终 source Run 中完成全链：active `处理中` 时 progress/milestone/Task=1/1/1、raw tool=0/0；terminal 无刷新自动变成 `已处理（7 个步骤）`、final=1、process=0/0；展开恢复 1/1、再次折回 0/0；hard reload 后 `loading=false`、默认折叠、final=1、Task=1、raw tool=0/0。
- status：`closed`。Input、Authority、Execution、Evidence、Recovery、Consumption、Acceptance 均已由当前生产 live path 证明；`SES-CONSUMER-001` 由 `EVID-G2-016`–`021` 关闭。Group 9 仍独立拥有完整历史 migration/V1 cleanup 与更长时浏览器重连，不反向重开本 leaf。
...
- 三服务 exact-source deployment：backend=`000000fe-0000-4000-8000-000000000000`、backend-api=`000000c2-0000-4000-8000-000000000000`、frontend=`00000489-0000-4000-8000-000000000000`，均 `SUCCESS`。首次并发 `railway up` 在 GraphQL TLS handshake EOF 前失败且未创建 deployment；随后按服务串行重试成功，未触碰 production data。
...
- production exact-source deployment：backend=`00000372-0000-4000-8000-000000000000`、backend-api=`00000115-0000-4000-8000-000000000000`、frontend=`000003d6-0000-4000-8000-000000000000`，latest 均 `SUCCESS`。schema readiness 的 expected/actual head 均为 `storage_blob_lifecycle_0715`、130 tables、`issues=[]`。首次 backend-api 在 migration 前按设计拒绝旧 schema，schema ready 后用同一 archive 重提成功；该“固定重试耗尽后需重提”恢复缺口继续留在 Group 8 schema-wait fault matrix。
