...
def test_promote_legacy_feishu_sessions_rekeys_open_id_session_to_user_id() -> None:
    ...
    with engine.begin() as conn:
        ...
        conn.execute(users.insert().values(id="user-1", feishu_user_id="u_123", feishu_open_id="ou_456"))
        conn.execute(
...
def test_promote_legacy_feishu_sessions_uses_external_identity_when_user_legacy_fields_are_empty() -> None:
    ...
    with engine.begin() as conn:
        metadata.create_all(conn)
        conn.execute(users.insert().values(id="user-1", feishu_user_id=None, feishu_open_id=None))
        conn.execute(
