...
> 状态：设计权威；Group 2/3/4 substrate 已分别形成并部署，但 2026-07-17 live-entry/path 复核重新打开 `P1-004`、`SES-CONSUMER-001`、`A2A-TERMINAL-001`、`TEAM-FANOUT-001`、`ROOT-TREE-001` 五个回归 leaf。`EVID-G2-016/017` 已证明 Task 专用消费、公开 commentary 持久化/reload、rolling render ownership 与 Peer A2A read-only。`EVID-G2-018` 随 commit `9ebb51f1b` 三服务发布后证明 runtime forwarder 与 live Task/final 接线；§28.5 / `EVID-G2-019` 的 suffix baseline、tail-watermark、first-snapshot `liveReady` 与 typed `cursor_mode=live_tail` 又分别随 commit `70c4a12f2`、`d29f52493`、`9ad195fc3`、`a1be2dfbe` 三服务发布。authenticated canary G 已在同一未刷新页面实时得到固定 Task 与 final，raw `track_todo/report_progress` row 精确为零；展开 terminal Run 后，两条 exact public progress/milestone 同样存在。terminal-fold commit `d60a669bd` 的 canary 进一步证明 successful Run 默认收起、final 独立、展开恢复均成立，但 running commentary 仍被 live sequence gap 卡到 reload。`EVID-G2-020` 已用 DB/WS correlation 定位 bridge 丢弃 `private_provider/operator` canonical event 的根因，并完成“用户投影脱敏但序号不断流”的 local Green。最终产品合同保持：运行中公开 commentary/prose 实时展开；成功完成后与 Thinking、工具、compaction 一起进入同一个可恢复的 `Processed` disclosure，默认只留 final；Task ledger 独立固定；整体 failed/blocked 继续自动展开。新 source 发布及 no-refresh/hard-reload/private-byte canary 前 `SES-CONSUMER-001` 仍为 `in_progress-local-green-live-sequence`。完整 Session V2 继续由 Group 6/7/8/9/10 验收。
>
...
|---|---|---:|---|
| 用户输入、final | 正文，保持原字节与顺序 | 否 | final 位于 `Processed` disclosure 之外；用户输入不与前一 Run 混合 |
| 模型公开 `assistant_text/commentary`、`report_progress.message` | 运行中的过程正文，保持原字节与顺序 | running 展开；successful terminal 进入同一 `Processed` disclosure 并默认折叠 | 不得改写成 `Thinking`；展开后必须恢复完整过程正文；不显示 `report_progress` tool call/ACK |
...
历史 REST recovery 与 live transport 是两个正交状态。上述 1–8 是已有安全 cursor `C` 时的常规 `resume` 模式；当首次 REST page 仍在 retry/504、客户端尚无任何可证明连续的 cursor 时，允许使用唯一的 typed 失败恢复模式：

1. 客户端发送 `session.subscribe { after_sequence: 0, cursor_mode: "live_tail", ... }`，不得把未知 cursor 伪装成 `C=0` 的完整历史请求；
2. 服务端先注册 live buffer，再在同一 authority transaction 中捕获 durable watermark `W`，返回 `accepted_after_sequence=W=last_committed_sequence`；
3. 此模式不经 WebSocket 重放 `(0, W]`，客户端以空 projection baseline `W` 建立 live fence，只接收 `W+1...`；此前所有 canonical evidence 仍由 REST newest-first/background backfill 自动恢复，并与 live events 按稳定 event ID/sequence 合并；
4. UI 必须显示“live 已连接、旧历史仍在恢复”的 typed notice；REST 失败保持可重试和可观察，不能被解释成空历史或数据删除；
5. 已有 compatibility/canonical cursor、检测到 gap/stale，或明确要求 full hydration 时禁止使用 `live_tail`，必须回到 `resume`/gap recovery。客户端还必须验证 live-tail `accepted_after_sequence == last_committed_sequence`，不接受服务端任意移动 cursor。

`live_tail` 不是机械丢历史，也不改变 canonical truth；它只把“从此刻起无损接收新事件”从“旧历史已经可读”中解耦。服务端 buffer-before-watermark 保证捕获 fence 后提交的 event 不丢，REST recovery 保证 fence 前 evidence 最终仍可读。
...
| 当前 Hive 七原子审计 | 已完成并持续更新；5 leaf 仍 open | §28.4 五个协作回归已部署但行为验收 open；§28.5 已用 canary G 证明 live-tail、Task 专用消费、final 实时送达与展开后 commentary recovery。canary H 又证伪 `bdb68fbd3` 的 terminal 常驻策略；最终 terminal-fold local Green 尚待三服务发布与 no-refresh/hard-reload 终验，总报告继续维持 38/103 |
...
| Frontend typed projection | `in_progress-local-green-live-sequence:EVID-G2-016/017/018/019/020` | 三类协作、Composer Task、query invalidation、final artifact、read-only A2A、commentary persist/reload、live bridge、typed `live_tail` 与 terminal fold 已部署/canary；`020` 保留脱敏 private event 的 sequence continuity，待三服务发布与 no-refresh + hard reload + private-byte 终验 |
...
| 自动化黄金轨迹验收 | 既有 Group 4 保持；`EVID-G2-016/017/018/019/020` 保持 | `020`：live sequence Red=`2 failed / 2 passed`，Green backend focused=`57 passed`、frontend focused=`4 files / 54 tests`、frontend full=`122 files / 746 tests`，build/bundle budgets 通过；共享 backend full 的 7 个失败均归属另一 session 未提交 IM scope |
| Railway 行为修复与生产验收 | `EVID-G2-016/017/018/019` 已提供 durable/reload/live-bridge/live-tail/terminal-fold proof；`020` 待发布 | terminal-fold commit `d60a669bd` 已由 backend=`d83531f6-69c0-4f12-9e89-51bf9349502d`、backend-api=`e1ccd312-6fa1-4e1c-8044-e0eab851dcf7`、frontend=`26dc855a-8031-4b8d-8172-b50dda5e5f43` 发布且均 `SUCCESS`；canary 证明完成态折叠矩阵正确，同时用 live frame gap 证伪 running delivery。`020` 必须三服务同源发布，验证运行中实时、完成态默认只留 final、展开恢复、hard reload 等价与 private bytes 为零 |
...
2. `sessionEventConsumer` 与 `chatDisclosureReducer` 把 canonical `assistant_text` 投影为 Markdown 过程正文；运行中保持实时展开，successful terminal 后与该 Run 的 reasoning/tool/compaction 一起进入可恢复的 `Processed` disclosure。terminal final 的 source item 由 final render owner 去重并留在 disclosure 外，正文不会同时出现在 process/final。private reasoning 继续与公开文字分离。
...
10. hard reload 正确而同页 live 缺失的最终根因不在 reducer：production runtime health 显示 canonical outbox 已 publish 93 条，但 `web_chat_stream_forwarder.running=false/forwarded=0`。`web_chat_broker` 的连接是 process-local，browser socket 在 runtime；旧 lifespan 却只在 API role 启动 Redis subscriber。修复让 `runtime` 与 `api` 两个 socket-serving role 都启动 forwarder，`read_model` 不启动；initial newest-page、legacy fallback 与 failure hydration 也必须用 exact `sessionId` 写 per-Session store，不能从 active selection 猜目标。
11. bridge 发布后的同页 canary 证明 Task/final 已实时且 raw tool row 已隐藏，但 accepted input/commentary 仍缺失。production DB 已有同一 run 的完整 canonical sequence；前端 newest-first 首页却可能从 sequence `1206` 等后缀开始，旧 reducer 固定以 0 为 contiguous baseline，于是整页和后续 live event 都被当作 gap 缓存。projection 必须以首个已取 sequence 的前一位作为 omitted-prefix baseline；完整历史仍从 0 开始，suffix 内真实 gap 继续阻止越序。
12. suffix baseline 随 commit `70c4a12f2` 发布后的 authenticated canary 又证明：hard reload 能恢复 accepted input、progress、Task、milestone 与 final，但未刷新页仍缺 progress，console 同时报告 canonical history backfill `HTTP 504`。根因是 live connector await 完整旧历史 promise，错误地把历史可用性当成实时 admission 条件。newest suffix 一旦形成非 stale、非 gap、非 full-recovery 的 contiguous durable tail，就必须立即用该 watermark 打开 WebSocket；旧页继续后台回填，504 进入可观测 recovery notice，但不得让新 commentary/final 等待用户刷新。
13. tail-watermark commit `d29f52493` 三服务发布后的 canary D 又证明：Task 已无刷新进入固定 Work Ledger，raw `track_todo/report_progress` row 仍为零，但公开 progress/milestone 依然缺失。根因是 `selectSession()` 在首个 snapshot 投影前已把完整 hydration completion 注册为 in-flight，transport 的第一次 `onBackfill` 直接复用该 promise，store 尚不存在，后续 watermark 分支无法执行。canonical hydration 因此必须提供两个机械状态：`liveReady` 由首个安全 newest snapshot 的 contiguous watermark 完成；full completion 继续自动补齐旧历史。真实 gap/stale/full-recovery 只能让 `liveReady` 拒绝或等待修复，不能伪造连续性。
14. first-snapshot `liveReady` commit `9ad195fc3` 三服务发布后的 canary F 再次证明：Task 无刷新更新且 raw `track_todo/report_progress` row 精确为零，但页面持续显示 `Loading durable session history...`，公开 progress/milestone 仍未出现；直接请求首个 transcript page 得到 `HTTP 504`。这不是 `liveReady` 算错，而是它仍需要至少一个 REST snapshot 才能产生安全 cursor。最终协议把未知历史 cursor 显式建模为 `cursor_mode=live_tail`：服务端 register buffer → capture `W` → `session.ready(accepted=W)` → 从 `W+1` live delivery；客户端以 `W` 建立空基线，旧历史继续独立 backfill，不能再让首个 REST page 的可用性控制 live admission。
15. typed live-tail commit `a1be2dfbe` 三服务发布后的 authenticated canary G 证明 transport 断点已关闭：同一未刷新页面实时出现固定 `SESSION_LIVE_TASK_0718_G` 与 `SESSION_LIVE_FINAL_0718_G`，raw `track_todo/report_progress` row 精确为零；展开 completed Run 后 `SESSION_LIVE_PROGRESS_0718_G` 与 `SESSION_LIVE_MILESTONE_0718_G` 也各唯一出现。commit `bdb68fbd3` 随后三服务发布并由 canary H 证伪：Run header 已是 `aria-expanded=false`，但 exact progress/milestone/final 仍各显示一次，用户无法“收起过程只留 final”。最终矩阵因此锁定为：公开 commentary/prose 在 running 时实时展开，successful terminal 后与该 Run 的 private process/tool/compaction 一起默认折叠，展开后完整恢复；final 始终在 disclosure 外；整体 failed/blocked 仍自动展开恢复面。
16. terminal-fold commit `d60a669bd` 三服务发布后的 fresh/no-refresh canary 已证明第 15 条的完成态矩阵真实接线；随后 CDP frame 与 DB correlation 解释了“为什么 running 仍像没实装”：同一 run 的 public commentary 已在 final 前 62–95 秒 durable commit，但浏览器 canonical live sequence 从 `3229` 跳到 `3240/3243`。bridge 按 audience 丢弃 `private_provider/operator` event，严格 contiguous reducer 因而正确缓存缺口之后的所有 public event。live delivery 必须先按 canonical user projection 删除 explicit redaction paths 指向的私有 bytes，再把每个 immutable sequence 送入浏览器；private event 不显示正文，但必须推进 cursor。滚动部署期间 new forwarder 也必须脱敏 old publisher envelope，不能以版本切换重新制造 gap。

**Red→Green 与验收：** task `tool_result` 不刷新 read model、artifact authority 在 outcome seal 后漂移、即时 broadcast 失败污染已提交 round 三条新增回归均先按预期失败，再分别转绿。第一版 `report_progress` 发布后，真实 hard-reload canary 又证明 canonical tool event 不含 public args；follow-up real-PG、legacy serializer、frontend rolling replay以及 old-binary existing-invocation early-return Red 均先失败，再由原生 commentary transaction、幂等 rolling repair + V1 exact-envelope adapter 转绿。commit `a456932ec` 的 production DB/browser 又证明 commentary 本身已 durable/renderable，同时抓到旧消息 sequence 漂移和同 run 双 render owner；两个 production-shaped frontend Red 均先失败，再由 first-materialization sequence 与 typed-run canonical ownership 转绿。task/live-route 5 个 Red 也先按预期失败，Green 后 focused=`149 passed`、frontend full=`120 files / 728 tests`、architecture ledger=`11 passed`。bridge Red 进一步要求 runtime/api 启动 subscriber、read_model 不启动，并禁止 initial hydration 隐式写 active Session；`9ebb51f1b` 发布后 forwarder/live Task/final 已通过。suffix hydration 两个 Red首次为 `2 failed / 3 passed`，Green 后 newest suffix 立即投影且页内 gap 仍被捕获；focused=`6 files / 194 tests`、commit `70c4a12f2` clean exact-source frontend full=`120 files / 730 tests`，`npm run build` 通过。`70c4a12f2` 三服务发布后，真实 hard-reload canary 关闭 suffix projection 疑点并暴露 history 504 对 live connect 的阻塞；tail-watermark follow-up Red=`2 failed / 3 passed`，Green focused=`6 files / 36 tests`，commit `d29f52493` clean exact-source frontend full=`120 files / 730 tests`。canary D 继续暴露 first-waiter/full-completion race；first-snapshot readiness Red=`2 failed / 12 passed`，Green focused=`2 files / 14 tests`、相关 Session suite=`4 files / 133 tests`、共享工作树 frontend full=`122 files / 739 tests`，commit `9ad195fc3` 已三服务发布。canary F 又用真实 transcript `HTTP 504` 坐实“零安全 cursor”分支；live-tail Red 先表现为 backend helper import/contract collection error、frontend `2 failed / 11 passed`，Green 后 backend Session/WebSocket=`43 passed`、frontend focused=`4 files / 27 tests`、共享工作树 frontend full=`122 files / 744 tests`，ruff/format 全绿。`npm run build` 通过，AgentDetail=`340022/380000` bytes、gzip=`93847/115000`，vendor=`591449/620000`、gzip=`186474/200000`。commit `a1be2dfbe` 已三服务发布并由 canary G 证明 Task/final/commentary 都能 live 到达且展开可恢复。`bdb68fbd3` 的 terminal-persistent 方案经 canary H 证伪后，最终 terminal-fold Red=`2 failed / 8 passed`；第一轮 Green 又暴露两条旧 surface 契约，相关 suite 为 `2 failed / 153 passed`；统一裁决后 component=`10 passed`、focused Session disclosure/reducer=`4 files / 155 tests`、frontend full=`122 files / 745 tests`。terminal-fold commit `d60a669bd` 的 production canary 证明成功完成后默认折叠、final 独立与展开恢复均成立，并继续暴露 live sequence gap。sequence-continuity Red=`2 failed / 2 passed`，Green backend contract/wiring=`57 passed`、frontend disclosure/reducer/hydration=`4 files / 54 tests`、frontend full=`122 files / 746 tests`；`npm run build` 通过，AgentDetail=`339876/380000` bytes、gzip=`93784/115000`，vendor=`591449/620000`、gzip=`186474/200000`，Ruff/format 全绿。共享 backend full=`7631 passed, 2 skipped, 7 failed`，7 个失败均属于另一 session 未提交的 IM router/migration/RLS scope，不与本项 owned paths 相交。user-projected continuity source 三服务 deploy 与 authenticated no-refresh/hard-reload/private-byte canary 前仍不得写 production closed。

**七原子：** Input=provider public text、model-authored `report_progress.message`、canonical task/tool events、current-turn artifact declarations；Authority=Session visibility + exact RuntimeTask/ChatArtifact ownership，progress expression 不被 effect capability 接管，private bytes 只按 authoritative JSON-pointer redaction；Execution=round seal/outbox→user projection→Redis→socket-role bridge、core progress tool、pre-effect commentary commit、single Session reducer、ledger query 与 terminal outcome；Evidence=source/projected envelope hash、immutable event sequence、SessionEventOutbox、forwarder health、stable commentary/item/invocation/run ID、first-materialization sequence、suffix projection baseline、server-selected live-tail fence、受控 tool arguments、RuntimeTask metadata、artifact manifest hash；Recovery=old-publisher/new-forwarder rolling projection、outbox replay、V1 exact-envelope adapter、canonical render ownership、query invalidation/polling、旧历史 REST recovery 与 live transport 解耦、typed `resume/live_tail`、exact-Session newest-first/reload/reconnect reducer、typed reconciliation；Consumption=private continuity event 只推进 cursor、不展示正文，running 时实时公开、successful terminal 可折叠且可恢复的 commentary/process，disclosure 外唯一 final、Composer Task dock、right-rail deliverables；Acceptance=Red→Green、full frontend/backend、production build、browser live/reload/private-byte canary。`EVID-G2-016`–`020` 保持；user-projected continuity source 三服务发布并通过 no-refresh/hard-reload 终验前 `SES-CONSUMER-001` 仍不 closed。
