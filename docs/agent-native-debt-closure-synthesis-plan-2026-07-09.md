...
| `codex_optimization_ledger` 本机路径脆弱 | 成立。控制面 payload 不应携带 `${LOCAL_HOME}/...` 这种本机绝对路径作为可运行事实。 | 第一部分 P1 |
...
| 文件 | 改动 |
|---|---|
| `backend/app/runtime/codex_optimization_ledger.py` | 移除 `${LOCAL_HOME}/...` 绝对路径；改成相对 source family 或文档 ref |
