...
- Wire `UnderstandingStore` into `MemoryRetriever` so relationship
  memory enters the same activation and prompt assembly path as T3
  semantic facts.
...
- Utility smoke: an `agent_a -[collaborator]-> agent_b` understanding
  appears in retrieval with `source_type=understanding_store`; a T2/T3
  memory write creates a matching lifecycle entry keyed by the same
  `entry_id`; an approved `consider_full_authority` proposal adds the
