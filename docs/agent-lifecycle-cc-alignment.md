...
| B4 | **自主模式语义框架缺失** | ✅ **已修复（2026-06-04）**：`build_dynamic_prompt_suffix` 新增 `source` 参数，source∈{trigger,heartbeat} 时注入统一 `## Autonomous Work` 段（wake context/bias toward action/authority unchanged/pacing 防 busy-loop/state recording 责任）——CC `# Autonomous work` 的 Hive 化版本；kernel 5 个调用点接线（source 字符串与 trigger_daemon/heartbeat 实际值核实匹配）。**证据**：`runtime/prompt_builder.py` `_AUTONOMOUS_WORK_SECTION`、`kernel/engine.py` 5 处传参；tests/runtime/test_prompt_builder.py +3（trigger 注入/heartbeat 注入/live chat 不注入），runtime+kernel 606 passed | source∈{trigger,heartbeat} 统一注入 |
