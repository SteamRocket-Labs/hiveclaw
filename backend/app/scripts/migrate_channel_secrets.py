...
from app.services.channel_secret_storage import inspect_channel_secret_rows, migrate_channel_secret_rows
    ...
    if apply:
        ...
        async with schema_engine.begin() as connection:
            return await connection.run_sync(
                lambda bind: migrate_channel_secret_rows(bind, provider=provider, apply=True)
            )

    async with schema_engine.connect() as connection:
        return await connection.run_sync(lambda bind: inspect_channel_secret_rows(bind))


def main() -> None:
    parser = argparse.ArgumentParser(
        description="Count legacy plaintext channel credentials or encrypt/rotate them in place."
    )
