...
def promote_legacy_feishu_sessions(connection: Connection) -> int:
    ...
    external_identities = (
        Table("external_identities", metadata, autoload_with=connection)
        if "external_identities" in table_names
        else None
    )
    identity_providers = (
        Table("identity_providers", metadata, autoload_with=connection) if "identity_providers" in table_names else None
    )
        ...
        if existing_session and existing_session["id"] != row["id"]:
            ...
            existing_updates = _chat_session_update_values(
                ...
                last_message_at=(
                    ...
                    and (
                        existing_session["last_message_at"] is None
                        or row["last_message_at"] > existing_session["last_message_at"]
                    )
                    else None
                ),
            ...
            if existing_updates:
                connection.execute(
                    update(chat_sessions).where(chat_sessions.c.id == existing_session["id"]).values(**existing_updates)
                )
