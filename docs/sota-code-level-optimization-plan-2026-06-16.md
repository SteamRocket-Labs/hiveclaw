...
| Phase 7: memory lifecycle hold writers 接入 T2 主写入路径 | completed | `append_t2_entries()` 将低置信且 tentative/uncertain/volatile/stale 的抽取写成 lifecycle sketch 而非 active T2；消费 `conflicts_with` metadata 调用 `record_conflict()`；对缺失的本地 `workspace/` / `memory/` / `runtime_artifacts/` 引用调用 `mark_reference_revalidation_required()` | Red: `pytest tests/memory/test_t2_store.py::test_append_t2_entries_routes_low_confidence_memory_to_lifecycle_sketch tests/memory/test_t2_store.py::test_append_t2_entries_records_conflict_hold_from_extraction_metadata tests/memory/test_t2_store.py::test_append_t2_entries_marks_missing_workspace_ref_for_revalidation -q` -> `3 failed`（低置信仍 active、conflict/revalidation hold 未写入）；Green: `cd backend && source .venv/bin/activate && pytest tests/memory/test_t2_store.py tests/memory/test_lifecycle_maintenance.py tests/memory/test_retrieval_pipeline.py::test_retriever_suppresses_conflicted_and_revalidation_required_t3_entries -q` -> `17 passed`; `ruff check app/memory/t2_store.py tests/memory/test_t2_store.py` -> `All checks passed` | lifecycle maintenance 不再只有测试假数据；但语义矛盾发现仍依赖上游抽取 metadata，仍缺 live recall eval、外部引用真实重校验样本和双时态 KG |
| Phase 8: connector ACL authority fail-closed + protected snippet check | completed | `authoritative_connector_source_item()` 支持显式 `user_ids` / `department_ids` / `group_ids` / `tenant_ids` document ACL，并把仅有 `agent_id` 的 Feishu/Drive source item 标记 `deny_by_default` + `acl_authority=connector_unverified`；Feishu doc/drive source item 写入 content digest / protected snippet signatures；`validate_generated_source_permissions()` 不再只匹配 source URI，也会拦截 forbidden source 的受保护片段复述 | Red: `pytest tests/services/test_connector_acl.py::test_authoritative_connector_item_accepts_document_user_acl_and_blocks_other_user tests/services/test_connector_acl.py::test_tool_content_envelope_agent_only_acl_is_not_authoritative_for_connector_source tests/services/test_connector_acl.py::test_post_generation_permission_check_blocks_protected_snippet_without_source_uri -q` -> `3 failed`（缺 user ACL 入参、agent-only 被放行、snippet 未检查）；Green: `cd backend && source .venv/bin/activate && pytest tests/services/test_connector_acl.py tests/kernel/test_generated_source_acl.py tests/services/test_feishu_cli_runtime.py tests/services/test_feishu_drive_runtime.py tests/tools/test_office_tools.py -q` -> `30 passed, 4 warnings`; `ruff check app/services/connector_acl.py app/services/agent_tool_domains/feishu_docs.py app/services/agent_tool_domains/feishu_drive.py app/tools/handlers/office.py tests/services/test_connector_acl.py tests/kernel/test_generated_source_acl.py tests/services/test_feishu_cli_runtime.py tests/services/test_feishu_drive_runtime.py tests/tools/test_office_tools.py` -> `All checks passed` | 自指 agent ACL 已不能作为 Feishu/Drive 权威证据；但完整 Glean 式覆盖仍需 Feishu/Drive collaborator API + Hive/connector identity mapping + 生产 access-denial trace |
| Phase 9: `needs_reconciliation` admin/API/UI 消费面 | completed | 新增 `runtime_reconciliation` service；`/admin/runtime-reconciliation` 支持 list/get/action；action 覆盖 `mark_resolved`、`archive`、`retry`，其中 retry 默认 409，只有 `reconciliation_retry_allowed=true` 才会切回 `pending`；Platform Dashboard 新增 Runtime Reconciliation 队列组件和 admin API adapter | Red: `pytest tests/services/test_runtime_reconciliation.py -q` -> `3 failed`（service 缺失）；`pytest tests/api/test_admin_runtime_reconciliation.py -q` -> `2 failed`（路由 404 / service attrs 缺失）；`npm run test -- src/api/domains/admin.test.ts` -> `1 failed`（adapter method 缺失）；`npm run test -- src/pages/admin-companies/AdminRuntimeReconciliationSection.test.tsx` -> module missing。Green: `cd backend && source .venv/bin/activate && pytest tests/services/test_runtime_task_service.py tests/services/test_runtime_reconciliation.py tests/api/test_admin_runtime_reconciliation.py tests/api/test_admin_workflow_ops.py -q` -> `16 passed`; `ruff check app/services/runtime_reconciliation.py app/api/admin.py tests/services/test_runtime_reconciliation.py tests/api/test_admin_runtime_reconciliation.py` -> `All checks passed`; `cd frontend && npm run test -- src/api/domains/admin.test.ts src/pages/admin-companies/AdminRuntimeReconciliationSection.test.tsx` -> `2 passed`; `npm run build` -> success | `needs_reconciliation` 不再是不可消费状态；但真实生产处理仍需要运营 SOP、权限更细的 org-admin tenant 面，以及对 mutating side effect 的人工核验记录 |
| Phase 10: sandbox probe scheduler + health component | completed | `code_execution.probe` 新增 `vercel_sandbox` 默认启用的定期 probe scheduler、一次性 scheduled probe failure report 持久化、latest setting health summary；startup 注册 `code_execution_sandbox_probe_scheduler`；`/api/health` 暴露 `code_execution_sandbox_probe` component，且仅在当前 scheduler 启用时把 probe 降级计入总 health | Red: `pytest tests/services/test_code_execution_probe.py::test_scheduled_sandbox_probe_once_persists_failure_report tests/services/test_code_execution_probe.py::test_sandbox_probe_scheduler_defaults_to_vercel_provider tests/services/test_code_execution_probe.py::test_sandbox_probe_health_component_reports_latest_status -q` -> `3 failed`（scheduler/health helpers 缺失）；`pytest tests/api/test_health_liveness.py::test_health_includes_code_execution_sandbox_probe_component -q` -> `1 failed`（`app.main.latest_sandbox_probe_health` 缺失）。Green: `cd backend && source .venv/bin/activate && pytest tests/services/test_code_execution_probe.py tests/services/test_vercel_code_execution.py tests/api/test_health_liveness.py -q` -> `21 passed, 3 warnings`; `ruff check app/services/code_execution/probe.py app/main.py tests/services/test_code_execution_probe.py tests/api/test_health_liveness.py` -> `All checks passed` | 定期 evidence path 不再依赖人工 CLI；但真实 Railway/Vercel 生产样本仍需 env/secrets、deploy 后 probe artifact、history trend 和 credential egress proxy / network allowlist 证据 |
