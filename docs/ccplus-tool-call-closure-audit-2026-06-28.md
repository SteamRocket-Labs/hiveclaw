...
| 基线 | 读取重点 |
| --- | --- |
| FreeCode / CC baseline | `${LOCAL_HOME}/vc-saas/free-code-main/src/query.ts`, `src/skills/loadSkillsDir.ts`, `src/setup.ts` |
| Codex Rust baseline | `${LOCAL_HOME}/Context Engineering/codex/codex-rs/mcp-server/src/message_processor.rs`, `mcp-server/src/exec_approval.rs`, `core/src/protocol.rs` |
