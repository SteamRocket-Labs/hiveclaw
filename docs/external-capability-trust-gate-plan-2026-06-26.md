...
状态：CC/Codex component adapter、Trust Gate review/snapshot、Catalog、Agent durable activation、session-scoped try activation、component-level selected activation substrate、credential handle binding、external component/hook evidence tables、GitHub/raw/command-shaped source materializer boundary、revoke/deactivate 后端入口、Marketplace manual/GitHub/CC/Codex discovery source 管理、Capability Factor Intake / promotion proposal substrate、Skill/Subagent 自产候选自动进入 Factor Intake、LegacyPackAdapter migration-only report、legacy pack migration dry-run API / Workspace UI、Agent/Workspace Extensions 入口已落地；本轮真实输入通道已闭环；hook/slash/app connector runtime activation 仍 fail-closed
...
- 已有：`SkillGuard` 静态扫描、Skill active installer、MCP server-first 管理、MCP tool policy、legacy plugin install/projection、`/agents/{agent_id}/extensions` read model、`ToolRuntimeService` / `CapabilityGate` / `ActionPreflightService` 运行时治理、CC plugin normalized adapter、Codex plugin normalized adapter、`external_capability_reviews` / `external_capability_snapshots` / `external_extension_catalog_entries` / `external_extension_components` / `external_extension_hook_registrations` / `external_extension_activations` Trust Gate substrate、`capability_factors` / `capability_factor_reviews` / `capability_promotion_proposals` intake substrate、`admission_class` / `governance_projection_json` 落库、enterprise review stage / approve / catalog API、agent-scoped catalog API、agent durable activation API 的 selected component / credential handle contract、agent session-scoped try API、manual/GitHub/CC/Codex marketplace source / entry / submit-review API、capability factor capture / proposal / approve / reject / archive API、外部 Skill URL / ClawHub / skills.sh / code execution sandbox / MCP prompt -> Skill 入口的 Trust Gate staging、standalone MCP import 的 Trust Gate staging、approved Skill snapshot -> existing Skill runtime activation、approved Skill snapshot -> session overlay Skill catalog / load_skill activation、approved MCP snapshot -> existing MCP server-first runtime activation、approved Subagent snapshot -> sanitized agent definition activation、approved Hook snapshot -> pending hook approval evidence、approved external snapshot activation 在 `/agents/{agent_id}/extensions` 的 read model 可见、Agent Detail 顶层 `Tools / Skills / Subagents` 已收敛为 `Extensions` 且默认显示可安装 Catalog / Try in chat / Self-grown factors，Workspace Settings 顶层 `Tools / Skills / Subagents` 已收敛为 `Extensions` 且默认显示 Catalog / Review queue / Marketplace sources / Marketplace entries / Capability Factor Intake，旧路径 redirect。
...
未完成：真正隔离 install worker/smoke test、少数组件 runtime binding、以及 proposal -> trusted snapshot 自动生成面。
```
...
- `backend/app/services/external_capabilities/trust_gate.py`：stage review、approve snapshot、publish catalog entries、persist component evidence、persist pending hook registrations。
...
- `backend/app/services/external_capabilities/marketplace_sources.py`：manual / GitHub / CC marketplace / Codex marketplace source sync、entry cache、entry submit -> Trust Gate review。
...
7. Marketplace source 管理已有 manual / GitHub / CC marketplace / Codex marketplace discovery source -> entry cache -> submit review 闭环；Capability Factor Intake 已有 capture / review / proposal / approve / reject / archive substrate，Skill/Subagent 自产候选自动采集已接入；legacy pack migration dry-run API / UI 已有。仍缺 approved fork -> trusted snapshot 生成、production dry-run 实际执行与正式 backfill。
...
- `backend/app/services/external_capabilities/marketplace_sources.py`：`sync_marketplace_source()` 支持 `manual`、`github`、`cc_marketplace`、`codex_marketplace`；GitHub repo/tree URL 会解析到 raw manifest；remote manifest 的 `entries` 或 `plugins` 只进入 `external_marketplace_entries` discovery cache。
- `frontend/src/pages/workspace/WorkspaceExtensionCatalogSection.tsx`：Marketplace source 创建表单新增 source type selector，不再 hardcode `manual`；不同 source type 有默认 URI/config。
- 验证命令：
...
ruff check app/services/external_capabilities/marketplace_sources.py \
...
| CC 源码 | 事实 | 对 Hive 的约束 |
|---|---|---|
| `${LOCAL_HOME}/vc-saas/free-code-main/src/types/plugin.ts:48` | `LoadedPlugin` 由 `manifest`、`path`、`source`、`enabled`、`commandsPath(s)`、`agentsPath(s)`、`skillsPath(s)`、`hooksConfig`、`mcpServers`、`settings` 等组成 | Hive adapter 的输入必须以 component container 处理，不允许把 plugin 当成单一 Skill 或单一 MCP |
| `${LOCAL_HOME}/vc-saas/free-code-main/src/utils/plugins/schemas.ts:884` | `PluginManifestSchema` 合并 metadata、hooks、commands、agents、skills、outputStyles、channels、mcpServers、lspServers、settings、userConfig | Hive normalized manifest 必须覆盖这些字段，并把暂不支持的字段明确放进 `unsupported_components` |
| `${LOCAL_HOME}/vc-saas/free-code-main/src/utils/plugins/pluginLoader.ts:1348` | `createPluginFromPath` 读取 `.claude-plugin/plugin.json`，并自动探测 `commands/`、`agents/`、`skills/`、`hooks/` 等目录 | Hive `CCPluginAdapter` 也要支持 manifest 声明和目录约定两种 component source |
| `${LOCAL_HOME}/vc-saas/free-code-main/src/utils/plugins/mcpPluginIntegration.ts:132` | plugin MCP 可以来自 `.mcp.json`、manifest `mcpServers`、JSON path、MCPB path/URL、inline config | Hive 必须把 MCP 作为 plugin-provided MCP component 审核，不是重写 MCP runtime |
| `${LOCAL_HOME}/vc-saas/free-code-main/src/utils/plugins/installedPluginsManager.ts:800` | `user` / `managed` scope 对当前 project relevant，`project` / `local` 需要 projectPath 命中 | Hive 不能把 enterprise/user availability 解释成所有 agent 自动 runtime 注入 |
| `${LOCAL_HOME}/vc-saas/free-code-main/src/utils/plugins/refresh.ts:59` | `/reload-plugins` 刷新 commands、agents、hooks、plugin MCP reconnect key、AppState plugins | Hive 的“reload/activation refresh”只能刷新 active component projection，不能创建新 runtime |
| `${LOCAL_HOME}/vc-saas/free-code-main/src/utils/messages/systemInit.ts:53` | system init 输出 `tools`、`mcp_servers`、`slash_commands`、`agents`、`skills`、`plugins` | Hive context composition 也要分 surface 组装，而不是把 plugin 整包塞进 prompt |
...
Round 3 / Workspace Settings 入口收敛证据（2026-07-07）：

- 修改 `frontend/src/surfaces/workspace/sections.ts`：
  - 新增 canonical `/enterprise/extensions`。
  - 从 workspace 主 section 中移除 `/enterprise/tools`、`/enterprise/skills`、`/enterprise/subagents`。
  - `WORKSPACE_LEGACY_REDIRECTS` 明确保留三条 legacy redirect 到 `/enterprise/extensions`。
- 修改 `frontend/src/App.tsx`：
  - `/enterprise/tools`、`/enterprise/skills`、`/enterprise/subagents` 路由层直接 redirect 到 `extensions`。
- 新增 `frontend/src/pages/workspace/WorkspaceExtensionsSection.tsx`：
  - 二级分段：`MCP & Plugins`、`Skills`、`Sub-agents`。
  - 复用既有 `WorkspaceToolsSection`、`WorkspaceSkillsSection`、`WorkspaceSubagentsSection`，不重写后台能力模块。
- 修改 `frontend/src/pages/EnterpriseSettings.tsx`、`frontend/src/pages/ControlPlane.tsx`：
  - 后台导航和 Control Plane card 只暴露 `Extension Catalog`。
  - Feature Hub 中旧 `subagents` 产品链接改为 `/enterprise/extensions`；Agent Detail 链接改为 `#extensions`。
- 验证命令：
...
cd frontend && npm run test -- \
  ...
  src/pages/WorkspaceFeatureHub.test.tsx \
...
Round 3 component-level activation / credential handle / hook pending evidence（2026-07-09）：

- 新增 `backend/alembic/versions/external_extension_components_0709.py`：
  - 创建 `external_extension_components`，把 approved snapshot 的 component type、qualified name、source path、content hash、runtime projection、metadata 落成独立审计 surface。
  - 创建 `external_extension_hook_registrations`，把 Hook component 记录为 `pending_approval`，带 snapshot、component、event、matcher、handler、approval metadata。
  - 给 `external_extension_activations` 增加 `selected_components_json` 与 `credential_handles_json`；activation 记录保存“本次启用了哪些 component”和“使用了哪些 credential handle 引用”。
- 修改 `backend/app/services/external_capabilities/trust_gate.py`：
  - `approve_external_capability_snapshot()` 在创建 snapshot 后同步写 component records、pending hook registrations、catalog entries。
  - Hook 只进入 fail-closed registration，不投影到 hook dispatcher。
- 修改 `backend/app/services/external_capabilities/activation.py`：
  - `activate_external_extension_for_agent()` 接受 `component_qualified_names`，未传时保持旧行为：整包 durable activation。
  - 传入 `component_qualified_names` 时只激活选中的 Skill/MCP/Subagent/Hook component。
  - MCP component 的 `credential_requirements` 必须由 `credential_handles` 满足；缺失时 fail-closed，不执行 MCP import。
  - credential 只保存 brokered handle 引用，不保存 secret value。
  - Hook component activation 只返回 `pending_hook_approval`，不注册 runtime hook。
- 修改 `backend/app/api/external_capabilities.py`：
  - `POST /agents/{agent_id}/external-extensions/{snapshot_id}/activate` 接收 `component_qualified_names` 与 `credential_handles`。
- 修改 `frontend/src/api/domains/extensions.ts`：
  - `activateExternalExtension(agentId, snapshotId, body?)` 支持 selected component / credential handle request；现有无 body 调用仍兼容。
- 修改 `backend/app/db_bootstrap.py`：
  - fresh bootstrap 的 forced tenant RLS 表名单补齐 external capability 全表族。
- 验证命令：

```bash
cd backend && source .venv/bin/activate && pytest \
  tests/services/test_external_capability_trust_gate.py::test_approve_external_capability_review_persists_component_records_and_hook_registrations \
  tests/services/test_external_capability_activation.py::test_activate_external_snapshot_selects_components_and_requires_credential_handles \
  tests/api/test_external_capability_activation_api.py::test_activate_external_extension_route_checks_agent_access_and_activates_selected_components -q
# 3 passed, 4 warnings in 1.96s
...
cd backend && source .venv/bin/activate && alembic heads
# external_extension_components_0709 (head)

cd frontend && npm run test -- src/api/domains/extensions.test.ts
...
Round 3 try-in-chat / session-scoped activation evidence（2026-07-09）：

- 新增 `backend/alembic/versions/external_extension_session_try_0709.py`：
  - `external_extension_activations` 增加 `activation_scope`、`session_id`、`expires_at`。
  - 替换旧的 snapshot 级唯一约束为两个 partial unique index：
    - durable agent activation：`tenant_id + agent_id + snapshot_id` 且 `activation_scope='agent' AND status='active'`。
    - session try activation：`tenant_id + agent_id + snapshot_id + session_id` 且 `activation_scope='session' AND status='active'`。
- 修改 `backend/app/services/external_capabilities/activation.py`：
  - 新增 `try_external_extension_in_chat()`。
  - Skill try 写入 `workspace/session_extensions/<session_id>/skills/<skill>/...`，不写 durable `workspace/skills`。
  - Session activation 记录 `activation_scope=session`、`session_id`、`expires_at`、selected components、credential handles。
  - MCP 在 session try 中不创建 durable `AgentMCPServerAssignment`，返回 `session_runtime_projection_pending`，直到 MCP session tool projection 单独落地。
- 修改 `backend/app/services/agent_context.py`、`backend/app/skills/loader.py`、`backend/app/runtime/invoker.py`：
  - runtime skill catalog 构建接收 session id。
  - `WorkspaceSkillLoader` 默认跳过 `session_extensions/`；只有匹配 session id 时才扫描对应 overlay。
- 修改 `backend/app/tools/adapters.py`、`backend/app/tools/handlers/skills.py`、`backend/app/services/agent_tool_domains/workspace.py`：
  - `workspace_args` handler 可选接收 `session_id`。
  - `load_skill` 按当前 session id 解析 session overlay skill；无 session 或 session 不匹配时无法按 name 读取。
- 修改 `backend/app/api/external_capabilities.py`：
  - 新增 `POST /agents/{agent_id}/external-extensions/{snapshot_id}/try`。
  - route 校验 chat session 属于当前 agent/tenant。
- 修改 `frontend/src/pages/agent-detail/AgentExtensionCatalogSection.tsx`：
  - Agent Catalog 在 URL 存在 `session_id` / `session` 时展示 `Try in chat`。
  - `Try in chat` 只传当前 catalog entry 的 `qualified_name`，避免整包 session activation。
- 验证命令：

```bash
cd backend && source .venv/bin/activate && pytest \
  tests/services/test_external_capability_activation.py::test_try_external_extension_in_chat_creates_session_activation_overlay \
  tests/services/test_agent_context_session_extensions.py::test_skill_catalog_includes_session_extension_overlay_only_for_matching_session \
  tests/api/test_external_capability_activation_api.py::test_try_external_extension_route_scopes_activation_to_chat_session \
  tests/tools/test_workspace.py::test_load_skill_allows_only_matching_session_extension_overlay -q
# 4 passed, 4 warnings in 2.05s

cd backend && source .venv/bin/activate && pytest \
  tests/services/test_external_capability_activation.py \
  tests/services/test_agent_context_session_extensions.py \
  tests/api/test_external_capability_activation_api.py \
  tests/services/test_mcp_server_service.py::test_get_agent_extensions_includes_external_snapshot_activations \
  tests/tools/test_workspace.py::test_load_skill_rejects_sibling_prefix_escape \
  tests/tools/test_workspace.py::test_load_skill_allows_only_matching_session_extension_overlay -q
# 12 passed, 4 warnings in 2.78s
...
cd backend && source .venv/bin/activate && ruff check \
  ...
  alembic/versions/external_extension_session_try_0709.py
# All checks passed!

cd backend && source .venv/bin/activate && alembic heads
# external_extension_session_try_0709 (head)
...
- Agent Detail 当前子页是 `Catalog`、`MCP & Plugins`、`Skills`、`Sub-agents`、`Self-grown`；`Self-grown` 已能展示和捕获 agent-local factors，但 pending reviews 仍未作为 agent 级独立 tab 展示。
- Workspace Extensions 当前有 approved catalog、pending review queue、Marketplace sources/entries、Capability Factor Intake；仍缺更细的 review detail drawer 和 factor-to-snapshot 自动生成面。
- Agent Catalog 支持 durable `Install` / `Deactivate`，并在当前 URL 有 chat `session_id` 时支持 `Try in chat` session activation。
- Durable activation service/API 已支持 `component_qualified_names` selected activation 和 `credential_handles` binding；Agent Catalog UI 仍没有 component picker、credential prompt、per-component installed state，因此产品闭环还不是 component-level。
- Session try 对 Skill component 已接入 session overlay Skill catalog / `load_skill`；MCP session tool projection 和 Subagent session resolver 仍未接入 runtime。
- Hook 已有 `external_extension_hook_registrations.status=pending_approval` 的 fail-closed evidence；仍没有 hook allowlist approval route、approval UI、或 existing hook dispatcher projection。
- Workspace Catalog 支持 reject/revoke；Agent Catalog 支持 deactivate。仍缺专门的 revoke history/detail view。
...
因此 Round 3 只能标记为：Agent / Workspace Extensions 入口、Catalog UI、approved snapshot durable activation、session-scoped try activation、Skill session overlay runtime、component-level selected activation substrate、credential handle binding substrate、hook pending evidence、Marketplace discovery、Factor Intake 子面已实装；完整 installed/available/pending/candidate read model、component picker、credential prompt、MCP session projection、Subagent session resolver、hook approval runtime projection 未完成。
...
Round 4 当前状态（2026-07-09 实装）：

已落地的是 discovery-only marketplace 管理闭环：

- `backend/alembic/versions/external_marketplace_sources_0709.py`：新增 `external_marketplace_sources`、`external_marketplace_entries`，并纳入 tenant RLS。
- `backend/app/models/external_capability.py`：新增 `ExternalMarketplaceSource`、`ExternalMarketplaceEntry`。
- `backend/app/services/external_capabilities/marketplace_sources.py`：支持 manual source sync -> entry cache，也支持 `github` / `cc_marketplace` / `codex_marketplace` remote manifest refresh -> entry cache；entry submit -> `stage_external_capability_review()`；不创建 catalog，不写 agent runtime。
- `backend/app/api/external_capabilities.py`：新增 `/enterprise/external-capabilities/marketplace-sources`、`/sync`、`/marketplace-entries`、`/submit-review`。
- `frontend/src/api/domains/extensions.ts`：新增 marketplace source/entry API adapter。
- `frontend/src/pages/workspace/WorkspaceExtensionCatalogSection.tsx`：Workspace Extensions 的 Catalog 子页展示 `Marketplace sources`、`Marketplace entries`，支持 source type selector、source 创建、sync、entry submit review，同时保留 approved catalog / pending review queue。
- `frontend/src/i18n/en.json`、`frontend/src/i18n/zh.json`：补齐 marketplace source/entry 文案。

仍未完成的边界：

- Marketplace refresh 当前只拉取 manifest JSON 并写 entry cache；不会 materialize runnable artifact，不会创建 approved snapshot，不会进入 agent runtime。
- 远程 marketplace refresh 禁止在 Agent runtime 或 ordinary tool path 执行 `git clone`、`npm`、`npx`；当前实现只做 allowlisted HTTP manifest fetch。
- 当前 UI 支持 `manual`、`github`、`cc_marketplace`、`codex_marketplace` source type；review detail drawer、proposal -> snapshot 自动生成面仍未完成。
...
# 5 passed, 4 warnings in 2.39s

ruff check app/models/external_capability.py \
  app/services/external_capabilities/marketplace_sources.py \
  app/api/external_capabilities.py \
  app/db_bootstrap.py \
  tests/services/test_external_marketplace_sources.py \
...
Round 5 当前状态（2026-07-09 实装）：

已落地的是 factor intake / proposal substrate：

- `backend/app/models/capability_factor.py`：新增 `CapabilityFactor`、`CapabilityFactorReview`、`CapabilityPromotionProposal`。
...
# 4 passed, 3 warnings in 2.33s

ruff check app/models/capability_factor.py \
  app/services/capability_factor_intake.py \
  app/api/capabilities.py \
  app/db_bootstrap.py \
  tests/services/test_capability_factor_intake.py \
  tests/api/test_capabilities_api.py \
  alembic/versions/capability_factor_intake_0709.py
# All checks passed!
...
pytest tests/services/test_external_capability_trust_gate.py \
  ...
  tests/services/test_capability_factor_intake.py \
  tests/api/test_capabilities_api.py -q
```
...
|---|---|---|
| `${LOCAL_HOME}/vc-saas/free-code-main/src/QueryEngine.ts:529` | `QueryEngine.submitMessage` 在 system init 前加载 `getSlashCommandToolSkills(getCwd())` 和 `loadAllPluginsCacheOnly()`，然后把 `tools`、`mcpClients`、`commands`、`agents`、`skills`、`plugins` 一起传给 `buildSystemInitMessage` | Plugin 影响上下文和可见 component，但不替代原有 model loop / tool loop / MCP runtime |
| `${LOCAL_HOME}/vc-saas/free-code-main/src/commands.ts:353` | `getSkills` 合并 skill dir、plugin skills、bundled skills、builtin plugin skills | Plugin skill 是现有 Skill/Command surface 的来源之一，不是独立 runtime |
| `${LOCAL_HOME}/vc-saas/free-code-main/src/commands.ts:476` | `getCommands` 在 base commands 中插入 dynamic skills，并保持 command enable/availability 过滤 | Dynamic skill / plugin skill 是 command list 的扩展，不是 runtime 重构 |
| `${LOCAL_HOME}/vc-saas/free-code-main/src/commands/plugin/ManagePlugins.tsx:194` | plugin component 被展示为 `commands`、`agents`、`skills`、`hooks`、`mcpServers` | Plugin 是一组 component 的包；component 分别挂到已有 surface |
| `${LOCAL_HOME}/vc-saas/free-code-main/src/commands/plugin/DiscoverPlugins.tsx:228` | marketplace install 写入 scoped install，安装后提示 `/reload-plugins` 激活 | 安装和激活是两步；reload 只是刷新可见 component |
| `${LOCAL_HOME}/vc-saas/free-code-main/src/utils/plugins/refresh.ts:59` | `refreshActivePlugins` 刷新 commands、agents、hooks、plugin MCP reconnect key，并更新 AppState | reload 是 active component swap；不是重建 runtime |
| `${LOCAL_HOME}/vc-saas/free-code-main/src/commands/clear/conversation.ts:180` | `/clear` reset MCP state 时保留 `pluginReconnectKey`，该 key 只由 `/reload-plugins` 触发变化 | Plugin MCP 通过既有 MCP reconnect/runtime 进入系统，不是单独 MCP runtime |
...
|---|---|---|---|
| Direct external Skill import 已收口到 Trust Gate | `backend/app/api/files.py`、`backend/app/api/skills.py`、HR external refs、MCP prompt -> Skill、sandbox-installed Skill 都返回 `review_required` / `blocked`，不再直接写 active `skills/` | 当前阻断点已解决；后续风险是别的新外部入口不能绕过同一 adapter | 外部 Skill 统一走 `SkillSourceAdapter -> Trust Gate review -> approved snapshot -> agent enablement -> existing skill loader`；内部 preset/platform skill copy 仍可用 active installer，但不代表外部来源可信 |
| `execute_code` sandbox HOME 产物已收口为 review candidate | `backend/app/services/agent_tool_domains/code_exec.py` 已把 sandbox `$HOME/.agents/skills` 产物 stage 到 Trust Gate | 用户运行 `npx skills add` 后不会越过 review/snapshot 改变当前 agent skill surface | sandbox 产物只能生成 staged artifact / review candidate；审核通过后仍进入既有 Skill loader |
| Plugin-provided MCP component 已接入 admission 与 existing runtime projection | CC plugin 可包含 `mcpServers`；standalone MCP import 现在 stage review，approved MCP snapshot activation 后调用 `import_mcp_for_agent_and_register()` | 风险点不是 MCP runtime 本身，而是 plugin/standalone source 若未经过 provenance、tool diff、credential、smoke test 审核就被启用 | `mcp_server` component 只治理 review 和 enablement；通过后仍投影到现有 MCP registry/client/runtime。Standalone MCP import 保留产品入口，但不是 direct runtime install |
| Plugin-provided hook 需要 approval/runtime projection closure | `external_extension_hook_registrations` 已记录 approved snapshot、component id、event、matcher、handler，并以 `pending_approval` fail-closed；legacy `PluginHookRegistration` 仍只属于旧 installed plugin projection | hook 会影响 session lifecycle，必须在执行前完成 handler allowlist、approval route、runtime dispatcher projection | hook component 已绑定 snapshot/component provenance；下一步补 allowlist approval API/UI，并只把 approved handler 投影到 existing hook dispatcher |
| `/enterprise/plugins/install` 已降级为 legacy projection API | `backend/app/api/plugins.py` / `plugin_install_service.py` 文案已标记为 legacy builtin/local projection；Workspace Tools 不再暴露手工 plugin install tab；前端 `extensionsApi` 也已删除 enterprise plugin install/uninstall/backfill/list adapter | 该 API 仍为迁移/兼容保留，但不再是新外部 plugin 安装入口 | 新安装走 External Capability Review / Catalog / Enablement，再投影到现有 runtime surfaces；legacy API 不接受远程 source |
...
| 债务 | 当前证据 | 影响 | 收口方式 |
|---|---|---|---|
| Agent Detail 四态 read model 已开始收敛 | `AgentExtensionsSection` 默认显示 Agent Catalog，可安装 approved catalog entries；`Self-grown` 子页已能展示/捕获 agent-local factors；`/agents/{agent_id}/extensions` 返回 active skills/MCP/plugins/external activations | 用户已经能从单入口区分 catalog availability、active activation、agent-local candidates；pending review 仍不是 agent 级独立 tab | 保持 canonical `/agents/{agent_id}/extensions` + `/agents/{agent_id}/external-extensions/catalog` + `/agents/{agent_id}/capability-factors` product contract；后续补 pending reviews；旧 `#tools/#skills/#subagents` 仅保留 hash 兼容映射 |
| Workspace Settings 治理面已接入 Catalog / Review queue / Marketplaces / Factor Intake | `WorkspaceExtensionsSection` 默认显示 approved catalog、pending review queue、manual/GitHub/CC/Codex marketplace source/entry、Capability Factor Intake，MCP/Skills/Subagents 是同一 Extensions 页的具体管理子面 | 后台已有审批、目录、发现源和因子入库入口；仍缺 review detail drawer、自动 telemetry intake、proposal -> snapshot generator | 继续在 `WorkspaceExtensionsSection` 内接入 detail drawer、automatic factor producers、Trust Gate snapshot generation；旧 `/enterprise/tools|skills|subagents` 仅保留 redirect |
...
| 债务 | 当前证据 | 收口方式 |
|---|---|---|
| 用户主路径文案已退役 `capability pack` | Agent create subtitle 已改为 extension catalog access；Workspace Tools 的 legacy plugin manual install tab 已从产品面移除 | 内部 legacy API / service 注释仍可保留迁移语义，但不能成为新能力入口 | 后续只在 migration docs / backend compatibility 注释中保留 pack 词汇 |
