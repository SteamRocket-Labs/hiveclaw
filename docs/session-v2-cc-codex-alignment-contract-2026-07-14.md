...
> 状态：设计权威；Group 2/3/4 substrate 已分别形成并部署，但 2026-07-17 live-entry/path 复核重新打开 `P1-004`、`SES-CONSUMER-001`、`A2A-TERMINAL-001`、`TEAM-FANOUT-001`、`ROOT-TREE-001` 五个回归 leaf。`EVID-G2-016` 已随 commit `11001f133` 三服务同源部署，Task 实时/折叠/reload 与 Peer A2A read-only browser canary 已通过。`report_progress` follow-up 已由 commit `a456932ec` 三服务同源部署，production DB 与 authenticated browser 证明 exact `assistant_commentary` 可持久化、reload 后展开 completed Run 可见；该 canary 同时暴露 rolling compatibility reducer 会把旧消息拖到最新 sequence，并与 canonical final 重复渲染。§28.5 / `EVID-G2-017` 已用 first-materialization sequence 与 typed `run_id` render ownership 转绿；新 consumer source 发布及 live/reload 顺序 canary 前 `SES-CONSUMER-001` 仍为 `in_progress-deployed-canary-partial`。完整 Session V2 继续由 Group 6/7/8/9/10 验收。
>
...
| Frontend typed projection | `in_progress-deployed-canary-partial:EVID-G2-016/017` | 三类协作、Composer Task、query invalidation、final artifact 与 read-only A2A 已部署/canary；原生 commentary 已证明 production persist/reload/render，rolling compatibility sequence 漂移与 legacy/canonical final 双渲染已 local Green，待新 consumer source 的无刷新 + hard reload 顺序验收 |
...
| 自动化黄金轨迹验收 | 既有 Group 4 保持；`EVID-G2-016` 已发布，`017` final consumer local Green | `016`：backend full=`7569 passed, 2 skipped`、frontend full=`720`、Playwright=`12`；`017` backend full=`7577 passed, 2 skipped`，final frontend focused=`61`、full=`120 files / 726 tests`、build/budget 绿；新 consumer production canary 待写回 |
| Railway 行为修复与生产验收 | `EVID-G2-016` 已部署/canary partial；`017` commentary source 已部署，final consumer patch 待发布 | commit `a456932ec` deployments：backend=`dd3c6417-7bba-401d-bd4f-de2caa84dc85`、backend-api=`27fc25a8-7cf7-4665-9d39-2c1717d3c26c`、frontend=`7473d4d4-f112-4f4e-93ff-bc743b9b69b1`，均 `SUCCESS`；exact commentary 已由 DB + browser 展开/reload 证明。compatibility ordering/render-owner patch 尚待同源发布与 browser 顺序终验 |
