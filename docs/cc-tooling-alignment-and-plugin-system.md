...
> 8 维度并行深读 CC 源码(`${LOCAL_HOME}/Context Engineering/claude-code-org`)+ Hive 源码，
...
### 2026-07-07 — Plugin hook 兼容性修复（✅ custom tool_executor 不再重复触发 runtime hooks）

**触发原因：** 插件系统更新后复查 CC 语义边界，发现 `PRE_TOOL_USE` 在默认工具执行路径已经由 kernel 统一触发，默认分支会传 `emit_runtime_hooks=False` 给 `ToolRuntimeService`；但 `request.tool_executor` 自定义分支没有传这个关闭参数。A2A message runtime 的 `_build_agent_message_tool_executor` 又是一个真实可达的 custom executor wrapper，且内部调用 `execute_tool(...)` 时使用默认 `emit_runtime_hooks=True`。结果是：同一个 tool call 在 A2A/custom executor 路径可能经历 kernel 层 `PRE_TOOL_USE` + ToolRuntimeService 层 `PRE_TOOL_USE` 两次，导致 plugin hook block、args overlay、审计记录重复，破坏 CC 的 hook 单次语义。

**修复：**
- `backend/app/runtime/invoker.py`：`_execute_tool_with_request` 的 `request.tool_executor` 分支在 executor 支持 `emit_runtime_hooks` 或 `**kwargs` 时，显式传 `emit_runtime_hooks=False`，与默认 `execute_tool` 分支保持一致。
- `backend/app/services/agent_tool_domains/messaging.py`：A2A message custom executor 接收 `emit_runtime_hooks` 并向 `app.services.agent_tools.execute_tool` 透传，避免 wrapper 吞掉 kernel 下发的关闭信号。
- `backend/tests/runtime/test_invoker.py`：新增 `test_custom_tool_executor_disables_inner_runtime_hooks`，锁定 custom executor 分支必须收到 `emit_runtime_hooks=False`。
- `backend/tests/services/test_agent_message_runtime.py`：更新 `test_build_agent_message_tool_executor_persists_tool_calls`，锁定 A2A wrapper 会接收并转发 `emit_runtime_hooks=False`，同时不影响 tool-call 持久化。

**边界确认：**
- Memory/KV 与 Attention Control 未改动；`runtime/invoker.py` 中 Activation Query 仍在 `USER_PROMPT_SUBMIT` 后、kernel 前由 runtime native path 构造，不依赖 plugin hook。
- plugin hook 仍只能作为外部 hook/control surface 追加 context、block 或收窄参数；不能替代 Q 构造，也不能绕过 tool schema/capability/ActionPreflight/approval。
- 这次修复只消除重复 hook 执行，不改变 hook event catalog、PluginHookRegistration、GovernedHookRunner、ToolRuntimeService governance 顺序。
...
失败点：
- custom executor 收到 emit_runtime_hooks=True，而不是 False。
...
$ source .venv/bin/activate && pytest \
    tests/runtime/test_invoker.py::test_custom_tool_executor_receives_delegation_token \
    tests/runtime/test_invoker.py::test_custom_tool_executor_disables_inner_runtime_hooks \
