...
- rolling compatibility projection 只能保留每条消息首次物化时的 durable sequence；后续 phase/tool 事件不得把已存在消息重新标成当前 sequence。若同一 typed `run_id` 已存在 terminal canonical `assistant_final`，legacy `assistant_message` 必须让出 render ownership；去重依据是 run identity，不得比较或扫描正文；
