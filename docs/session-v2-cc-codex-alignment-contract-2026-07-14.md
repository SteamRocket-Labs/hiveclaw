...
> 状态：设计权威；Group 2/3/4 substrate 已分别形成并部署，但 2026-07-17 live-entry/path 复核重新打开 `P1-004`、`SES-CONSUMER-001`、`A2A-TERMINAL-001`、`TEAM-FANOUT-001`、`ROOT-TREE-001` 五个回归 leaf。`EVID-G2-016/017` 已证明 Task 专用消费、公开 commentary 持久化/reload、rolling render ownership 与 Peer A2A read-only。`EVID-G2-018` 随 commit `9ebb51f1b` 三服务发布后，production health 已证明 runtime forwarder `running=true/forwarded=111/last_sequence=2340/last_error=null`，同页 canary 也无刷新得到 Task/final 且 raw task/progress row 为零。§28.5 / `EVID-G2-019` 的 suffix baseline 修复已随 commit `70c4a12f2` 三服务发布；authenticated canary 证明 hard reload 后 accepted input/progress/Task/final 全部可恢复，但同页 progress 仍被旧历史 backfill `HTTP 504` 阻断。follow-up 已让可信 newest tail watermark 立即启动 live subscription、完整旧历史继续后台恢复；新 source 发布和 no-refresh/hard-reload 等价 canary 前 `SES-CONSUMER-001` 仍为 `in_progress-local-green-live-after-tail`。完整 Session V2 继续由 Group 6/7/8/9/10 验收。
>
...
| 当前 Hive 七原子审计 | 已完成并持续更新；5 leaf 仍 open | §28.4 五个协作回归已部署但行为验收 open；§28.5 又以真实截图和 candidate canary 坐实 `SES-CONSUMER-001` 的 live presentation/product-consumption 缺口。tail-watermark follow-up 仅 local Green，三服务发布与 authenticated browser canary 前总报告仍维持 38/103 |
...
| Frontend typed projection | `in_progress-local-green-live-after-tail:EVID-G2-016/017/018/019` | 三类协作、Composer Task、query invalidation、final artifact、read-only A2A、commentary persist/reload 与 live bridge 已部署/canary；suffix baseline 已发布，旧历史后台恢复与 live subscription 解耦已 Green，待 no-refresh + hard reload 终验 |
