...
| Frontend typed projection | `in_progress-local-green-terminal-fold:EVID-G2-016/017/018/019` | 三类协作、Composer Task、query invalidation、final artifact、read-only A2A、commentary persist/reload、live bridge 与 typed `live_tail` 已部署/canary；运行中公开过程、successful terminal 整体折叠、展开恢复与 final 独立的最终矩阵已 local Green，待三服务发布与 no-refresh + hard reload 终验 |
...
| 自动化黄金轨迹验收 | 既有 Group 4 保持；`EVID-G2-016/017/018` 保持，`019` local Green | `019`：live-tail source 已部署；terminal-fold Red=`2 failed / 8 passed`，修复后 component=`10 passed`；完整 focused/full/build 数字在本轮最终复跑后写回 |
| Railway 行为修复与生产验收 | `EVID-G2-016/017/018/019` 已提供 durable/reload/live-bridge/live-tail proof；terminal fold 待重新发布 | commit `bdb68fbd3` 已由 backend=`9409d935-562f-4b51-b65a-f55a59d8a69e`、backend-api=`a337c5b3-d07a-4e3a-94b0-90a2f56dae16`、frontend=`ed850682-1f0e-451e-9d6a-959fed7f811b` 发布且均 `SUCCESS`；canary H 证明 live Task/progress/milestone/final 与 raw-tool suppression 正常，但 completed Run 折叠时 progress/milestone 仍常驻，故该 source 不是最终闭环。修正 source 必须再次三服务同源发布，验证完成态默认只留 final、展开恢复过程、hard reload 等价 |
