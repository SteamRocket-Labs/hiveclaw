...
> are deferred until this foundation is measurable and better than `/Users/example-owner/vc-saas/hermes-agent` on core
...
Current authority guard: historical phase wording in this file never overrides the root AI-Native Design Law or Model Agency Boundary. In particular:

- "direct" memory means an immediately session-visible overlay or candidate; no durable T2/T3/Soul/Skill write bypasses the governed LLM review and Platform Gate commit path;
- "compact pointer" is allowed only for large evidence transport when the complete coverage index is visible and the original bytes are losslessly discoverable and recoverable; it is not permission for silent head/tail/top-k pruning;
- "low-signal" or learning-value decisions belong to the learning LLM, not keywords, counters, similarity, message length, or fixed scores;
- deterministic checks govern exact tests, schemas, states, receipts, permissions, and side effects. Open-ended meaning, root cause, importance, communication quality, and learning value remain LLM or human judgments;
- fail-closed freezes only the affected unauthorized or unprovable durable commit/external effect. It does not disable ordinary reasoning, unrelated tools, or model-authored final expression;
- Personal KB remains a governed tool-only knowledge source and is never preloaded into original context assembly.
...
For large artifacts, the model may see compact pointers only when it also sees a complete coverage index and has a governed, lossless, reliable path to retrieve the original bytes on demand. Pointers are transport optimization, not semantic filtering; decisive evidence must never disappear behind an undiscoverable reference.
...
- P0 memory such as user correction and blocked patterns may become an immediate session-visible overlay or candidate, but it never bypasses governance for durable T2/T3/Soul/Skill writes.
- P1/P2 memory can move toward index-first retrieval only when the model sees truthful coverage metadata, can recover full evidence, and shadow/live behavior evidence shows acceptable recall. Index-first is a disclosure protocol, not mechanical authority to discard evidence.
...
- The reflection LLM must distinguish, using complete evidence and an explicit abstain option:
...
- Ordinary chatter does not create churn because the learning LLM abstains with recorded evidence, not because a keyword, length, counter, similarity, or fixed-score classifier blocks it.
...
- No verification -> observable, recoverable hold of the durable promotion; session reasoning and candidate generation continue.
- Failed exact verification -> hold/reject only the durable promotion or effect, with a typed next action.
- Passing deterministic verification + source refs produces promotion evidence. Open semantic fitness still requires the designated LLM or human review; the Platform Gate commits the resulting exact authorized decision and never invents semantic approval.
...
- Score exact machine facts with deterministic checks; score open semantic quality with an LLM rubric; use human review for disputed or policy-bound cases. "Deterministic first" is not a license to approximate semantic quality mechanically.
