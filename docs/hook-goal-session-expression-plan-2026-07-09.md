...
| Step 2 | Goal 收尾闭环：§2.2 三空缺全补——①**收尾 wake**：`goal_continuation_service._maybe_issue_budget_summary_turn` 在 goal bridge 内检测 budget run `summary_only`，经 `mark_summary_turn_state` 行锁 CAS（None→issued 单赢家）恰好一次调度收尾 turn（`budget_summary_turn` 元数据 + `awaiting_budget` phase 广播），已发过则拒绝任何正常 continuation 穿透；②**模型可见收尾契约**：`prompts/goals.py:budget_summary_contract_prompt`（预算已尽/单次通道/禁放大四硬规/写收尾报告/update_goal 落 complete|blocked）；③**放行一次强制**：`reserve()` 收尾 lane 仅对带 `budget_summary_turn` 标记的非放大预约豁免维度检查（tokens 爆掉也放行、settle 照常记账、放大仍拒），`RuntimeBudgetedLLMClient(summary_lane=)` 经 invoker 从 session metadata 透传；收尾 turn 完成 → `hard_stop_run`（扩展接受 summary_only→hard_stopped）关死通道 + goal 兜底 BUDGET_LIMITED（update_goal 已落则不覆盖）；失败重试恰好一次（issued→retried CAS），二次失败 → sealed `summary_failed` + `runtime_action_failed` 事件告知前端"收尾未能生成"（拍板 2.5-a）；收尾 turn 全程锁 `summarizing` 相位（thinking/chunk/tool 不覆盖）。新列 `runtime_budget_runs.metadata_json` + 迁移 `runtime_budget_run_metadata_0709`。 | 新 `tests/services/test_budget_summary_turn.py` 7 passed（恰好一次/永不双发且永不穿透正常续跑/完成封口+goal 兜底/尊重模型已落状态/失败重试一次后二次封口+事件广播/健康 run 走正常路/契约 prompt 钉）；`test_runtime_budget_service.py` 真 PG 3 新钉（lane 豁免+settle 记账/hard_stop 封 summary_only 后 lane 标记也进不来/CAS 单赢家）33 passed 全文件；`test_web_chat_runtime.py` summarizing 相位钉（[summarizing, done] 全程不跳相）；相关回归 155+36 passed；顺手根修既有批跑污染（policy 测试泄漏 enabled policies 遮蔽 builtin fallback → 污染者 finally 清场，5 failed→0）。 | ✅ |
| Step 3 | Hook 治理双泳道全链落地（§1.6 六件）：①**消费桥**——`governance.py:_run_tenant_governance_hooks` 插在 `_run_governance_inner` 全部平台门放行后、`return None` 前（只缩不放的最后加严层）；生产接线经 `ToolGovernanceResolver.build_dependencies()`（service.py:690 每次 execute 走到，非孤儿）。②**声明式快路**——新模块 `tools/hook_governance.py`：CC matcher 语义（空/`*` 全匹配、`[A-Za-z0-9_|]+` 精确/列表、否则 regex 且非法 regex 永不匹配）+ `ArgRule` 参数条件（全命中才触发）+ 进程内求值零沙箱开销。③**command 沙箱慢路**——`governance_resolver.run_sandboxed_governance_hook`：payload 写 workspace 临时文件经 shell stdin 重定向注入（CC stdin 协议保真）、`execute_agent_command` 沙箱执行（D2 禁裸 subprocess、env 空白名单、network deny）、协议=exit 0+JSON decision/exit 0 无 JSON=no_opinion/exit 2=deny（CC parity）/其余 exit/timeout/error/malformed JSON=**fail-closed deny**（D1，五参数化钉测）；快路 deny 短路不进沙箱+未配 matcher 工具零开销（拍板 1.7-d）。④**聚合器**——deny>ask>allow（CC hooks.ts:2820 序）+ D3 只缩不放：company/user 层 allow 降级 no_opinion，managed 层 allow 保留 grant 但只作用于 hook 泳道内（压制租户层 ask、压不过 deny；平台门是地板）（拍板 1.7-b）；ask 翻译进现有 session-permission 确认流。⑤**多层合并**——`ExternalExtensionHookRegistration`（status=approved+PreToolUse）为统一存储（顺带闭环 C6-hook 的 runtime 读者缺口：`runtime_execution` 由 `not_yet_supported` 翻为 `governance_preflight`），layer∈managed/company/user 拼接合并 + 平台强制开关 `HIVE_DISABLE_ALL_GOVERNANCE_HOOKS`/`HIVE_ALLOW_MANAGED_HOOKS_ONLY`（CC disableAllHooks/allowManagedHooksOnly parity）+ TTL single-flight 缓存；配置解析失败=skip+audit（管理面问题），运行时失败=fail-closed（运行面问题）。⑥慢路执行前 emit `hook_evaluating` 相位（Step 1 骨架复用，phase 事件不持久化）；registry 读取失败 fail-closed。PostToolUse 注入型 hook 沿用既有 GovernedHookRunner fail-open（拍板 1.7-c）；prompt/agent/http 型作为既有 GovernedHookRunner 可选面保留（拍板 1.7-a 后续显式可选，不装死）。 | 新 `tests/tools/test_hook_governance.py` 20 passed（matcher 全表/arg_rules/聚合序含 D3 双钉/registration 解析含坏行 skip 钉）；新 `tests/tools/test_governance_hooks_pipeline.py` 12 passed（零配置零开销/deny 拦+事件/ask 进 session permission/快路短路沙箱/慢路 fail-closed×2/loader 失败 fail-closed/managed allow 压 ask/未命中零开销/hook_evaluating 相位钉）；新 `tests/tools/test_governance_hook_resolver.py` 10 passed（沙箱语义转换全表含 5 参数化 fail-closed/平台开关/managed-only 过滤）；`test_external_capability_activation.py` 翻旗断言更新；跨面回归 654 passed。 | ✅ |
| Step 4 | Session 表达补全（拍板 3.6-a 四接缝一次到位；接缝1 已在 Step 1）：**接缝3 F1 投影补全**——审计后校准：`session-workbench/timelineModel.ts` 的 cell 投影（user_turn/assistant_final/active_run/boundary + `buildThreadTimelineCached` 稳定缓存）实为 F1 骨架已存，不推倒；本轮把 RuntimePhase 接为投影一等输入：`BuildThreadTimelineInput.runtimePhase`、active_run cell 新增 `phase` 字段（pending cell 与最后 open run cell 附着，settled run 不带）、header status 由 parked/working 相位直接推导、缓存签名与 `sameCellIdentity` 均含 phase（live 转换必然失效重建——缓存复用漏比 phase 的 bug 被红测抓出后根修）。**接缝2 activeTail 工作态 cell**——`ActiveTailStatusLine` 组件（相位文案 i18n en+zh 12 相位全表、活秒表 tabular-nums 每秒 tick、shimmer 复用 codex 同款 `session-tui-shimmer`、tool_running 时附最后 running step 的工具标题、awaiting_approval/awaiting_budget parked 态 warning 点不转动、未知相位前向兼容渲染空），挂在 active_run cell 的 `RunDisclosureBlock` 下方；等首 token（starting/thinking）、tool 间隙、审批/预算等待全部有可见工作态 + 秒表——8 态清单中原零表达的①④⑤⑦全数接活。**接缝4 中间↔右栏关联**——中间流 runtime 事件卡与右栏 `session-runtime-row` 互挂 `data-runtime-link-id`（同源 `eventRuntimeTaskId`/`childSessionId`），hover 双向高亮走纯 DOM class 切换（`setRuntimeLinkHighlight`，零 React 重渲——remount 风暴教训），右栏非跳转 row 点击 `scrollToRuntimeLinkMarker` 滚到中间流 marker；样式克制（背景 + inset hairline，零阴影，设计法 §3.1-6）。 | `timelineModel.test.ts` 新增 5 pins（pending cell 带 phase/open run cell 附 phase/settled 不带/parked 相位推 header waiting/缓存签名失效钉——最后一个抓出 `sameCellIdentity` 漏比 phase 的真 bug）28 passed；`AgentDetailSections.test.tsx` 新增 3 pins（相位标签+工具 detail+秒表节点/未知相位渲染空/parked data-phase）94 passed；前端全量 84 files / 522 passed + `tsc --noEmit` 通过 + 生产 build 成功；i18n en/zh 各 +14 行纯插入（JSON roundtrip 会折叠既有重复键，改用文本级插入避免 300 行假 diff）。 | ✅ |
...
- **后端全量**：`pytest tests -q` → **5997 passed / 1 skipped**（Docker-on 环境，真 PG Testcontainers 全跑）。本轮引入的回归为零：全量首跑暴露的 11 failed 中 6 个归本轮（`test_cc_permission_modes`×5 = 管线新字段对 SimpleNamespace duck-typed deps 直接属性访问 → 改 `getattr` 防御；`test_alembic_single_head` = head 钉测随新迁移 `runtime_budget_run_metadata_0709` 常规更新），已全部修复。
- **既有环境性失败 13 个（5 failed + 8 errors）非本轮引入**：在改动前基线 `be8da95fb` 的 worktree 上逐文件复跑同样失败——`tests/migrations/test_workflow_migration.py` 8 ERROR（迁移链 fixture 在 session 级共享 PG container 上重放不幂等，`DuplicateTableError: runtime_budget_policies already exists`）、`test_subagent_run_service`×2 / `test_subagent_wake_consumer`×2 / `test_workflow_runtime_service`×1（real_pg 时序断言 `'pending' == 'running'` 类）。上午基线的 "5732 passed / 0 failed" 是 Docker-off（~109 真 PG 用例 skip）环境数字；这批失败属主线测试基建（隔离/幂等）债，建议单独一轮修复。
- **前端全量**：84 files / **522 passed** + `tsc --noEmit` 通过 + 生产 build 成功。
- **技术债扫描**：本轮全部 diff（be8da95fb..HEAD，34 文件 +3838/-127）零 TODO/FIXME/占位/default-off 藏半成品；`not_yet_supported` 全仓仅剩 grep 零命中（C6-hook 翻旗为 `governance_preflight`）。
