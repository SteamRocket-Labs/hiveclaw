...
| D-01 | T0 事件补齐 runtime metadata discipline | `services/t0_logger.py`、T0 writers | T0 event schema tests | 已完成：本提交；Red targeted test -> JSONL missing top-level `turn_id` / `intent_id`；Green targeted -> 1 passed, T0 ledger file -> 10 passed；Lint `ruff check app/memory/t0/ledger.py tests/memory/test_t0_session_ledger.py` -> All checks passed |
