...
async def test_real_schema_readiness_detects_force_drift_and_survives_rollback_reupgrade(pg_container) -> None:
    ...
    try:
        ...
        assert "rls_not_forced" in {
            issue.code for issue in drifted.issues if issue.object_name == "runtime_tasks"
        }
    ...
    try:
        ...
        assert "alembic_head_mismatch" in {issue.code for issue in rolled_back.issues}
    finally:
        await engine.dispose()
