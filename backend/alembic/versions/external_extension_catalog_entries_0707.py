...
def upgrade() -> None:
    op.create_table(
        ...
        sa.UniqueConstraint(
            "tenant_id", "snapshot_id", "qualified_name", name="uq_external_catalog_snapshot_component"
        ),
        if_not_exists=True,
    )
    op.create_index(
        op.f("ix_external_extension_catalog_entries_tenant_id"),
        "external_extension_catalog_entries",
        ["tenant_id"],
        if_not_exists=True,
    )
    op.create_index(
        op.f("ix_external_extension_catalog_entries_snapshot_id"),
        "external_extension_catalog_entries",
        ["snapshot_id"],
        if_not_exists=True,
    )
    ...
    op.create_index(
        op.f("ix_external_extension_catalog_entries_policy"),
        "external_extension_catalog_entries",
        ["policy"],
        if_not_exists=True,
    )
    op.create_index(
        op.f("ix_external_extension_catalog_entries_status"),
        "external_extension_catalog_entries",
        ["status"],
        if_not_exists=True,
    )
...
def downgrade() -> None:
    op.execute(
        "DROP POLICY IF EXISTS tenant_isolation_external_extension_catalog_entries ON external_extension_catalog_entries"
    )
